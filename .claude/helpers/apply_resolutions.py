...
def update_conflicts(migration: dict, resolutions: dict) -> dict:
    ...
    migration['conflicts'] = [
        c for c in migration['conflicts']
        if not any(dest in c for dest in resolved_dests)
    ]
...
def main():
    ...
    print(f"Auto-resolved: {len(resolutions['actions']['KEEP_ONE']) + len(resolutions['actions']['RENAME'])}")
