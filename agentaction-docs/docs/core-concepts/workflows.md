...
#### `observe` - Pass Input Fields to Output

Fields in `observe` are:
- **Pass-through from input** (not generated by LLM, copied from input to output)
- **Included in the action's output** (written to output file)
- **AVAILABLE to next agent's LLM** (next agent can reference them in prompts)

Use `observe` for:
- Passing input metadata through the workflow
- Enriching LLM-generated output with input fields
- Correlation IDs and tracking information

```yaml
actions:
  - name: extractor
    schema: {summary: string, entities: array}
    observe: [document_id, timestamp, source_url]
    # document_id, timestamp, source_url from INPUT
    # are added to OUTPUT (alongside summary & entities)
    prompt: "Extract entities and create a summary"

  - name: classifier
    prompt: |
      Classify document {extractor.document_id}
      Summary: {extractor.summary}
      Entities: {extractor.entities}
    depends_on: ["extractor"]
    # ✅ Can reference document_id (from observe)
    # ✅ Can reference summary (from schema)
    # ✅ Can reference entities (from schema)
```
...
2. **LLM Context**: `(schema fields + observe fields) - drops` from previous action
3. **LLM Generates**: Only fields defined in current action's `schema`
4. **Action Output**: `schema` fields (LLM-generated) + `observe` fields (pass-through from input)
5. **Next Agent Sees**: All fields from output (schema + observe, minus drops)
...
actions:
  ...
  - name: classify_entities
    ...
    prompt: |
      ...
      Document: {extract_entities.document_id}
      # ✅ Can reference document_id and created_at from previous action
      # They're in observe, so they're available to this LLM
```

**Key Benefits:**
- ✅ **Enrich output** by passing input fields through with `observe`
- ✅ **Preserve context** for downstream actions (observe fields are referenceable)
- ✅ **Clean up** temporary data with `drops`
- ✅ **Explicit control** over what fields propagate through workflow
...
Agent Actions **automatically validates** field references in prompts when configs are loaded, preventing runtime errors.

#### What Gets Validated

Every field reference like `{agent.field}` is checked to ensure:
- ✅ Referenced agent is in `depends_on`
- ✅ Field exists in the agent's output (schema or observe)
- ✅ Field is not in `drops` (would be removed from output)
...
agents:
  ...
  - name: analyzer
    prompt: |
      Analyze document {extractor.document_id}
      Summary: {extractor.summary}
      Metrics: {extractor.metrics}
    depends_on: ["extractor"]  # ✅ Valid - all fields available
```
...
❌ {extractor.summary}
   ...
   → Add 'extractor' to dependencies list
```

#### LLM Context Formula

The fields available to an agent's LLM are computed as:
...
agent:
  output_schema:
    properties:
      summary: {}
  observe: [document_id, timestamp]
# LLM context for next agent: {summary, document_id, timestamp}
...
agent:
  ...
  observe: [document_id]
  drops: [internal_id]
# LLM context for next agent: {summary, metrics, document_id}
```

#### When Validation Runs

Validation happens **at config load time**, not at runtime:

```bash
# Config with invalid references
$ agent-actions run workflow.yml

# ❌ Fails immediately with clear errors:
================================================================================
...
- ✅ **Auto-documentation** - See what fields are available

For more details, see [Input Validation Guide](/guides/input-validation)
