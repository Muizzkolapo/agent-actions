...
- **Required**: No
- **Default**: `[]` (empty list)
- **Description**: Fields to exclude from LLM prompt but include in output (passthrough)
- **Example**:
  ```yaml
  observe:
    - original_url
  ...
  ```
- **Notes**:
  - Reduces token usage by excluding fields from LLM
  - Fields still appear in action output
  - Useful for preserving metadata
