...
from agent_actions.integrations.providers.anthropic.provider import AnthropicBatchProvider
...
class TestAnthropicBatchProvider(BaseBatchProviderTests):
    ...
    def provider(self):
        ...
        # Mock exception classes that provider uses
        mock_anthropic_module.APIError = Exception

        # Use patch.dict to add anthropic to sys.modules before import
        with patch.dict('sys.modules', {'anthropic': mock_anthropic_module}):
            # Now the import will use our mock
            provider = AnthropicBatchProvider(api_key="test-anthropic-key")
            provider.client = mock_client

            # Configure mock responses for message batches API
            mock_batch = Mock()
            ...
            # Configure mock for results download
    ...
    def test_anthropic_format_task_uses_params_not_body(
        self,
        provider,
        sample_batch_task
    ):
    ...
    def test_anthropic_format_task_system_as_top_level(
        self,
        provider,
        sample_batch_task
    ):
    ...
    def test_anthropic_parse_tool_use_response(
        self,
        provider,
    ...
    def test_anthropic_parse_text_response(
        self,
        provider,
    ...
    def test_retrieve_invalid_batch_id_raises_error(self, tmp_path):
        ...
        # Mock the APIError exception class that provider tries to catch
        mock_anthropic_module.APIError = Exception
    ...
    # ✅ test_submit_and_retrieve_workflow (overridden above - skipped)
