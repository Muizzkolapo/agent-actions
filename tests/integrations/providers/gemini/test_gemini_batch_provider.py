...
class TestGeminiBatchProvider(BaseBatchProviderTests):
    ...
    def provider(self):
        ...
        """
        # Mock the google.genai modules at sys.modules level
        mock_genai_module = Mock()
        mock_types_module = Mock()
        mock_client = Mock()

        # Configure the Client constructor to return our mock
        ...
        # Use patch.dict to add google.genai to sys.modules before import
        with patch.dict('sys.modules', {
            'google': Mock(),
            'google.genai': mock_genai_module,
            'google.genai.types': mock_types_module
        }):
            with patch('agent_actions.integrations.providers.gemini.provider.GEMINI_AVAILABLE', True):
                with patch('agent_actions.integrations.providers.gemini.provider.genai', mock_genai_module):
                    with patch('agent_actions.integrations.providers.gemini.provider.types', mock_types_module):
                        from agent_actions.integrations.providers.gemini.provider import GeminiBatchProvider
    ...
    def test_gemini_format_task_uses_key_not_custom_id(
        self,
        provider,
        sample_batch_task
    ):
    ...
    def test_gemini_format_task_uses_contents_structure(
        self,
        provider,
        sample_batch_task
    ):
    ...
    def test_gemini_format_task_with_schema_uses_response_schema(
        self,
        provider,
        sample_batch_task
    ):
    ...
    # ==================== Override Incompatible Base Tests ====================
    # Gemini uses "key" instead of "custom_id", so we need custom implementations
    ...
    # ✅ test_format_task_basic (overridden above)
    # ✅ test_format_task_with_schema (overridden above)
    # ✅ test_format_task_no_max_tokens (overridden above)
    # ✅ test_parse_success_response_json (inherited, passes)
    # ✅ test_parse_success_response_string (inherited, passes)
    # ✅ test_parse_error_response (inherited, passes)
    # ✅ test_prepare_tasks_json_mode_true (overridden above)
    # ✅ test_prepare_tasks_json_mode_false (overridden above)
    # ⏭️ test_check_status_returns_valid_state (skipped - different format)
    # ⏭️ test_submit_and_retrieve_workflow (skipped - different API)
    # ⏭️ test_retrieve_invalid_batch_id_raises_error (skipped - different API)
