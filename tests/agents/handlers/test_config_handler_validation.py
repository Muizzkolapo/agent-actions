...
class TestConfigManagerInputValidation:
    ...
    def create_agent_config(self, **kwargs):
        ...
        defaults = {'agent_type': 'llm', 'model_name': 'gpt-4o-mini', 'model_vendor': 'openai', 'prompt': '', 'dependencies': [], 'is_operational': True, 'output_schema': {'properties': {}}}
    ...
    def test_valid_field_references_pass_validation(self, mock_validator_class, mock_topo_sort):
        ...
        config_manager.agent_configs = {'extractor': self.create_agent_config(output_schema={'properties': {'summary': {}, 'metrics': {}}}), 'analyzer': self.create_agent_config(prompt='Analyze {extractor.summary}', dependencies=['extractor'])}
    ...
    def test_invalid_missing_field_raises_error(self, mock_validator_class, mock_topo_sort):
        ...
        config_manager = ConfigManager('test.yml', 'default.yml')
        config_manager.agent_configs = {'extractor': self.create_agent_config(output_schema={'properties': {'summary': {}}}), 'analyzer': self.create_agent_config(prompt='Use {extractor.nonexistent}', dependencies=['extractor'])}
        mock_validator_instance = Mock()
        mock_validator_class.return_value = mock_validator_instance
    ...
    def test_invalid_dropped_field_raises_error(self, mock_validator_class, mock_topo_sort):
        ...
        config_manager = ConfigManager('test.yml', 'default.yml')
        config_manager.agent_configs = {'extractor': self.create_agent_config(output_schema={'properties': {'summary': {}, 'temp': {}}}, drops=['temp']), 'analyzer': self.create_agent_config(prompt='Use {extractor.temp}', dependencies=['extractor'])}
        mock_validator_instance = Mock()
        mock_validator_class.return_value = mock_validator_instance
        with pytest.raises(ConfigValidationError) as exc_info:
            config_manager.determine_execution_order()
    ...
    def test_invalid_undeclared_dependency_raises_error(self, mock_validator_class, mock_topo_sort):
        ...
        config_manager = ConfigManager('test.yml', 'default.yml')
        config_manager.agent_configs = {'extractor': self.create_agent_config(output_schema={'properties': {'summary': {}}}), 'analyzer': self.create_agent_config(prompt='Use {unknown.field}', dependencies=['extractor'])}
        mock_validator_instance = Mock()
        mock_validator_class.return_value = mock_validator_instance
        with pytest.raises(ConfigValidationError) as exc_info:
            config_manager.determine_execution_order()
    ...
    def test_valid_observe_field_passes(self, mock_validator_class, mock_topo_sort):
        ...
        config_manager = ConfigManager('test.yml', 'default.yml')
        config_manager.agent_configs = {'extractor': self.create_agent_config(output_schema={'properties': {'summary': {}}}, observe=['document_id', 'metadata']), 'analyzer': self.create_agent_config(prompt='Process {extractor.document_id}', dependencies=['extractor'])}
        mock_validator_instance = Mock()
        mock_validator_class.return_value = mock_validator_instance
        mock_topo_sort.return_value = ['extractor', 'analyzer']
        config_manager.determine_execution_order()
    ...
    def test_special_source_reference_passes(self, mock_validator_class, mock_topo_sort):
        ...
        config_manager = ConfigManager('test.yml', 'default.yml')
        config_manager.agent_configs = {'extractor': self.create_agent_config(prompt='Extract from {source.page_content}', dependencies=[])}
        mock_validator_instance = Mock()
        mock_validator_class.return_value = mock_validator_instance
    ...
    def test_special_loop_reference_passes(self, mock_validator_class, mock_topo_sort):
        ...
        config_manager = ConfigManager('test.yml', 'default.yml')
        config_manager.agent_configs = {'processor': self.create_agent_config(prompt='Processing item {loop.index} of {loop.total}', dependencies=[])}
        mock_validator_instance = Mock()
        mock_validator_class.return_value = mock_validator_instance
    ...
    def test_special_workflow_reference_passes(self, mock_validator_class, mock_topo_sort):
        ...
        config_manager = ConfigManager('test.yml', 'default.yml')
        config_manager.agent_configs = {'reporter': self.create_agent_config(prompt='Workflow: {workflow.name} v{workflow.version}', dependencies=[])}
        mock_validator_instance = Mock()
        mock_validator_class.return_value = mock_validator_instance
    ...
    def test_multiple_errors_all_reported(self, mock_validator_class, mock_topo_sort):
        ...
        config_manager = ConfigManager('test.yml', 'default.yml')
        config_manager.agent_configs = {'extractor': self.create_agent_config(output_schema={'properties': {'summary': {}}}), 'analyzer': self.create_agent_config(prompt='Use {extractor.missing1} and {extractor.missing2}', dependencies=['extractor'])}
        mock_validator_instance = Mock()
        mock_validator_class.return_value = mock_validator_instance
        with pytest.raises(ConfigValidationError) as exc_info:
            config_manager.determine_execution_order()
    ...
    def test_no_prompt_skips_validation(self, mock_validator_class, mock_topo_sort):
        ...
        config_manager = ConfigManager('test.yml', 'default.yml')
        config_manager.agent_configs = {'function_agent': self.create_agent_config(agent_type='function', prompt='', dependencies=[])}
        mock_validator_instance = Mock()
        mock_validator_class.return_value = mock_validator_instance
    ...
    def test_complex_workflow_with_all_directives(self, mock_validator_class, mock_topo_sort):
        ...
        config_manager = ConfigManager('test.yml', 'default.yml')
        config_manager.agent_configs = {'extractor': self.create_agent_config(output_schema={'properties': {'summary': {}, 'metrics': {}, 'temp': {}}}, observe=['document_id'], drops=['temp']), 'analyzer': self.create_agent_config(prompt='\n                Source: {source.title}\n                Summary: {extractor.summary}\n                Metrics: {extractor.metrics}\n                Document ID: {extractor.document_id}\n                ', dependencies=['extractor'])}
        mock_validator_instance = Mock()
        mock_validator_class.return_value = mock_validator_instance
        mock_topo_sort.return_value = ['extractor', 'analyzer']
        config_manager.determine_execution_order()
    ...
    def test_non_operational_agents_not_validated(self, mock_validator_class, mock_topo_sort):
        ...
        config_manager = ConfigManager('test.yml', 'default.yml')
        config_manager.agent_configs = {'disabled_agent': self.create_agent_config(prompt='Use {nonexistent.field}', is_operational=False), 'active_agent': self.create_agent_config(prompt='Valid prompt', is_operational=True)}
        mock_validator_instance = Mock()
        mock_validator_class.return_value = mock_validator_instance
...
class TestFormatInputValidationErrors:
    ...
    def test_format_single_error(self):
        ...
        from agent_actions.validation.input_signature_validator import ValidationResult, FieldValidationResult
        config_manager = ConfigManager('test.yml', 'default.yml')
    ...
    def test_format_multiple_errors_multiple_agents(self):
        ...
        result1.errors.append(FieldValidationResult(field_reference='{x.field}', agent_name='x', field_name='field', is_valid=False, message='Error 1'))
        result2 = ValidationResult(agent_name='agent2')
        result2.errors.append(FieldValidationResult(field_reference='{y.field}', agent_name='y', field_name='field', is_valid=False, message='Error 2'))
        formatted = config_manager._format_input_validation_errors([('agent1', result1), ('agent2', result2)])
