...
class TestNewFormatConfigHandlerIntegration:
    ...
    def basic_new_format_workflow(self):
        ...
        return {'name': 'test_workflow', 'description': 'Test workflow in new format', 'version': '2.0.0', 'defaults': {'model_vendor': 'openai', 'api_key': 'TEST_API_KEY', 'model_name': 'gpt-4'}, 'actions': [{'name': 'extract_data', 'intent': 'Extract structured data from content', 'kind': 'llm', 'reads': ['content'], 'writes': ['extracted_data'], 'prompt': 'Extract data from the content'}, {'name': 'process_data', 'intent': 'Process extracted data', 'kind': 'llm', 'reads': ['extracted_data'], 'writes': ['processed_data'], 'prompt': 'Process the extracted data'}], 'plan': ['extract_data', 'process_data <- extract_data']}

    @pytest.fixture
    def project_config(self):
        """Project-level configuration (agent_actions.yml)."""
        return {'default_agent_config': {'model_name': 'gpt-4-project-default', 'model_vendor': 'openai', 'api_key': 'TEST_API_KEY', 'chunk_config': {'chunk_size': 800, 'chunk_overlap': 150}, 'is_operational': True, 'run_mode': 'online', 'json_mode': False, 'granularity': 'record', 'few_shot': 2}, 'tool_path': 'project_tools/'}

    @pytest.fixture
    def hierarchy_test_workflow(self):
        """Workflow for testing configuration hierarchy."""
        return {'name': 'test_project_hierarchy', 'description': 'Test workflow with project config integration', 'version': '2.0.0', 'defaults': {'model_name': 'gpt-4o-mini-workflow', 'model_vendor': 'openai', 'api_key': 'TEST_API_KEY', 'json_mode': True, 'granularity': 'file'}, 'actions': [{'name': 'action_with_project_inheritance', 'intent': 'Action inheriting from project + workflow defaults', 'kind': 'llm', 'reads': ['content'], 'writes': ['extracted_data'], 'prompt': 'Extract using hierarchy: project < workflow < action'}, {'name': 'action_with_overrides', 'intent': 'Action overriding all levels', 'kind': 'llm', 'model_name': 'claude-3-sonnet', 'model_vendor': 'anthropic', 'api_key': 'TEST_API_KEY', 'json_mode': False, 'few_shot': 5, 'reads': ['extracted_data'], 'writes': ['processed_data'], 'prompt': 'Process with action-level overrides'}], 'plan': ['action_with_project_inheritance', 'action_with_overrides <- action_with_project_inheritance']}
    ...
    def test_basic_new_format_loading(self, temp_dir, basic_new_format_workflow):
        ...
        workflow_path, default_path = self.create_test_files(temp_dir, basic_new_format_workflow, basic_new_format_workflow)
    ...
    def test_new_format_agent_conversion(self, temp_dir, basic_new_format_workflow):
        ...
        assert extract_agent['model_vendor'] == 'openai'
        assert extract_agent['model_name'] == 'gpt-4'
    ...
    def test_plan_parsing_with_dependencies(self, temp_dir):
        ...
        workflow = {'name': 'dependency_test', 'description': 'Test dependency parsing', 'version': '2.0.0', 'defaults': {'model_vendor': 'openai', 'model_name': 'gpt-4', 'api_key': 'TEST_API_KEY'}, 'actions': [{'name': 'step1', 'intent': 'First step', 'kind': 'llm', 'reads': ['input'], 'writes': ['step1_output'], 'prompt': 'Process step 1'}, {'name': 'step2', 'intent': 'Second step', 'kind': 'llm', 'reads': ['step1_output'], 'writes': ['step2_output'], 'prompt': 'Process step 2'}, {'name': 'step3', 'intent': 'Third step', 'kind': 'llm', 'reads': ['step1_output', 'step2_output'], 'writes': ['final_output'], 'prompt': 'Process step 3'}], 'plan': ['step1', 'step2 <- step1', 'step3 <- step1, step2']}
    ...
    def test_plan_filters_actions_correctly(self, temp_dir):
        ...
        workflow = {'name': 'filter_test', 'description': 'Test action filtering', 'version': '2.0.0', 'defaults': {'model_vendor': 'openai', 'model_name': 'gpt-4', 'api_key': 'TEST_API_KEY'}, 'actions': [{'name': 'included_action', 'intent': 'This action is in the plan', 'kind': 'llm', 'reads': ['input'], 'writes': ['output'], 'prompt': 'Process data'}, {'name': 'excluded_action', 'intent': 'This action is NOT in the plan', 'kind': 'llm', 'reads': ['input'], 'writes': ['output'], 'prompt': 'Process data'}], 'plan': ['included_action']}
        workflow_path, default_path = self.create_test_files(temp_dir, workflow, workflow)
        config_manager = ConfigManager(workflow_path, default_path)
        config_manager.load_configs()
        user_agents = config_manager.get_user_agents()
    ...
    def test_chunking_configuration_support(self, temp_dir):
        ...
        workflow = {'name': 'chunking_test', 'description': 'Test chunking config support', 'version': '2.0.0', 'defaults': {'model_vendor': 'openai', 'model_name': 'gpt-4', 'api_key': 'TEST_API_KEY', 'chunk_config': {'chunk_size': 1000, 'chunk_overlap': 200}}, 'actions': [{'name': 'chunked_action', 'intent': 'Action with chunking config', 'kind': 'llm', 'chunk_config': {'chunk_size': 500, 'chunk_overlap': 100}, 'reads': ['content'], 'writes': ['processed'], 'prompt': 'Process with chunking'}], 'plan': ['chunked_action']}
        workflow_path, default_path = self.create_test_files(temp_dir, workflow, workflow)
        config_manager = ConfigManager(workflow_path, default_path)
        config_manager.load_configs()
        user_agents = config_manager.get_user_agents()
    ...
    def test_comprehensive_defaults_application(self, temp_dir):
        ...
        workflow = {'name': 'defaults_test', 'description': 'Test comprehensive defaults', 'version': '2.0.0', 'defaults': {'model_name': 'gpt-4-default', 'model_vendor': 'openai', 'api_key': 'TEST_API_KEY', 'json_mode': True, 'granularity': 'file', 'run_mode': 'offline', 'few_shot': 3}, 'actions': [{'name': 'test_action', 'intent': 'Action with defaults', 'kind': 'llm', 'reads': ['input'], 'writes': ['output'], 'prompt': 'Process data'}], 'plan': ['test_action']}
        workflow_path, default_path = self.create_test_files(temp_dir, workflow, workflow)
        config_manager = ConfigManager(workflow_path, default_path)
        config_manager.load_configs()
        user_agents = config_manager.get_user_agents()
    ...
    def test_project_config_hierarchy(self, temp_dir, hierarchy_test_workflow, project_config):
        ...
        workflow_path, default_path = self.create_test_files(temp_dir, hierarchy_test_workflow, hierarchy_test_workflow)
        with patch('agent_actions.state_management.context.path_manager.PathManager.get_project_root', return_value=str(temp_dir)), patch('agent_actions.agents.handlers.config_handler.load_project_config', return_value=project_config):
            config_manager = ConfigManager(workflow_path, default_path)
            config_manager.load_configs()
            user_agents = config_manager.get_user_agents()
            assert len(user_agents) == 2
            ...
            assert inheritance_agent['model_name'] == 'gpt-4o-mini-workflow'
            assert inheritance_agent['json_mode'] is True
            assert inheritance_agent['granularity'] == 'File'
            assert inheritance_agent['model_vendor'] == 'openai'
            assert inheritance_agent['few_shot'] == 2
            ...
            assert override_agent['model_name'] == 'claude-3-sonnet'
            assert override_agent['model_vendor'] == 'anthropic'
            assert override_agent['json_mode'] is False
            assert override_agent['few_shot'] == 5

    def test_tool_actions_handling(self, temp_dir):
        """Test handling of tool-type actions (kind: tool)."""
        templates_dir = temp_dir / 'templates'
        templates_dir.mkdir(exist_ok=True)
        workflow = {'name': 'tool_test', 'description': 'Test tool action handling', 'version': '2.0.0', 'actions': [{'name': 'tool_action', 'intent': 'Tool-based action', 'kind': 'tool', 'impl': 'custom_tool', 'reads': ['input'], 'writes': ['output']}], 'plan': ['tool_action']}
        workflow_path, default_path = self.create_test_files(temp_dir, workflow, workflow)
    ...
    def test_tool_action_batch_mode_override(self, temp_dir):
        ...
        templates_dir = temp_dir / 'templates'
        templates_dir.mkdir(exist_ok=True)
        workflow = {'name': 'tool_batch_test', 'description': 'Test tool action with batch mode defaults', 'version': '2.0.0', 'defaults': {'model_vendor': 'openai', 'api_key': 'TEST_API_KEY', 'model_name': 'gpt-4', 'run_mode': 'batch'}, 'actions': [{'name': 'tool_action', 'intent': 'Tool-based action', 'kind': 'tool', 'impl': 'custom_tool', 'reads': ['input'], 'writes': ['output']}], 'plan': ['tool_action']}
        workflow_path, default_path = self.create_test_files(temp_dir, workflow, workflow)
        with patch('pathlib.Path.cwd', return_value=temp_dir):
            config_manager = ConfigManager(workflow_path, default_path)
            config_manager.load_configs()
    ...
    def test_tool_action_explicit_batch_mode_raises_error(self, temp_dir):
        ...
        templates_dir = temp_dir / 'templates'
        templates_dir.mkdir(exist_ok=True)
        workflow = {'name': 'tool_batch_error_test', 'description': 'Test tool action with explicit batch mode', 'version': '2.0.0', 'actions': [{'name': 'tool_action', 'intent': 'Tool-based action', 'kind': 'tool', 'impl': 'custom_tool', 'run_mode': 'batch', 'reads': ['input'], 'writes': ['output']}], 'plan': ['tool_action']}
        workflow_path, default_path = self.create_test_files(temp_dir, workflow, workflow)
        with patch('pathlib.Path.cwd', return_value=temp_dir):
            config_manager = ConfigManager(workflow_path, default_path)
            config_manager.load_configs()
    ...
    def test_schema_handling(self, temp_dir):
        ...
        workflow = {'name': 'schema_test', 'description': 'Test schema handling', 'version': '2.0.0', 'defaults': {'model_vendor': 'openai', 'model_name': 'gpt-4', 'api_key': 'TEST_API_KEY'}, 'actions': [{'name': 'schema_action', 'intent': 'Action with output schema', 'kind': 'llm', 'schema': 'test_schema.json', 'reads': ['input'], 'writes': ['output'], 'prompt': 'Extract with schema'}], 'plan': ['schema_action']}
        workflow_path, default_path = self.create_test_files(temp_dir, workflow, workflow)
        config_manager = ConfigManager(workflow_path, default_path)
        config_manager.load_configs()
        user_agents = config_manager.get_user_agents()
...
class TestNewFormatFeatureIntegration:
    ...
    def test_end_to_end_new_format_processing(self, tmp_path):
        ...
        workflow = {'name': 'e2e_test', 'description': 'End-to-end test workflow', 'version': '2.0.0', 'defaults': {'model_name': 'gpt-4', 'model_vendor': 'openai', 'api_key': 'TEST_API_KEY', 'json_mode': False, 'granularity': 'record'}, 'actions': [{'name': 'extract', 'intent': 'Extract data from content', 'kind': 'llm', 'json_mode': True, 'reads': ['raw_content'], 'writes': ['structured_data'], 'prompt': 'Extract structured data'}, {'name': 'validate', 'intent': 'Validate extracted data', 'kind': 'llm', 'granularity': 'file', 'reads': ['structured_data'], 'writes': ['validated_data'], 'prompt': 'Validate the data'}, {'name': 'transform', 'intent': 'Transform validated data', 'kind': 'llm', 'reads': ['validated_data'], 'writes': ['final_output'], 'prompt': 'Transform the data'}], 'plan': ['extract', 'validate <- extract', 'transform <- validate']}
        ...
        assert extract_agent['json_mode'] is True
        assert extract_agent['granularity'] == 'Record'
        validate_agent = next((a for a in user_agents if a['agent_type'] == 'validate'))
        assert validate_agent['granularity'] == 'File'
        assert validate_agent['json_mode'] is False
    ...
    def test_standard_field_names_all_present(self, tmp_path):
        ...
        workflow = {'name': 'test', 'version': '1.0', 'description': 'Test standard field names', 'defaults': {'model_vendor': 'openai', 'model_name': 'gpt-4', 'api_key': 'DEFAULT_KEY'}, 'actions': [{'name': 'test_action', 'intent': 'Test', 'kind': 'llm', 'reads': [], 'writes': [], 'prompt': 'test'}], 'plan': ['test_action']}
    ...
    def test_standard_field_names_at_workflow_level(self, tmp_path):
        ...
        workflow = {'name': 'test', 'version': '1.0', 'description': 'Test workflow defaults', 'defaults': {'model_vendor': 'anthropic', 'model_name': 'claude-3-5-sonnet', 'api_key': 'WORKFLOW_KEY'}, 'actions': [{'name': 'test_action', 'intent': 'Test', 'kind': 'llm', 'reads': [], 'writes': [], 'prompt': 'test'}], 'plan': ['test_action']}
        workflow_file = tmp_path / 'test.yml'
        with open(workflow_file, 'w') as f:
            yaml.dump(workflow, f)
        config_manager = ConfigManager(str(workflow_file), str(workflow_file))
    ...
    def test_standard_field_names_at_action_level(self, tmp_path):
        ...
        workflow = {'name': 'test', 'version': '1.0', 'description': 'Test action-level config', 'actions': [{'name': 'test_action', 'intent': 'Test', 'kind': 'llm', 'model_vendor': 'openai', 'model_name': 'gpt-4o-mini', 'api_key': 'ACTION_KEY', 'reads': [], 'writes': [], 'prompt': 'test'}], 'plan': ['test_action']}
        workflow_file = tmp_path / 'test.yml'
        with open(workflow_file, 'w') as f:
            yaml.dump(workflow, f)
        config_manager = ConfigManager(str(workflow_file), str(workflow_file))
    ...
    def test_standard_field_names_hierarchy_precedence(self, tmp_path):
        ...
        workflow = {'name': 'test', 'version': '1.0', 'description': 'Test hierarchy precedence', 'defaults': {'model_vendor': 'anthropic', 'model_name': 'claude-3-5-sonnet', 'api_key': 'WORKFLOW_KEY'}, 'actions': [{'name': 'test_action', 'intent': 'Test', 'kind': 'llm', 'model_vendor': 'openai', 'model_name': 'gpt-4o-mini', 'reads': [], 'writes': [], 'prompt': 'test'}], 'plan': ['test_action']}
        workflow_file = tmp_path / 'test.yml'
        with open(workflow_file, 'w') as f:
            yaml.dump(workflow, f)
        config_manager = ConfigManager(str(workflow_file), str(workflow_file))
