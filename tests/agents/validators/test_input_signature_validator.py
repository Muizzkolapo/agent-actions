...
class TestValidateAgentInputs:
    ...
    def test_invalid_missing_field_reference(self):
        ...
        agent_config = {
            'prompt': 'Get {extractor.nonexistent}',
            'dependencies': ['extractor']
        }
        dep_configs = {
            'extractor': {
        ...
        result = InputSignatureValidator.validate_agent_inputs(
            agent_config, dep_configs, 'getter'
        )
    ...
    def test_multiple_valid_references(self):
        ...
        dep_configs = {
            ...
            'classifier': {
                'output_schema': {'properties': {'label': {}, 'confidence': {}}}
            }
        }

        result = InputSignatureValidator.validate_agent_inputs(
            agent_config, dep_configs, 'combiner'
        )
    ...
    def test_mixed_valid_and_invalid_references(self):
        ...
        agent_config = {
            'prompt': 'Valid: {extractor.summary} Invalid: {extractor.missing}',
            'dependencies': ['extractor']
        }
        dep_configs = {
            'extractor': {
    ...
    def test_special_source_reference(self):
        ...
        result = InputSignatureValidator.validate_agent_inputs(
            agent_config, dep_configs, 'extractor'
        )
    ...
    def test_special_workflow_reference(self):
        ...
        result = InputSignatureValidator.validate_agent_inputs(
            agent_config, dep_configs, 'reporter'
        )
    ...
    def test_nested_field_validates_first_level_only(self):
        ...
        dep_configs = {
            'extractor': {
                'output_schema': {'properties': {'data': {}, 'summary': {}}}
            }
        }

        result = InputSignatureValidator.validate_agent_inputs(
            agent_config, dep_configs, 'counter'
        )

        assert result.is_valid()
        assert len(result.successes) == 1
        # Validates that 'data' exists, not 'metrics' or 'count'
    ...
    def test_empty_prompt_returns_valid(self):
        ...
        result = InputSignatureValidator.validate_agent_inputs(
            agent_config, dep_configs, 'agent'
        )
    ...
    def test_no_references_returns_valid(self):
        ...
        agent_config = {
            'prompt': 'This is a static prompt with no references',
            'dependencies': ['extractor']
        }
        dep_configs = {
            'extractor': {
    ...
    def test_missing_dependency_config(self):
        ...
        agent_config = {
            'prompt': 'Use {extractor.summary}',
            'dependencies': ['extractor']
        }
        dep_configs = {}  # Missing extractor config
    ...
    def test_depends_on_field_support(self):
        ...
        agent_config = {
            'prompt': 'Use {extractor.summary}',
            'depends_on': ['extractor']  # Using depends_on instead of dependencies
        }
        dep_configs = {
            'extractor': {
                'output_schema': {'properties': {'summary': {}}}
...
class TestFormatValidationErrors:
    ...
    def test_format_multiple_errors(self):
        ...
        result.errors.extend([
            FieldValidationResult(
                ...
                is_valid=False,
                message="Field 'missing' not available",
                help_text="Available fields: ['summary']"
            ),
            FieldValidationResult(
                field_reference='{unknown.field}',
                agent_name='unknown',
                field_name='field',
                is_valid=False,
                message="Agent 'unknown' not in dependencies",
                help_text="Add 'unknown' to dependencies list"
            )
        ])
    ...
    def test_format_no_errors_returns_empty(self):
        ...
        result.successes.append(
            FieldValidationResult(
                field_reference='{extractor.summary}',
                agent_name='extractor',
                field_name='summary',
                is_valid=True,
                message='Valid'
