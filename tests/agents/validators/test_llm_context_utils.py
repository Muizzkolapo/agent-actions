...
import pytest
from agent_actions.preprocessing.context.llm_context_utils import LLMContextUtils
...
class TestComputeLLMContext:
    ...
    def test_with_context_scope_exclude(self):
        ...
        config = {
            "output_schema": {
                "properties": {"summary": {}, "metrics": {}, "internal_id": {}, "temp_data": {}}
            }
        }
        result = LLMContextUtils.compute_llm_context(config)
    ...
    def test_with_context_scope_passthrough(self):
        ...
        config = {
            ...
            "context_scope": {
                "passthrough": ["upstream.document_id", "upstream.author", "upstream.metadata"]
            },
        }
        result = LLMContextUtils.compute_llm_context(config)
    ...
    def test_all_directives_combined(self):
        ...
        config = {
            ...
            "context_scope": {
                "passthrough": ["upstream.document_id", "upstream.author", "upstream.original_text"]
            },
        }
        result = LLMContextUtils.compute_llm_context(config)
        assert result == {
