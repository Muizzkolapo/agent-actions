...
| `TestStructuralGate` | 4 | A schema-violating record is repaired; the `_structural` failure reaches the repair prompt under `structural: auto`; by default a structural failure re-runs the original prompt; observe mode never applies the gate |
| `TestExhaustion` | 4 | `return_last` ships the annotated record; `fail` refuses to ship it; `raise` halts the run; `max_iterations` counts the first generation |
