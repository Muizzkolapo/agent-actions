...
def _submitted_batches(project):
    ...
    Records accumulate: `AgacBatchClient._forget_state` has no callers, by its own
    decision, so a read never spends the record. Callers therefore compare against
    a before-snapshot rather than reading a count.
    """
