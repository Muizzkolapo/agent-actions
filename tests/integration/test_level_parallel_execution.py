...
class TestLevelBasedParallelExecution:
    ...
    async def test_two_parallel_groups_with_intermediate(self):
        ...
        """
        # This test verifies the core use case that motivated the level-based design
        execution_order = ['extract', 'gen_1', 'gen_2', 'gen_3', 'validate', 'enrich_1', 'enrich_2', 'enrich_3']
        agent_configs = {
            'extract': {'dependencies': [], 'agent_type': 'extract'},
            'gen_1': {'dependencies': ['extract'], 'agent_type': 'gen_1'},
            'gen_2': {'dependencies': ['extract'], 'agent_type': 'gen_2'},
            'gen_3': {'dependencies': ['extract'], 'agent_type': 'gen_3'},
            'validate': {'dependencies': ['gen_1', 'gen_2', 'gen_3'], 'agent_type': 'validate'},
            'enrich_1': {'dependencies': ['validate'], 'agent_type': 'enrich_1'},
            'enrich_2': {'dependencies': ['validate'], 'agent_type': 'enrich_2'},
            'enrich_3': {'dependencies': ['validate'], 'agent_type': 'enrich_3'},
        }
    ...
    async def test_parallel_speedup_measurement(self):
        ...
        # Simple test: 3 agents @ 0.1s each
        # Sequential: ~0.3s, Parallel: ~0.1s

        execution_order = ['A', 'B', 'C']
        agent_configs = {
            'A': {'dependencies': [], 'agent_type': 'A'},
            'B': {'dependencies': [], 'agent_type': 'B'},
            'C': {'dependencies': [], 'agent_type': 'C'},
        }
        ...
        # Should be single level with all 3 agents
        assert len(levels) == 1
        ...
        async def mock_agent_run(agent_name):
            await asyncio.sleep(0.1)  # Each agent takes 0.1s
    ...
    async def test_error_stops_subsequent_levels(self):
        ...
        agent_configs = {
            'A': {'dependencies': [], 'agent_type': 'A'},
            'B': {'dependencies': ['A'], 'agent_type': 'B'},
            'C': {'dependencies': ['B'], 'agent_type': 'C'},  # This will fail
            'D': {'dependencies': ['C'], 'agent_type': 'D'},
        }
        ...
        # Verify levels: [A], [B], [C], [D]
        assert len(levels) == 4

        # Simulate error in level 2 (C)
        ...
        # Level 3 (D) should never execute
        # (This is implicit - the error stops execution)
    ...
    async def test_batch_agents_in_parallel_level(self):
        ...
        agent_configs = {
            'batch_1': {'dependencies': [], 'agent_type': 'batch_1', 'run_mode': 'batch'},
            'batch_2': {'dependencies': [], 'agent_type': 'batch_2', 'run_mode': 'batch'},
            'batch_3': {'dependencies': [], 'agent_type': 'batch_3', 'run_mode': 'batch'},
        }
        ...
        # All batch agents should be in same level (can submit in parallel)
        assert len(levels) == 1
    ...
    async def test_sequential_workflow_creates_single_agent_levels(self):
        ...
        # Should be 4 levels, each with 1 agent
        assert len(levels) == 4
    ...
    async def test_concurrency_limit_respected(self):
        ...
        # 5 agents in same level, concurrency limit = 2
        execution_order = ['A', 'B', 'C', 'D', 'E']
        agent_configs = {
            'A': {'dependencies': [], 'agent_type': 'A'},
            'B': {'dependencies': [], 'agent_type': 'B'},
            'C': {'dependencies': [], 'agent_type': 'C'},
            'D': {'dependencies': [], 'agent_type': 'D'},
            'E': {'dependencies': [], 'agent_type': 'E'},
        }
