...
class TestRenderCommand:
    ...
    def test_render_template_deterministic_output(self, tmp_path):
        ...
        args = RenderCommandArgs(
            agent_name="deterministic_agent",
            template_dir=str(template_dir)
        )
        command = RenderCommand(args)
        ...
        with patch('agent_actions.tasks.compile.render_pipeline_with_templates') as mock_render:
            ...
            # Verify render was called with same parameters both times
...
class TestRenderCommandExecution:
    ...
    def test_execute_template_rendering_error(self, tmp_path):
        ...
        args = RenderCommandArgs(
            agent_name="template_error_test",
            template_dir=str(template_dir)
        )
        command = RenderCommand(args)
...
class TestRenderCommandConfigValidation:
    ...
    def test_fails_fast_on_invalid_configs_with_helpful_messages(self, tmp_path):
        ...
        args = RenderCommandArgs(
            agent_name="invalid_config_test",
            template_dir=str(template_dir)
        )
        command = RenderCommand(args)

        # Mock invalid configuration scenarios
        invalid_configs = [
            ("empty_config.yml", ""),
            ("malformed.yml", "invalid: yaml: content: ["),
            ("missing_required.yml", "name: missing_type"),
            ("invalid_structure.yml", "invalid_top_level: structure")
        ]
        ...
        for config_name, config_content in invalid_configs:
            ...
            with patch('agent_actions.tasks.services.project_paths_factory.ProjectPathsFactory') as mock_factory:
                ...
                with patch('agent_actions.tasks.compile.render_pipeline_with_templates') as mock_render:
                    # Simulate helpful error message for invalid config
                    mock_render.side_effect = ValidationError(
                        f"Invalid configuration in {config_name}: {config_content[:20]}..."
                    )
                    ...
                    assert "Invalid configuration" in error_msg or \
    ...
    def test_config_schema_validation_errors(self, tmp_path):
        ...
        validation_errors = [
            ...
            "Invalid agent type: unknown_type",
            "Template file not found: nonexistent.j2",
        ...
        for error_message in validation_errors:
            ...
            with patch('agent_actions.tasks.services.project_paths_factory.ProjectPathsFactory') as mock_factory:
                ...
                with patch('agent_actions.tasks.compile.render_pipeline_with_templates') as mock_render:
                    ...
                    # Error should be propagated with helpful context
    ...
    def test_template_syntax_errors(self, tmp_path):
        ...
        args = RenderCommandArgs(
            agent_name="template_syntax_test",
            template_dir=str(template_dir)
        )
        command = RenderCommand(args)
        ...
        template_errors = [
            "Template syntax error: Invalid Jinja2 syntax at line 10",
            "Undefined variable: missing_variable in template",
            "Template file not found: missing_template.j2",
            "Filter not found: custom_filter in template"
        ]
        ...
        for error_message in template_errors:
            with patch('agent_actions.tasks.services.project_paths_factory.ProjectPathsFactory') as mock_factory:
                ...
                with patch('agent_actions.tasks.compile.render_pipeline_with_templates') as mock_render:
                    ...
                    # Should provide helpful template error context
                    assert error_message in str(exc_info.value) or \
                           "ClickException" in str(type(exc_info.value))
...
class TestRenderClickCommand:
    ...
    def test_render_click_command_validation_error(self):
        ...
        # Mock RenderCommandArgs to raise validation error
        with patch('agent_actions.tasks.compile.RenderCommandArgs') as mock_args_class:
            # Simulate a validation error during RenderCommandArgs construction
            mock_args_class.side_effect = PydanticValidationError.from_exception_data(
                'ValidationError',
...
"""
        ...
        # Run render multiple times with identical inputs
        results = []
        for i in range(3):
            command = RenderCommand(args)
    ...
    def test_different_inputs_produce_different_outputs(self, tmp_path):
        ...
        configs = [
            ("config1.yml", "name: test1\ntype: generator"),
            ("config2.yml", "name: test2\ntype: transformer"),
            ("config3.yml", "name: test3\ntype: validator")
        ]
        ...
        for config_name, config_content in configs:
            ...
            args = RenderCommandArgs(
                agent_name=config_name.replace('.yml', ''),
                template_dir=str(template_dir)
            )
            command = RenderCommand(args)
        ...
        assert len(set(results)) == len(results)  # All results are unique
    ...
    def test_template_caching_behavior(self, tmp_path):
        ...
        with patch('agent_actions.tasks.compile.render_pipeline_with_templates') as mock_render:
            ...
            # Multiple calls should use same underlying render function
            result1 = command._render_template(config_file)
            result2 = command._render_template(config_file)
            ...
            # Render function should be called each time (no implicit caching)
            assert mock_render.call_count == 2
