...
class TestToolsResolver:
    ...
    def test_legacy_format_list(self, tmp_path):
        ...
        agent_config = {
            'tool_path': [str(tmp_path), '/other/path'],
            'prompt': 'Test prompt'
        }

        resolved = resolve_tools_path(agent_config)
        assert resolved == str(tmp_path), \
            f"Expected first path from list, got {resolved}"
    ...
    def test_legacy_format_string(self, tmp_path):
        ...
        resolved = resolve_tools_path(agent_config)
        assert resolved == str(tmp_path), \
            f"Expected tool_path string, got {resolved}"
    ...
    def test_simple_format(self, tmp_path):
        ...
        resolved = resolve_tools_path(agent_config)
        assert resolved == str(tmp_path), \
            f"Expected tools.path value, got {resolved}"
    ...
    def test_openai_format(self, tmp_path):
        ...
        resolved = resolve_tools_path(agent_config)
        assert resolved == str(tmp_path), \
            f"Expected module_path from tool config, got {resolved}"
    ...
    def test_no_tools_configured(self):
        ...
        assert resolved is None, \
            f"Expected None when no tools configured, got {resolved}"
    ...
    def test_empty_tool_path_list(self):
        ...
        resolved = resolve_tools_path(agent_config)
        assert resolved is None, \
            f"Expected None for empty list, got {resolved}"
