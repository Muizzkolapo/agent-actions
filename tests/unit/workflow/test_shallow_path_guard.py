...
# ── coordinator._get_workflows_root ─────────────────────────────────────


def _make_coordinator(constructor_path: str):
    """Create an AgentWorkflow with only .config set, bypassing __init__."""
    from agent_actions.workflow.coordinator import AgentWorkflow

    with patch.object(AgentWorkflow, "__init__", lambda self, *a, **kw: None):
        wf = AgentWorkflow.__new__(AgentWorkflow)
        wf.config = _make_workflow_config(constructor_path)
    return wf


class TestGetWorkflowsRootShallowPath:
    """_get_workflows_root must reject paths too shallow for parents[2]."""

    @pytest.mark.parametrize(
        "shallow_path",
        ["agent.yml", "config/agent.yml", ""],
        ids=["one-parent", "two-parents", "empty-string"],
    )
    def test_shallow_path_raises_valueerror(self, shallow_path):
        wf = _make_coordinator(shallow_path)
        with pytest.raises(ValueError, match="too shallow"):
            wf._get_workflows_root()

    def test_boundary_depth_passes(self):
        """a/b/c.yml has exactly 3 parents — parents[2] should succeed."""
        wf = _make_coordinator("a/b/c.yml")
        result = wf._get_workflows_root()
        assert result == Path("a/b/c.yml").parents[2]

    def test_deep_path_returns_workflows_root(self, tmp_path):
        """A sufficiently deep path should return the workflows root."""
        config_file = tmp_path / "workflows" / "my_wf" / "agent_config" / "my_wf.yml"
        wf = _make_coordinator(str(config_file))
        result = wf._get_workflows_root()
        assert result == tmp_path / "workflows"
