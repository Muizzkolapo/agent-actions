...
# ── Bug 4: coordinator._resolve_upstream_workflows passes agent_configs= ────


class TestCoordinatorUpstreamKwargs:
    """_resolve_upstream_workflows must pass agent_configs= to the orchestrator."""
    ...
    def test_resolve_upstream_passes_agent_configs_kwarg(self):
        ...
        metadata.execution_order = ["action_a"]
        metadata.action_indices = {"action_a": 0}
        ...
        wf.config.paths.default_path = "/tmp/default"
        wf.config.use_tools = False
        ...
        mock_orchestrator.resolve_upstream_workflows.return_value = True
        wf.dependency_orchestrator = mock_orchestrator

        result = wf._resolve_upstream_workflows()

        assert result is True
        mock_orchestrator.resolve_upstream_workflows.assert_called_once()
