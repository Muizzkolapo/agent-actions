...
class TestTheRetiredEnvironmentName:
    ...
    def test_it_is_refused_even_when_the_run_asked_for_its_own_limit(self, monkeypatch):
        ...
        with pytest.raises(ValueError, match="AGAC_RECORD_LIMIT"):
            effective_record_limit({"record_limit": 23, RECORD_LIMIT_KEY: 5})
...
class TestTheEnvironmentLimit:
    ...
    def test_an_active_limit_says_so(self, monkeypatch, caplog):
        monkeypatch.setenv("AGAC_RECORD_LIMIT", "2")

        with caplog.at_level("WARNING"):
            effective_record_limit({"record_limit": 23})

        assert any("AGAC_RECORD_LIMIT" in r.message for r in caplog.records), (
...
class TestTheRunCeiling:
    ...
    def test_it_caps_a_larger_configured_limit(self):
        assert effective_record_limit({"record_limit": 23, RECORD_LIMIT_KEY: 2}) == 2

    def test_it_does_not_raise_a_smaller_configured_limit(self):
        assert effective_record_limit({"record_limit": 3, RECORD_LIMIT_KEY: 50}) == 3

    def test_it_applies_where_the_config_sets_no_limit(self):
        assert effective_record_limit({RECORD_LIMIT_KEY: 2}) == 2
    ...
    def test_it_wins_even_when_the_variable_is_stricter(self, monkeypatch):
        ...
        assert effective_record_limit({"record_limit": 23, RECORD_LIMIT_KEY: 10}) == 10
    ...
    def test_a_value_that_cannot_be_a_limit_is_refused_loudly(self, value):
        with pytest.raises(ValueError, match="record-limit"):
            effective_record_limit({"record_limit": 23, RECORD_LIMIT_KEY: value})
    ...
    def test_a_null_cap_reads_as_no_cap(self, monkeypatch):
        ...
        assert effective_record_limit({"record_limit": 23, RECORD_LIMIT_KEY: None}) == 23

    def test_an_active_limit_names_the_flag_not_the_variable(self, caplog):
        """The message has to name what actually capped the run."""
        with caplog.at_level("WARNING"):
            effective_record_limit({"record_limit": 23, RECORD_LIMIT_KEY: 2})

        said = " ".join(r.message for r in caplog.records)
        assert "--record-limit" in said, said
        assert "AGAC_RECORD_LIMIT" not in said, said
...
class TestTheTwoSourcesTogether:
    ...
    def test_a_variable_that_cannot_cap_fails_under_a_flag_too(self, monkeypatch, value):
        ...
        with pytest.raises(ValueError, match="AGAC_RECORD_LIMIT"):
            effective_record_limit({RECORD_LIMIT_KEY: 2})
    ...
    def test_a_cap_applies_over_a_configured_limit_that_cannot_cap(self, configured):
        ...
        assert effective_record_limit({"record_limit": configured, RECORD_LIMIT_KEY: 5}) == 5
...
class TestSilenceWhenNothingIsTruncated:
    ...
    def test_no_warning_without_any_override(self, monkeypatch, caplog):
        monkeypatch.delenv("AGAC_RECORD_LIMIT", raising=False)
    ...
    def test_no_warning_when_the_flag_does_not_bite(self, caplog):
        with caplog.at_level("WARNING"):
            effective_record_limit({"record_limit": 3, RECORD_LIMIT_KEY: 50})
