...
class TestMultiLevelResolution:
    ...
    def test_step2_workflow_level_found(self, tmp_path):
        """Schema in current workflow's schema dir is found."""
        _setup_project(tmp_path)
        _write_schema(
            tmp_path / "agent_workflow" / "my_wf" / "schema" / "wf_schema.yml",
            "workflow_level",
        )
        result = SchemaLoader.load_schema(
            "wf_schema", project_root=tmp_path, workflow_name="my_wf"
        )
    ...
    def test_no_workflow_name_skips_step2(self, tmp_path):
        """Without workflow_name, step 2 is skipped (still found via step 3)."""
        _setup_project(tmp_path)
        _write_schema(
...
class TestGlobalUniqueness:
    """Tests for global uniqueness enforcement."""

    def test_same_name_in_two_locations_raises(self, tmp_path):
        """Same schema name in project-level and workflow-level raises error."""
        _setup_project(tmp_path)
        _write_schema(tmp_path / "schema" / "dup.yml", "project_copy")
        _write_schema(
            tmp_path / "agent_workflow" / "wf" / "schema" / "dup.yml", "wf_copy"
        )
    ...
    def test_same_name_in_two_workflows_raises(self, tmp_path):
        """Same schema name in two different workflow dirs raises error."""
        _setup_project(tmp_path)
        _write_schema(
            tmp_path / "agent_workflow" / "wf_a" / "schema" / "dup.yml", "wf_a_copy"
        )
        _write_schema(
            tmp_path / "agent_workflow" / "wf_b" / "schema" / "dup.yml", "wf_b_copy"
        )
        with pytest.raises(FileNotFoundError, match="multiple locations"):
            SchemaLoader.load_schema("dup", project_root=tmp_path)

    def test_yml_and_yaml_same_name_raises(self, tmp_path):
        """Same schema name with .yml and .yaml extensions raises uniqueness error."""
        _setup_project(tmp_path)
...
class TestSchemaPathConfig:
    ...
    def test_custom_schema_path_workflow_level(self, tmp_path):
        ...
        result = SchemaLoader.load_schema(
            "my_schema", project_root=tmp_path, workflow_name="wf"
        )
