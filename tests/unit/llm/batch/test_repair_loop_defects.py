...
that retry already uses. Sharing them is the point — but each is a place the
...
class TestGraduationIsPerStrategy:
    ...
    Another evaluation loop tags every record that passed its own rules, and the
    ...
    def test_a_record_another_loop_graduated_is_still_judged_by_expectations(self):
        result = _failing()
        EvaluationLoop(_OtherStrategy()).tag_graduated([result])
...
class _OtherStrategy:
    """A second evaluation strategy, so a record can be graduated by one loop.

    Its name is what matters: graduation is keyed by strategy, because one
    loop's pass says nothing about another's rules.
    """
...
class TestRepairDoesNotDiscardTheRestOfRecoveryState:
    """A repair round writes state; the retry bookkeeping in it must survive.
...
class TestRecordsTheProviderDropsAreNotLost:
    ...
    Another recovery round stamps and graduates dropped records. Repair submits in one
