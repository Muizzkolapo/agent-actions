...
class TestApiKeyFormatChecks:
    """Tests for _check_api_key_formats() via resolve_all()."""

    # ── Valid keys produce no warnings ────────────────────────────────

    def test_valid_openai_key_no_warning(self, monkeypatch):
        monkeypatch.setenv("OPENAI_API_KEY", "sk-proj-abc123def456ghi789jkl012mno")
        monkeypatch.delenv("AA_SKIP_ENV_VALIDATION", raising=False)
    ...
    def test_valid_anthropic_sk_ant_key_no_warning(self, monkeypatch):
        monkeypatch.setenv(
            "ANTHROPIC_API_KEY",
            "sk-ant-api03-abcdefghij1234567890xyzxyz",
        )
        monkeypatch.delenv("AA_SKIP_ENV_VALIDATION", raising=False)

        svc = WorkflowResolutionService(
            action_configs={"writer": {"model_vendor": "anthropic"}},
        )
        result = svc.resolve_all()
        assert len(result.warnings) == 0

    def test_valid_groq_key_no_warning(self, monkeypatch):
        monkeypatch.setenv("GROQ_API_KEY", "gsk_abc123def456ghi789jkl012mno")
        monkeypatch.delenv("AA_SKIP_ENV_VALIDATION", raising=False)

        svc = WorkflowResolutionService(
            action_configs={"extractor": {"model_vendor": "groq"}},
        )
        result = svc.resolve_all()
        assert len(result.warnings) == 0

    def test_valid_gemini_key_no_warning(self, monkeypatch):
        monkeypatch.setenv("GEMINI_API_KEY", "AIza" + "a" * 35)
        monkeypatch.delenv("AA_SKIP_ENV_VALIDATION", raising=False)

        svc = WorkflowResolutionService(
            action_configs={"classifier": {"model_vendor": "gemini"}},
        )
        result = svc.resolve_all()
        assert len(result.warnings) == 0

    # ── Invalid keys produce warnings (not errors) ────────────────────

    def test_malformed_openai_key_warns(self, monkeypatch):
        ...
        assert result.is_valid, "Format mismatch must be a warning, not a blocking error"
        assert len(result.warnings) == 1
        assert "openai" in result.warnings[0].message
        assert "summarizer" in result.warnings[0].message

    def test_malformed_groq_key_warns(self, monkeypatch):
        monkeypatch.setenv("GROQ_API_KEY", "sk-invalid-expired-key-12345")
        monkeypatch.delenv("AA_SKIP_ENV_VALIDATION", raising=False)
        ...
        assert len(result.warnings) == 1
        assert "groq" in result.warnings[0].message

    def test_malformed_anthropic_key_warns(self, monkeypatch):
        monkeypatch.setenv("ANTHROPIC_API_KEY", "bad-key-here")
        monkeypatch.delenv("AA_SKIP_ENV_VALIDATION", raising=False)
        ...
        assert result.is_valid
        assert len(result.warnings) == 1
        assert "anthropic" in result.warnings[0].message

    def test_warning_hint_includes_expected_pattern(self, monkeypatch):
        ...
        assert result.warnings[0].hint is not None
        assert "gsk_" in result.warnings[0].hint

    # ── Literal keys are format-checked ───────────────────────────────

    def test_literal_key_format_checked(self, monkeypatch):
        """Literal api_key in config is still format-checked."""
        monkeypatch.delenv("AA_SKIP_ENV_VALIDATION", raising=False)
        ...
        assert result.is_valid
        assert len(result.warnings) == 1
        assert "(literal)" in result.warnings[0].location.raw_reference

    def test_valid_literal_key_no_warning(self, monkeypatch):
        monkeypatch.delenv("AA_SKIP_ENV_VALIDATION", raising=False)
    ...
    def test_same_vendor_key_deduplicated(self, monkeypatch):
        """Multiple actions with the same vendor+key only produce one warning."""
        monkeypatch.setenv("OPENAI_API_KEY", "bad-key")
        monkeypatch.delenv("AA_SKIP_ENV_VALIDATION", raising=False)

        svc = WorkflowResolutionService(
            action_configs={
                "action_a": {"model_vendor": "openai"},
                "action_b": {"model_vendor": "openai"},
                "action_c": {"model_vendor": "openai"},
            },
        )
        result = svc.resolve_all()
        assert len(result.warnings) == 1

    # ── Vendors without patterns are skipped ──────────────────────────

    def test_vendor_without_pattern_no_warning(self, monkeypatch):
        """Vendors without a known pattern (e.g. cohere) produce no warning."""
        monkeypatch.setenv("COHERE_API_KEY", "anything-goes-here")
        monkeypatch.delenv("AA_SKIP_ENV_VALIDATION", raising=False)

        svc = WorkflowResolutionService(
            action_configs={"ranker": {"model_vendor": "cohere"}},
        )
        result = svc.resolve_all()
        assert len(result.warnings) == 0

    # ── AA_SKIP_ENV_VALIDATION bypasses format checks ─────────────────

    def test_skip_env_validation_skips_format_checks(self, monkeypatch):
        monkeypatch.setenv("AA_SKIP_ENV_VALIDATION", "1")
        monkeypatch.setenv("OPENAI_API_KEY", "bad-key")
    ...
    # ── Missing key skips format check (handled by _check_api_keys) ───

    def test_missing_key_no_format_warning(self, monkeypatch):
        """Missing key should produce an error, not a format warning."""
        monkeypatch.delenv("OPENAI_API_KEY", raising=False)
        monkeypatch.delenv("AA_SKIP_ENV_VALIDATION", raising=False)
        ...
        assert len(result.errors) == 1  # missing key error
        assert len(result.warnings) == 0  # no format warning on missing key
