...
class TestAgentWorkflowSequentialLoops:
    ...
    def workflow_with_sequential_loop(self, temp_dir):
        ...
        config_data = {
            ...
            "execution_plan": ["input", "refine_1", "refine_2", "refine_3", "output"],
            "actions": [
                {"name": "input", "tool": "mock_tool"},
                {"name": "refine", "tool": "mock_tool", "loop": {"param": "stage", "range": [1, 3], "mode": "sequential"}},
                {"name": "output", "tool": "mock_tool"}
            ]
        }
        ...
        with patch('agent_actions.core.graph.agent_workflow.ConfigManager') as MockConfigManager, \
            ...
            # Setup mock ConfigManager with sequential dependencies
    ...
    def workflow_with_parallel_loop(self, temp_dir):
        ...
        config_data = {
            "name": "parallel_test",
            "execution_plan": ["input", "process_1", "process_2", "process_3", "output"],
            "actions": [
                {"name": "input", "tool": "mock_tool"},
                {"name": "process", "tool": "mock_tool", "loop": {"param": "idx", "range": [1, 3], "mode": "parallel"}},
                {"name": "output", "tool": "mock_tool"}
            ]
        }
        ...
        with patch('agent_actions.core.graph.agent_workflow.ConfigManager') as MockConfigManager, \
            ...
            # Setup mock ConfigManager with parallel dependencies
            mock_config_manager = MagicMock()
            mock_config_manager.agent_configs = {
                "input": {
                    "agent_type": "input",
                ...
                "process_1": {
                    "agent_type": "process_1",
                    "dependencies": ["input"],  # All depend on input
                    "is_loop_agent": True,
                    "loop_base_name": "process",
                    "loop_mode": "parallel"
                },
                "process_2": {
                    "agent_type": "process_2",
                    "dependencies": ["input"],  # Parallel - all depend on input
                    "is_loop_agent": True,
                    "loop_base_name": "process",
                    "loop_mode": "parallel"
                },
                "process_3": {
                    "agent_type": "process_3",
                    "dependencies": ["input"],  # Parallel - all depend on input
                    "is_loop_agent": True,
                    "loop_base_name": "process",
                    "loop_mode": "parallel"
    ...
    def test_parallel_loop_has_independent_dependencies(self, workflow_with_parallel_loop):
        ...
        # Verify all iterations depend on input (no chaining)
    ...
    def test_sequential_execution_order_enforced(self, workflow_with_sequential_loop):
        ...
        # Verify refine_1 comes before refine_2, refine_2 before refine_3
    ...
    def test_iteration_can_access_previous_output_directory(self, workflow_with_sequential_loop):
        ...
        # Create output directories for previous iterations
        ...
        # Test that _get_input_directory returns correct previous directory
        # Index 2 is refine_2, which should get input from refine_1 (index 1)
        input_dir = workflow_with_sequential_loop._get_input_directory(2)
        assert "node_1_refine_1" in input_dir

        # Index 3 is refine_3, which should get input from refine_2 (index 2)
    ...
    def test_error_in_iteration_prevents_subsequent_execution(self, workflow_with_sequential_loop):
        ...
        # This tests the concept - actual execution blocking is handled by AgentWorkflow's
        # dependency system which won't execute agents whose dependencies failed

        configs = workflow_with_sequential_loop.agent_configs

        # If refine_1 fails, refine_2 depends on it and won't run
        # If refine_2 fails, refine_3 depends on it and won't run
        ...
        # The actual blocking behavior is tested in AgentWorkflow's core tests
        # Here we verify the structure that enables it
    ...
    def test_sequential_loop_metadata_preserved(self, workflow_with_sequential_loop):
        ...
        for agent_name in ["refine_1", "refine_2", "refine_3"]:
            ...
            # Verify all loop metadata fields exist
    ...
    def test_first_iteration_inherits_original_dependencies(self, workflow_with_sequential_loop):
        ...
        # First iteration (refine_1) should depend on input, not on a previous iteration
        assert configs["refine_1"]["dependencies"] == ["input"]
        assert "refine_0" not in configs  # No iteration 0
    ...
    def test_sequential_loop_with_single_iteration(self, temp_dir):
        ...
        config_file = temp_dir / "config.yml"
        config_data = {
            "name": "single_iteration_test",
            "execution_plan": ["input", "process_1", "output"],
            "actions": [
                {"name": "input", "tool": "mock_tool"},
                {"name": "process", "tool": "mock_tool", "loop": {"param": "i", "range": [1, 1], "mode": "sequential"}},
                {"name": "output", "tool": "mock_tool"}
            ]
        }
        ...
        with patch('agent_actions.core.graph.agent_workflow.ConfigManager') as MockConfigManager, \
            ...
            mock_config_manager = MagicMock()
            mock_config_manager.agent_configs = {
                "input": {"agent_type": "input", "dependencies": []},
                "process_1": {
                    "agent_type": "process_1",
                    "dependencies": ["input"],
                    "is_loop_agent": True,
                ...
                "output": {"agent_type": "output", "dependencies": ["process_1"]}
            }
            mock_config_manager.agent_config_map = MagicMock(agent_configs=mock_config_manager.agent_configs)
            ...
            # Single iteration should have original dependencies
