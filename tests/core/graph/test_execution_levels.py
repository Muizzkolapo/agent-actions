...
class TestExecutionLevels:
    ...
    def test_simple_parallel_fork(self):
        ...
        agent_configs = {'A': {'dependencies': []}, 'B': {'dependencies': ['A']}, 'C': {'dependencies': ['A']}, 'D': {'dependencies': ['A']}}
        workflow = self.create_mock_workflow(execution_order, agent_configs)
        levels = workflow._compute_execution_levels()
        assert len(levels) == 2
        assert levels[0] == ['A']
    ...
    def test_parallel_join(self):
        ...
        execution_order = ['A', 'B', 'C', 'D']
        agent_configs = {'A': {'dependencies': []}, 'B': {'dependencies': []}, 'C': {'dependencies': []}, 'D': {'dependencies': ['A', 'B', 'C']}}
        workflow = self.create_mock_workflow(execution_order, agent_configs)
        levels = workflow._compute_execution_levels()
        assert len(levels) == 2
    ...
    def test_diamond_pattern(self):
        ...
        execution_order = ['A', 'B', 'C', 'D']
        agent_configs = {'A': {'dependencies': []}, 'B': {'dependencies': ['A']}, 'C': {'dependencies': ['A']}, 'D': {'dependencies': ['B', 'C']}}
        workflow = self.create_mock_workflow(execution_order, agent_configs)
        levels = workflow._compute_execution_levels()
        assert len(levels) == 3
        assert levels[0] == ['A']
    ...
    def test_edge_case_intermediate_dependencies(self):
        ...
        agent_configs = {'A': {'dependencies': []}, 'B': {'dependencies': ['A']}, 'C': {'dependencies': ['A']}, 'D': {'dependencies': ['A']}, 'E': {'dependencies': ['B', 'C', 'D']}, 'F': {'dependencies': ['E']}, 'G': {'dependencies': ['E']}, 'H': {'dependencies': ['E']}}
        workflow = self.create_mock_workflow(execution_order, agent_configs)
        levels = workflow._compute_execution_levels()
        assert len(levels) == 4
        assert levels[0] == ['A']
    ...
    def test_complex_multi_level(self):
        ...
        agent_configs = {'A': {'dependencies': []}, 'B': {'dependencies': ['A']}, 'C': {'dependencies': ['A']}, 'D': {'dependencies': ['B', 'C']}, 'E': {'dependencies': ['B', 'C']}, 'F': {'dependencies': ['D', 'E']}}
        workflow = self.create_mock_workflow(execution_order, agent_configs)
        levels = workflow._compute_execution_levels()
        assert len(levels) == 4
        assert levels[0] == ['A']
    ...
    def test_no_dependencies_all_parallel(self):
        ...
        execution_order = ['A', 'B', 'C']
        agent_configs = {'A': {'dependencies': []}, 'B': {'dependencies': []}, 'C': {'dependencies': []}}
        workflow = self.create_mock_workflow(execution_order, agent_configs)
        levels = workflow._compute_execution_levels()
        assert len(levels) == 1
        assert set(levels[0]) == {'A', 'B', 'C'}
    ...
    def test_partial_dependencies(self):
        ...
        execution_order = ['A', 'B', 'C', 'D']
        agent_configs = {'A': {'dependencies': []}, 'B': {'dependencies': ['A']}, 'C': {'dependencies': []}, 'D': {'dependencies': ['C']}}
        workflow = self.create_mock_workflow(execution_order, agent_configs)
        levels = workflow._compute_execution_levels()
        assert len(levels) == 2
    ...
    def test_circular_dependency_detection(self):
        ...
        execution_order = ['A', 'B', 'C']
        agent_configs = {'A': {'dependencies': ['C']}, 'B': {'dependencies': ['A']}, 'C': {'dependencies': ['B']}}
        workflow = self.create_mock_workflow(execution_order, agent_configs)
