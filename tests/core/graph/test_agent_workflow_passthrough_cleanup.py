...
class TestAgentWorkflowPassthroughCleanup:
    ...
    def temp_workspace(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            yield {'workspace': workspace, 'agent_io': agent_io, 'target': target, 'node_output': node_output}
    ...
    def test_passthrough_marker_cleanup_success(self, workflow, temp_workspace):
        ...
        if passthrough_marker.exists():
            ...
            workflow.ephemeral_directories.append({'output_folder': str(node_output_dir), 'ephemeral': agent_config.get('ephemeral', False)})
            try:
    ...
    def test_passthrough_data_integrity_after_cleanup(self, workflow, temp_workspace):
        ...
        test_data = [{'source_guid': 'test-guid-1', 'content': {'test': 'passthrough content'}, 'metadata': {'skipped_by_where_clause': True, 'agent_type': 'passthrough'}}]
        with open(data_file, 'w') as f:
