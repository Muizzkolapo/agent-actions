...
from agent_actions.state_management.error_context import with_error_context, with_command_context, with_agent_context, with_file_context
...
class TestWithErrorContext:
    ...
    def test_context_with_complex_args(self):
        ...
        with pytest.raises(RuntimeError) as exc_info:
            complex_function({'key': 'value'}, config={'setting': True}, flag=True)
        exc = exc_info.value
        context = exc.context
