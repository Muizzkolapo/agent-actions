...
class TestBaseLoader:
    ...
    async def test_load_file_async_with_aiofiles(self, tmp_path):
        ...
        # Mock aiofiles if it's available
        try:
            import aiofiles
            content = await loader.load_file_async(str(test_file))
            assert content == test_content
    ...
    def test_supports_filetype_abstract_method(self):
        ...
        # _TestableLoader should implement this
...
class TestBatchDataLoader:
    ...
    def test_load_invalid_jsonl_error(self, tmp_path):
        ...
        # Create test file with invalid JSONL
...
class TestLoaderBatchProcessing:
    ...
    def test_batch_processing_remainder_handling(self, tmp_path):
        ...
        # Create data that doesn't divide evenly into batches
        # Simulate a scenario where batch size might be 100 but we have 157 items
        ...
        # Should load all items including the remainder
    ...
    def test_error_propagation_in_batch_processing(self, tmp_path):
        ...
        # Create file that will cause JSON decode error
        test_file = tmp_path / "error.json"
        test_file.write_text("{ incomplete json", encoding="utf-8")

        # Error should propagate up, not be silently ignored
        with pytest.raises(ValueError, match="Error decoding JSON"):
            loader.load_data(str(test_file))
...
class TestDataLoadersModuleExports:
    ...
    def test_loader_interface_compliance(self):
        ...
        from agent_actions.agents.base.base_loader import BaseLoader
        from agent_actions.integrations.loaders.batch_data_loader import BatchDataLoader
        from agent_actions.core.contracts.interfaces import IDataLoader

        # BatchDataLoader should implement IDataLoader
        assert issubclass(BatchDataLoader, IDataLoader)

        # BaseLoader should implement IDataLoader
...
class TestLoaderErrorHandling:
    ...
    def test_retry_mechanism_in_base_loader(self, tmp_path):
        ...
        # Create a file that will succeed after retries
        ...
        with patch('builtins.open', side_effect=mock_open):
            try:
                ...
                # Some error handling configurations might not retry IOErrors
                pass
    ...
    def test_file_error_handling_mixin(self):
        ...
        # Should have error handling methods from mixin
    ...
    def test_concurrent_batch_loading(self, tmp_path):
        ...
        # Load all files (simulating concurrent batch processing)
        results = []
