...
from agent_actions.shared.exceptions import ValidationError, ConfigValidationError
...
class TestGuardConfig:
    ...
    def test_guard_config_with_skip_behavior(self):
        ...
        config = GuardConfig(condition='udf:validators.should_extract_facts', on_false=GuardBehavior.SKIP)
...
class TestFormatConverterIntegration:
    ...
    def test_convert_skip_behavior_to_conditional_clause(self):
        ...
        action = {'name': 'test_action', 'intent': 'Test action with skip guard', 'guard': {'condition': 'udf:validators.should_process', 'on_false': 'skip'}, 'model_vendor': 'openai', 'model_name': 'gpt-4o-mini', 'api_key': 'TEST_API_KEY'}
        defaults = {}
...
class TestSchemaValidation:
    ...
    def test_action_config_validates_consolidated_guard(self):
        ...
        action_data = {'name': 'test_action', 'intent': 'Test action', 'guard': {'condition': 'udf:validators.check_quality', 'on_false': 'skip'}}
    ...
    def test_action_config_validates_legacy_guard_string(self):
        ...
        from agent_actions.configuration.new_format_schema import ActionConfig
        action_data = {'name': 'test_action', 'intent': 'Test action', 'guard': 'udf:validators.check_quality'}
        action = ActionConfig(**action_data)
    ...
    def test_action_config_rejects_invalid_guard(self):
        ...
        from agent_actions.configuration.new_format_schema import ActionConfig
        with pytest.raises(ValidationError, match='Invalid UDF expression format'):
            ActionConfig(name='test_action', intent='Test action', guard={'condition': 'udf:invalid_format', 'on_false': 'skip'})
