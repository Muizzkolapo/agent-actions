...
class TestProcessorUtilsThreadSafety:
    ...
    def test_concurrent_position_based_correlation_id_consistency(self):
        ...
        def worker():
            ...
            correlation_id = ProcessorUtils.get_or_create_position_based_loop_correlation_id(record_index, loop_base_name, self.get_test_session_id(), file_context)
            with correlation_ids_lock:
    ...
    def test_stress_test_many_concurrent_operations(self):
        ...
        def worker(worker_id: int):
            ...
            for i in range(operations_per_worker):
                if i % 3 == 0:
                    ProcessorUtils.get_or_create_loop_correlation_id(f'guid-{worker_id}-{i}', f'loop-{worker_id % 5}', self.get_test_session_id())
                elif i % 3 == 1:
                    ProcessorUtils.get_or_create_position_based_loop_correlation_id(i, f'pos-loop-{worker_id % 3}', self.get_test_session_id(), f'file-{worker_id % 2}')
                else:
                    ProcessorUtils.get_or_create_loop_correlation_id('shared-guid', 'shared-loop', self.get_test_session_id())
        threads = []
    ...
    def test_add_loop_correlation_id_thread_safety(self):
        ...
        agent_config = {'is_loop_agent': True, 'loop_base_name': 'concurrent_loop', 'workflow_session_id': self.get_test_session_id()}
        results: List[str] = []
        results_lock = threading.Lock()
