...
from agent_actions.core.utils.processor_helpers import (
    run_dynamic_agent,
    _should_skip_legacy_conditional,
    _should_skip_where_clause,
)
...
class TestShouldSkipWhereClause:
    ...
    def test_skip_when_filter_returns_false(self, mock_get_filter):
        ...
        agent_config = {
            'where_clause': {
                'clause': 'should_keep_cluster == false',
                'behavior': 'skip'
            }
        }
    ...
    def test_error_handling_with_passthrough_on_error_true(self, mock_get_filter):
        ...
        assert result is True  # Should skip on error when passthrough_on_error is True
    ...
    def test_error_handling_with_passthrough_on_error_false(self, mock_get_filter):
        ...
        assert result is False  # Should not skip on error when passthrough_on_error is False
    ...
    def test_error_handling_default_passthrough_on_error(self, mock_get_filter):
        ...
        agent_config = {
            'where_clause': {
                'clause': 'invalid_clause',
                'behavior': 'skip'
                # passthrough_on_error not specified (should default to True)
            }
        }
        context = {'data': 'test'}
        ...
        assert result is True  # Should skip on error with default passthrough_on_error=True
...
class TestRunDynamicAgent:
    ...
    def test_executes_agent_when_no_guards(self, mock_apply_remove, mock_agent_builder):
        ...
        result, executed = run_dynamic_agent(
            agent_config, 'test_agent', context, 'test prompt'
        )
    ...
    def test_skips_agent_when_legacy_conditional_returns_true(self, mock_should_skip):
        ...
        assert result == context  # Original context unchanged
    ...
    def test_executes_agent_when_all_guards_pass(
        self, mock_legacy_skip, mock_where_skip, mock_apply_remove, mock_agent_builder
    ):
        ...
        agent_config = {
            'conditional_clause': 'validators.should_process',
            'where_clause': {
                'clause': 'should_keep_cluster == false',
                'behavior': 'skip'
            }
    ...
    def test_passes_all_parameters_to_agent_builder(self, mock_apply_remove, mock_agent_builder):
        ...
        result, executed = run_dynamic_agent(
            agent_config, 'test_agent', context, formatted_prompt,
            tools_path=tools_path, tool_args=tool_args, source_content=source_content
        )

        assert executed is True
        mock_agent_builder.create_dynamic_agent.assert_called_once_with(
            agent_config, 'test_agent', {'processed': 'context'}, formatted_prompt,
            tools_path=tools_path, tool_args=tool_args, source_content=source_content
        )
...
class TestToolAgentGuardIntegration:
    ...
    def test_tool_agent_with_skip_guard_condition_false(
        ...
        # Setup mocks
        mock_filter_service = MagicMock()
        mock_filter_service.filter_item.return_value = True  # Condition matches
        mock_get_filter.return_value = mock_filter_service
        ...
        context = {'should_keep_cluster': False}  # Should match condition and execute

        result, executed = run_dynamic_agent(
            agent_config, 'create_new_clusters', context, 'test prompt'
        )

        # Verify execution
        assert executed is True
        assert result == [{'new_cluster': 'result'}]
        mock_filter_service.filter_item.assert_called_once_with(
            context, 'should_keep_cluster == false'
        )
        mock_agent_builder.create_dynamic_agent.assert_called_once()
    ...
    def test_tool_agent_with_skip_guard_condition_true(self, mock_get_filter):
        ...
        # Setup mocks
        mock_filter_service = MagicMock()
        mock_filter_service.filter_item.return_value = False  # Condition doesn't match
        mock_get_filter.return_value = mock_filter_service
        ...
        context = {'should_keep_cluster': True}  # Should NOT match condition and skip
    ...
    def test_tool_agent_guard_error_handling(self, mock_get_filter):
        ...
        # Should skip on error with passthrough_on_error=True
        assert executed is False
