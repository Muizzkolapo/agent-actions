...
import os
from agent_actions.core.project_root import (
    find_project_root,
    ensure_in_project,
...
class TestFindProjectRoot:
    ...
    def test_find_root_nested_projects(self, tmp_path):
        ...
        # Should find inner project (nearest ancestor)
        subdir = inner / "src"
        subdir.mkdir()
...
class TestGetProjectRootOrCwd:
    ...
    def test_returns_cwd_when_not_in_project(self, tmp_path, monkeypatch):
        ...
        # Compare resolved paths to handle symlinks in temp directories
...
class TestIsInProject:
    ...
    def test_false_from_parent_directory(self, tmp_path, monkeypatch):
        ...
        # Change to parent directory (tmp_path)
        monkeypatch.chdir(tmp_path)
        assert is_in_project() is False
...
class TestEdgeCases:
    ...
    def test_marker_file_is_directory_not_file(self, tmp_path):
        ...
        root = tmp_path / "project"
        root.mkdir()
        # Create a directory with the marker file name (should be ignored)
