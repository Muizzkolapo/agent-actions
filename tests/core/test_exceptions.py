...
class TestAgentActionsException:
    ...
    def test_exception_with_nested_dict_context(self):
        ...
        context = {'agent': 'test_agent', 'config': {'file': 'test.yml', 'section': 'agents', 'settings': {'timeout': 30, 'retries': 3}}, 'step': 'validation'}
        exc = AgentActionsException('Test error message', context)
        result = str(exc)
        assert 'Test error message' in result
        assert 'agent=test_agent' in result
    ...
    def test_exception_with_numeric_context(self):
        ...
        context = {'line_number': 42, 'error_count': 3, 'timeout': 30.5, 'success_rate': 0.95}
    ...
    def test_exception_context_with_special_characters(self):
        ...
        context = {'file_path': '/path/with spaces/config.yml', 'message': 'Error: invalid \'syntax\' in "config"', 'pattern': '*.yml', 'unicode': '测试'}
