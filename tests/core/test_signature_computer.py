...
class TestSignatureComputerOutputSignature:
    ...
    def test_compute_output_signature_with_observe_drops(self):
        ...
        # = (analysis, temp_data) + (document_id, source_url) - (temp_data, source_url)
    ...
    def test_compute_output_signature_no_properties(self):
        ...
        agent_config = {
            "output_schema": {"type": "object"},  # No properties field
            "observe": ["document_id"],
            "drops": []
        }
    ...
    def test_compute_output_signature_with_schema_registry(self):
        ...
        agent_config = {
            "output_schema": "ExtractorSchema",  # String reference
            "observe": ["metadata"],
            "drops": []
        }
    ...
    def test_compute_output_signature_missing_schema_registry(self):
        ...
        agent_config = {
            "output_schema": "MissingSchema",
            "observe": ["field1"],
            "drops": []
        }
        
        # No schema registry provided
        signature = SignatureComputer.compute_output_signature(agent_config)
        
        # Should treat as empty schema when registry is missing
        assert signature.schema_fields == []
    ...
    def test_compute_output_signature_missing_schema_in_registry(self):
        ...
        schema_registry = {
            "ExistingSchema": {"properties": {"field": {}}}
        }
        
        signature = SignatureComputer.compute_output_signature(agent_config, schema_registry)
        
        # Should treat as empty schema when reference not found
        assert signature.schema_fields == []
        assert signature.observe_fields == ["field1"]
        assert signature.get_available_fields() == {"field1"}
...
class TestSignatureComputerInputSignature:
    ...
    def test_compute_input_signature_from_prompt(self):
        ...
        agent_config = {
            "prompt": "Analyze: {extractor.summary} and {classifier.category}. Document: {source.title}",
            "dependencies": ["extractor", "classifier"]
        }
        
        dependency_configs = {
            "extractor": {
                "output_schema": {"properties": {"summary": {}, "entities": {}}},
                "observe": [],
                "drops": []
            },
            "classifier": {
                "output_schema": {"properties": {"category": {}, "confidence": {}}},
                "observe": [],
                "drops": []
            }
        }
        ...
        # Should extract dependencies from prompt
        expected_deps = {
            "extractor": ["summary"],
            "classifier": ["category"]
        }
        ...
        # Should extract source fields
        assert "title" in signature.source_fields
        
        # All fields should include deps + source
    ...
    def test_compute_input_signature_no_prompt(self):
        ...
        # Should have empty dependencies since no prompt to parse
    ...
    def test_compute_input_signature_loop_references(self):
        ...
        agent_config = {
            "prompt": "Processing item {loop.index} of {loop.total} with data {loop.item.text}",
            "dependencies": []
        }
        
        signature = SignatureComputer.compute_input_signature(agent_config, {})
        
        # Should extract loop fields (full paths as parsed)
    ...
    def test_compute_input_signature_workflow_references(self):
        ...
        agent_config = {
            "prompt": "Workflow {workflow.name} version {workflow.version} run {workflow.run_id}",
            "dependencies": []
        }
        
        signature = SignatureComputer.compute_input_signature(agent_config, {})
        
        # Should extract workflow fields
    ...
    def test_compute_input_signature_nested_field_paths(self):
        ...
        agent_config = {
            "prompt": "Data: {extractor.data.metrics.count} and {extractor.results.items.0.value}",
            "dependencies": ["extractor"]
        }
        
        dependency_configs = {
            "extractor": {
                "output_schema": {"properties": {"data": {}, "results": {}}},
                "observe": [],
                "drops": []
            }
        }
    ...
    def test_dependency_field_extraction(self):
        ...
        agent_config = {
            "prompt": "Fields: {agent1.field1} {agent1.field2} {agent2.field3}",
            "dependencies": ["agent1", "agent2"]
        }
        
        dependency_configs = {
            "agent1": {
                "output_schema": {"properties": {"field1": {}, "field2": {}, "other": {}}},
                "observe": [],
                "drops": []
            },
            "agent2": {
                "output_schema": {"properties": {"field3": {}, "field4": {}}},
                "observe": [],
                "drops": []
            }
        }
...
class TestSignatureComputerFieldAvailability:
    ...
    def test_validate_field_availability_valid(self):
        ...
        input_signature = InputSignature(
            dependencies={"extractor": ["summary", "entities"]},
            source_fields=["title"]
        )
        
        dependency_signatures = {
            "extractor": OutputSignature(
        ...
        # extractor provides: summary, entities, document_id (metadata is dropped)
        # input needs: summary, entities
        # Should be valid

    def test_validate_field_availability_missing_fields(self):
        """Test field availability validation with missing fields."""
        input_signature = InputSignature(
            dependencies={"extractor": ["summary", "missing_field"]},
            source_fields=[]
        )
        
        dependency_signatures = {
            "extractor": OutputSignature(
                schema_fields=["summary", "entities"],
                observe_fields=[],
                dropped_fields=[]
            )
        }
    ...
    def test_validate_field_availability_dropped_fields(self):
        ...
        input_signature = InputSignature(
            dependencies={"extractor": ["summary", "metadata"]},
            source_fields=[]
        )
        
        dependency_signatures = {
            "extractor": OutputSignature(
                schema_fields=["summary", "metadata"],
                observe_fields=[],
                dropped_fields=["metadata"]  # metadata is dropped
            )
        }
    ...
    def test_validate_field_availability_missing_dependency(self):
        ...
        input_signature = InputSignature(
            dependencies={"missing_agent": ["field1"]},
            source_fields=[]
        )
        
        dependency_signatures = {}  # No dependency signatures provided
...
class TestSignatureComputerErrorHandling:
    ...
    def test_compute_input_signature_missing_dependencies(self):
        ...
        dependency_configs = {}  # Empty dependency configs
        
        signature = SignatureComputer.compute_input_signature(agent_config, dependency_configs)
        
        # Should handle missing dependency configs gracefully
        # Fields from missing dependencies are not included in signature
        assert signature.dependencies == {}
        assert signature.get_all_fields() == set()
    ...
    def test_compute_signature_with_malformed_schema(self):
        ...
        agent_config = {
            "output_schema": "not_a_dict_or_string",  # Invalid type
            "observe": [],
            "drops": []
        }
