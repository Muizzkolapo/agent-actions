...
class TestUserError:
    ...
    def test_user_error_creation(self):
        ...
        error = UserError(
            ...
            fix="Add 'name' field to your configuration",
            context={"file": "config.yml", "line": 5}
        )
...
class TestFormatUserError:
    ...
    def test_format_handles_broken_exception(self):
        ...
        # Should not raise exception and provide some output
        assert len(result) > 0
    ...
    def test_format_command_specific_guidance(self):
        ...
        # Should contain command-specific context
        assert len(init_result) > 0
        assert len(run_result) > 0
        # They might have different guidance
