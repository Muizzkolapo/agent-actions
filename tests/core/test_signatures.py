...
from agent_actions.state_management.signatures import FieldInfo, FieldSource, InputSignature, OutputSignature
...
class TestFieldInfo:
    ...
    def test_field_info_json_serialization(self):
        ...
        field_info = FieldInfo(name='entities', source=FieldSource.OBSERVE)
...
class TestInputSignature:
    ...
    def test_input_signature_field_aggregation(self):
        ...
        input_sig = InputSignature(dependencies={'extractor': ['summary', 'entities'], 'classifier': ['category']}, source_fields=['page_content', 'title'], loop_fields=['index', 'total'], workflow_fields=['run_id', 'version'])
        all_fields = input_sig.get_all_fields()
        expected_fields = {'summary', 'entities', 'category', 'page_content', 'title', 'index', 'total', 'run_id', 'version'}
    ...
    def test_input_signature_duplicate_fields(self):
        ...
        input_sig = InputSignature(dependencies={'agent1': ['field1', 'field2'], 'agent2': ['field2', 'field3']}, source_fields=['field1', 'field4'], loop_fields=['field3'], workflow_fields=['field5'])
        all_fields = input_sig.get_all_fields()
    ...
    def test_input_signature_serialization(self):
        ...
        input_sig = InputSignature(dependencies={'extractor': ['summary']}, source_fields=['content'], loop_fields=['index'], workflow_fields=['run_id'])
        sig_dict = input_sig.model_dump()
        expected = {'dependencies': {'extractor': ['summary']}, 'source_fields': ['content'], 'loop_fields': ['index'], 'workflow_fields': ['run_id']}
...
class TestOutputSignature:
    ...
    def test_output_signature_available_fields(self):
        ...
        output_sig = OutputSignature(schema_fields=['summary', 'entities', 'metadata'], observe_fields=['document_id', 'source_url'], dropped_fields=['metadata', 'source_url'])
    ...
    def test_output_signature_no_drops(self):
        ...
        output_sig = OutputSignature(schema_fields=['analysis', 'score'], observe_fields=['input_id'], dropped_fields=[])
        available_fields = output_sig.get_available_fields()
    ...
    def test_output_signature_empty_schema(self):
        ...
        output_sig = OutputSignature(schema_fields=[], observe_fields=['field1', 'field2'], dropped_fields=['field1'])
        available_fields = output_sig.get_available_fields()
    ...
    def test_output_signature_all_fields_dropped(self):
        ...
        output_sig = OutputSignature(schema_fields=['field1', 'field2'], observe_fields=['field3'], dropped_fields=['field1', 'field2', 'field3'])
        available_fields = output_sig.get_available_fields()
        assert available_fields == set()

    def test_output_signature_serialization(self):
        """Test OutputSignature Pydantic serialization."""
        output_sig = OutputSignature(schema_fields=['summary'], observe_fields=['document_id'], dropped_fields=['metadata'])
        sig_dict = output_sig.model_dump()
        expected = {'schema_fields': ['summary'], 'observe_fields': ['document_id'], 'dropped_fields': ['metadata']}
        assert sig_dict == expected
...
class TestSignatureValidationEdgeCases:
    ...
    def test_signature_json_compatibility(self):
        ...
        input_sig = InputSignature(dependencies={'agent1': ['field1']}, source_fields=['field2'])
        output_sig = OutputSignature(schema_fields=['field3'], observe_fields=['field4'], dropped_fields=[])
    ...
    def test_signature_empty_strings_and_lists(self):
        ...
        input_sig = InputSignature(dependencies={}, source_fields=[], loop_fields=[], workflow_fields=[])
        output_sig = OutputSignature(schema_fields=[], observe_fields=[], dropped_fields=[])
        assert input_sig.get_all_fields() == set()
    ...
    def test_signature_large_field_collections(self):
        ...
        input_sig = InputSignature(dependencies=large_deps, source_fields=large_source)
        output_sig = OutputSignature(schema_fields=large_schema, observe_fields=[], dropped_fields=[])
        input_fields = input_sig.get_all_fields()
        output_fields = output_sig.get_available_fields()
        assert len(input_fields) == 150
