...
class TestActionExpanderLoopModes:
    ...
    def test_parallel_loop_dependencies(self):
        ...
        config = {
            ...
            "actions": [
                {
                    "name": "input",
                    "intent": "Load input data",
                    "api_key": "OPENAI_API_KEY"
                },
                {
                    "name": "process",
                    "intent": "Process data in parallel",
                    "api_key": "OPENAI_API_KEY",
                    "loop": {
                        "param": "stage",
                        "range": [1, 3],
        ...
        # All process agents should depend on input (parallel mode)
    ...
    def test_sequential_loop_dependencies(self):
        ...
        config = {
            ...
            "actions": [
                ...
                {
                    "name": "refine",
                    "intent": "Refine data sequentially",
                    "api_key": "OPENAI_API_KEY",
                    "loop": {
                        "param": "stage",
                        "range": [1, 4],
        ...
        assert refine_agents[0]['dependencies'] == ['input']  # First depends on input
        assert refine_agents[1]['dependencies'] == ['refine_1']  # Second depends on first
        assert refine_agents[2]['dependencies'] == ['refine_2']  # Third depends on second
        assert refine_agents[3]['dependencies'] == ['refine_3']  # Fourth depends on third

        # Check loop_mode metadata
    ...
    def test_default_loop_mode_is_parallel(self):
        ...
        # Should behave like parallel mode
        for agent in process_agents:
            assert agent['dependencies'] == ['input']
            assert agent['loop_mode'] == 'parallel'
    ...
    def test_sequential_loop_with_multiple_parent_deps(self):
        ...
        # First iteration gets both parent dependencies
        assert set(merge_agents[0]['dependencies']) == {'input_a', 'input_b'}

        # Later iterations only depend on previous iteration
    ...
    def test_mixed_parallel_and_sequential_loops(self):
        ...
        # Parallel agents should all depend on input
        for agent in parallel_agents:
            assert agent['dependencies'] == ['input']
            assert agent['loop_mode'] == 'parallel'

        # Sequential agents should form a chain
        # First iteration depends on all parallel agents
