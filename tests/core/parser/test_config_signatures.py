...
class TestAgentConfigSignatureMethods:
    ...
    def test_agent_config_output_signature(self):
        ...
        agent_config = AgentConfig(agent_type='extractor', name='test_extractor', output_schema={'properties': {'summary': {'type': 'string'}, 'entities': {'type': 'array'}}}, observe=['document_id', 'source_url'], drops=['temp_data'])
    ...
    def test_agent_config_output_signature_with_schema_registry(self):
        ...
        agent_config = AgentConfig(agent_type='processor', name='test_processor', output_schema='ProcessorSchema', observe=['metadata'], drops=[])
        schema_registry = {'ProcessorSchema': {'properties': {'result': {'type': 'string'}, 'confidence': {'type': 'number'}}}}
    ...
    def test_agent_config_input_signature(self):
        ...
        agent_config = AgentConfig(agent_type='analyzer', name='test_analyzer', prompt='Analyze: {extractor.summary} and {classifier.category}', dependencies=['extractor', 'classifier'])
        dependency_configs = {'extractor': AgentConfig(agent_type='extractor', name='extractor', output_schema={'properties': {'summary': {'type': 'string'}, 'entities': {'type': 'array'}}}), 'classifier': AgentConfig(agent_type='classifier', name='classifier', output_schema={'properties': {'category': {'type': 'string'}, 'confidence': {'type': 'number'}}})}
        signature = agent_config.input_signature(dependency_configs)
        assert isinstance(signature, InputSignature)
        assert signature.dependencies == {'extractor': ['summary'], 'classifier': ['category']}
    ...
    def test_agent_config_input_signature_with_schema_registry(self):
        ...
        agent_config = AgentConfig(agent_type='validator', name='test_validator', prompt='Validate: {processor.result}', dependencies=['processor'])
        dependency_configs = {'processor': AgentConfig(agent_type='processor', name='processor', output_schema='ProcessorSchema')}
        schema_registry = {'ProcessorSchema': {'properties': {'result': {'type': 'string'}, 'score': {'type': 'number'}}}}
    ...
    def test_agent_config_signature_with_mixed_dependency_types(self):
        ...
        agent_config = AgentConfig(agent_type='combiner', name='test_combiner', prompt='Combine: {agent1.field1} and {agent2.field2}', dependencies=['agent1', 'agent2'])
        dependency_configs = {'agent1': AgentConfig(agent_type='agent1', name='agent1', output_schema={'properties': {'field1': {}}}), 'agent2': {'output_schema': {'properties': {'field2': {}}}, 'observe': [], 'drops': []}}
        signature = agent_config.input_signature(dependency_configs)
        assert signature.dependencies == {'agent1': ['field1'], 'agent2': ['field2']}

    def test_agent_config_signature_no_dependencies(self):
        """Test AgentConfig signatures with no dependencies."""
        agent_config = AgentConfig(agent_type='starter', name='test_starter', output_schema={'properties': {'initial_data': {}}}, dependencies=[])
    ...
    def test_agent_config_signature_empty_prompt(self):
        ...
        agent_config = AgentConfig(agent_type='silent', name='test_silent', prompt='', dependencies=['some_agent'])
        dependency_configs = {'some_agent': AgentConfig(agent_type='some_agent', name='some_agent', output_schema={'properties': {'field': {}}})}
        signature = agent_config.input_signature(dependency_configs)
...
class TestSignatureMethodErrorHandling:
    ...
    def test_agent_config_output_signature_empty_schema(self):
        ...
        agent_config = AgentConfig(agent_type='empty', name='test_empty', output_schema={}, observe=['field1'], drops=[])
        signature = agent_config.output_signature()
    ...
    def test_agent_config_input_signature_missing_dependency(self):
        ...
        agent_config = AgentConfig(agent_type='needy', name='test_needy', prompt='Need: {missing.field}', dependencies=['missing'])
        dependency_configs = {}
        signature = agent_config.input_signature(dependency_configs)
        assert signature.get_all_fields() == set()
    ...
    def test_signature_method_schema_registry_errors(self):
        ...
        agent_config = AgentConfig(agent_type='test', name='test', output_schema='NonExistentSchema')
    ...
    def test_signature_method_with_invalid_dependency_config(self):
        ...
        agent_config = AgentConfig(agent_type='test', name='test', prompt='Use: {dep.field}', dependencies=['dep'])
        dependency_configs = {'dep': 'not_a_config_object'}
        signature = agent_config.input_signature(dependency_configs)
        assert isinstance(signature, InputSignature)

    def test_signature_method_error_propagation(self):
        """Test that signature method errors are propagated correctly."""
        agent_config = AgentConfig(agent_type='test', name='test', output_schema={'properties': {'field': {}}}, observe=['valid_field'], drops=[])
        signature = agent_config.output_signature()
        assert isinstance(signature, OutputSignature)
...
class TestSignatureMethodComplexScenarios:
    ...
    def test_deep_dependency_chain(self):
        ...
        agent3_config = AgentConfig(agent_type='agent3', name='agent3', prompt='Final: {agent2.intermediate}', dependencies=['agent2'])
        agent2_config = AgentConfig(agent_type='agent2', name='agent2', output_schema={'properties': {'intermediate': {}}}, prompt='Middle: {agent1.initial}', dependencies=['agent1'])
        agent1_config = AgentConfig(agent_type='agent1', name='agent1', output_schema={'properties': {'initial': {}}})
    ...
    def test_circular_reference_handling(self):
        ...
        agent_a = AgentConfig(agent_type='agent_a', name='agent_a', prompt='Use: {agent_b.field_b}', dependencies=['agent_b'], output_schema={'properties': {'field_a': {}}})
        agent_b = AgentConfig(agent_type='agent_b', name='agent_b', prompt='Use: {agent_a.field_a}', dependencies=['agent_a'], output_schema={'properties': {'field_b': {}}})
    ...
    def test_large_workflow_signature_computation(self):
        ...
        for i in range(num_agents):
            agent_config = AgentConfig(agent_type=f'agent_{i}', name=f'agent_{i}', output_schema={'properties': {f'field_{i}': {}}}, prompt=f'Process: {{agent_{i - 1}.field_{i - 1}}}' if i > 0 else 'Start', dependencies=[f'agent_{i - 1}'] if i > 0 else [])
            agents[f'agent_{i}'] = agent_config
        final_agent = agents[f'agent_{num_agents - 1}']
        signature = final_agent.input_signature(agents)
        assert isinstance(signature, InputSignature)
        assert len(signature.dependencies) == 1