...
class TestTemplateVariableReplacement:
    ...
    def test_current_iteration_template_var(self):
        ...
        config = {'name': 'test_workflow', 'description': 'Test workflow', 'version': '1.0.0', 'defaults': {'model_vendor': 'openai', 'model_name': 'gpt-4o-mini'}, 'actions': [{'name': 'process', 'intent': 'Process with template vars', 'api_key': 'OPENAI_API_KEY', 'prompt': 'Processing stage ${stage}', 'loop': {'param': 'stage', 'range': [1, 3]}}], 'plan': ['process']}
    ...
    def test_previous_iteration_template_var(self):
        ...
        config = {'name': 'test_workflow', 'description': 'Test workflow', 'version': '1.0.0', 'defaults': {'model_vendor': 'openai', 'model_name': 'gpt-4o-mini'}, 'actions': [{'name': 'refine', 'intent': 'Refine with previous reference', 'api_key': 'OPENAI_API_KEY', 'prompt': 'Refine output from stage ${stage-1}', 'loop': {'param': 'stage', 'range': [1, 4], 'mode': 'sequential'}}], 'plan': ['refine']}
        result = ActionExpander.expand_actions_to_agents(config)
        agents = result['test_workflow']
        assert len(agents) == 4
    ...
    def test_both_template_vars_in_same_string(self):
        ...
        config = {'name': 'test_workflow', 'description': 'Test workflow', 'version': '1.0.0', 'defaults': {'model_vendor': 'openai', 'model_name': 'gpt-4o-mini'}, 'actions': [{'name': 'compare', 'intent': 'Compare iterations', 'api_key': 'OPENAI_API_KEY', 'prompt': 'Compare stage ${i} with stage ${i-1}', 'loop': {'param': 'i', 'range': [1, 3]}}], 'plan': ['compare']}
        result = ActionExpander.expand_actions_to_agents(config)
        agents = result['test_workflow']
    ...
    def test_template_vars_with_non_numeric_range(self):
        ...
        config = {'name': 'test_workflow', 'description': 'Test workflow', 'version': '1.0.0', 'defaults': {'model_vendor': 'openai', 'model_name': 'gpt-4o-mini'}, 'actions': [{'name': 'process', 'intent': 'Process with list range', 'api_key': 'OPENAI_API_KEY', 'prompt': 'Stage ${idx} (prev: ${idx-1})', 'loop': {'param': 'idx', 'range': [10, 20, 30]}}], 'plan': ['process']}
        result = ActionExpander.expand_actions_to_agents(config)
        agents = result['test_workflow']
        assert len(agents) == 3
    ...
    def test_template_var_without_previous_ref(self):
        ...
        config = {'name': 'test_workflow', 'description': 'Test workflow', 'version': '1.0.0', 'defaults': {'model_vendor': 'openai', 'model_name': 'gpt-4o-mini'}, 'actions': [{'name': 'simple', 'intent': 'Simple template var', 'api_key': 'OPENAI_API_KEY', 'prompt': 'Iteration ${iter} only', 'loop': {'param': 'iter', 'range': [1, 2]}}], 'plan': ['simple']}
        result = ActionExpander.expand_actions_to_agents(config)
        agents = result['test_workflow']
    ...
    def test_multiple_previous_refs_in_string(self):
        ...
        config = {'name': 'test_workflow', 'description': 'Test workflow', 'version': '1.0.0', 'defaults': {'model_vendor': 'openai', 'model_name': 'gpt-4o-mini'}, 'actions': [{'name': 'multi', 'intent': 'Multiple previous refs', 'api_key': 'OPENAI_API_KEY', 'prompt': 'Compare ${x-1} with ${x-1} again', 'loop': {'param': 'x', 'range': [1, 2]}}], 'plan': ['multi']}
        result = ActionExpander.expand_actions_to_agents(config)
        agents = result['test_workflow']
