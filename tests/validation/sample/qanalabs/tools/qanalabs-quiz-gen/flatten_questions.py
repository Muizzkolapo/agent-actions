...
class FlattenQuestionsInput(TypedDict, total=False):
    """Input schema for flattenquestions function."""
...
@udf_tool(input_type=FlattenQuestionsInput)
def flatten_questions(
    ...
    if len(validated) < len(flattened):
        print(
            f"📊 Filtered {len(flattened) - len(validated)} malformed questions, keeping {len(validated)}"
        )
