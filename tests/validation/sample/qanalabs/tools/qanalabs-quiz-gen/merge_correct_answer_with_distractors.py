...
def parse_answer_letters(answer_str) -> List[str]:
    """Parse answer string into list of letters - handles both string and complex list formats"""
    if not answer_str:
        return []

    # Debug: Print the input value and type
    print(f"Debug parse_answer_letters: input='{answer_str}', type={type(answer_str)}")

    # Handle complex format: [["text1", "text2"], "A,B,C"]
    if isinstance(answer_str, list) and len(answer_str) == 2:
        # Use the second element which should be the letter string
        answer_str = answer_str[1]
        print(f"Debug: Extracted from list: '{answer_str}', type={type(answer_str)}")

    # Handle simple string format: "A,B,C" or "AC" or "A"
    if isinstance(answer_str, str):
        # First try comma-separated format
        if "," in answer_str:
            letters = [letter.strip().upper() for letter in answer_str.split(",") if letter.strip()]
        else:
            # No comma - treat each character as a separate letter (e.g., "AC" -> ["A", "C"])
            letters = [char.upper() for char in answer_str if char.isalpha()]

        print(f"Debug: Parsed letters: {letters}")
        return letters

    # Fallback for other formats
    print(f"Debug: Fallback for type {type(answer_str)}")
    return []
...
def get_answer_indices(answer_letters: List[str]) -> List[int]:
    ...
    for letter in answer_letters:
        if len(letter) == 1 and letter.isalpha():
            ...
            print(f"Warning: Skipping invalid answer letter: '{letter}' (length: {len(letter)})")
    return indices
...
def create_combined_explanation(
    ...
    # Facts to Remember Section (only if flagged_items exists)
    if flagged_items:
        explanation_parts.append("## Facts to Remember:")
        for item in flagged_items:
            if "fact" in item:
                explanation_parts.append(f"• {item['fact']}")
        explanation_parts.append("")
...
def process_single_mcq(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Fallback to my_answer_key if answer is empty
    if not answer_str and data.get("my_answer_key"):
        answer_str = data.get("my_answer_key")
        print(f"Debug: Using my_answer_key '{answer_str}' as answer field was empty")
    ...
    result["distractors"] = [
        {
            ...
            "explanation_why_it_is_incorrect": next(
                (
                    exp
                    for exp_key, exp in explanations.items()
                    if exp_key.startswith("distractor_")
                    and data.get(f"distractor_{exp_key.split('_')[-1]}") == options[i]
                ),
                "",
            ),
        }
...
def add_answer_count_hint(question_text: str, answer_count: int) -> str:
    """
    Add answer count hint to 'Select all that apply' questions.
    Handles both plain text and HTML formats:
    - 'Select all that apply.' -> 'Select all that apply. <i>(choose X)</i>'
    - '<p>Select all that apply.</p>' -> '<p>Select all that apply. <i>(choose X)</i></p>'
    """
    ...
    # Pattern to match "Select all that apply" with optional period, handling HTML context
    # This matches both inside <p> tags and plain text
    pattern = r"(select\s+all\s+that\s+apply\.?)(\s*</p>)?"

    def replacement_func(match):
        text_part = match.group(1)
        closing_tag = match.group(2) or ""
        return f"<i>{text_part} (choose {answer_count})</i>{closing_tag}"

    # Apply replacement (case insensitive)
    updated_text = re.sub(pattern, replacement_func, question_text, flags=re.IGNORECASE)

    return updated_text


class MergeCorrectAnswerWithDistractorsInput(TypedDict, total=False):
    """Input schema for merge_correct_answer_with_distractors function.

    Source: node_15_generate_concept_explanation output
    Destination: node_16_OptionsCombiner output (creates options_combined structure)

    This function processes MCQ data to create the options_combined array
    structure with proper answer/distractor labeling and combined explanations.
    """
    ...
    # Feynman explanation fields (from node_14)
    question_explanation: str
    answer_reasoning: str
    key_concept_analogy: str
    memorable_takeaway: str

    # Concept explanation (from node_15)
    concept_explanation: str


@udf_tool(input_type=MergeCorrectAnswerWithDistractorsInput)
    ...
    if isinstance(mcq_data, list):
        ...
        for i, item in enumerate(mcq_data):
            try:
                ...
                # Add answer count hint for MA questions (only if question field exists)
                if processed_item.get("question_type") == "MA" and "question" in processed_item:
                    answer_count = len(processed_item.get("answer_indices", []))
                    processed_item["question"] = add_answer_count_hint(
                        processed_item["question"], answer_count
                    )

                processed_items.append(processed_item)
            except Exception as e:
                # Include failing record info in the error message
                error_msg = f"Error processing record at index {i}: {str(e)}\n"
                error_msg += f"Failing record: {json.dumps(item, indent=2, ensure_ascii=False)}"
        ...
        try:
            ...
            return json.dumps([processed_item], indent=2, ensure_ascii=False)
        except Exception as e:
            # Include failing record info in the error message
            error_msg = f"Error processing record: {str(e)}\n"
            error_msg += f"Failing record: {json.dumps(mcq_data, indent=2, ensure_ascii=False)}"
            raise ValueError(error_msg) from e
...
def process_file_content(data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if "answer_indices" in content:
        ...
        if isinstance(answer_field, list) and len(answer_field) == 2:
            ...
            original_answer_indices = [
                ord(letter) - ord("A") for letter in answer_letters if letter
            ]
                ...
                shuffled_options_combined.append(
                    {
                        "option": option,
                        "answer_or_distractor": "answer"
                        if option in [options[i] for i in original_answer_indices]
                        else "distractor",
...
def test_process_file_content():
    ...
    ma_data = {
        ...
        "options_combined": [
            {
                "option": "Store keys in Azure Key Vault",
                "answer_or_distractor": "answer",
                "explanation_why_it_is_correct_or_incorrect": "Secure storage is essential",
            },
            {
                "option": "Embed keys in source code",
                "answer_or_distractor": "distractor",
                "explanation_why_it_is_correct_or_incorrect": "This exposes keys",
            },
            {
                "option": "Rotate keys regularly",
                "answer_or_distractor": "answer",
                "explanation_why_it_is_correct_or_incorrect": "Regular rotation reduces risk",
            },
            {
                "option": "Share keys publicly",
                "answer_or_distractor": "distractor",
                "explanation_why_it_is_correct_or_incorrect": "This defeats security",
            },
            {
                "option": "Use RBAC restrictions",
                "answer_or_distractor": "answer",
                "explanation_why_it_is_correct_or_incorrect": "Access control is important",
            },
        ],
        "url": "https://example.com",
    }
...
def test_flagged_items():
    """Test the flagged_items functionality"""

    test_data = {
        "question": "How do you connect to Azure AI Foundry?",
        "options": [
        ...
        "answer": "A",
        "answer_explanation": "AIProjectClient with DefaultAzureCredential is the recommended approach for secure authentication.",
        "flagged_items": [
            {
                "fact": "Connect to an Azure AI Foundry project with AIProjectClient(credential=DefaultAzureCredential(), endpoint=project_endpoint).",
                "quote": "project_client = AIProjectClient(credential=DefaultAzureCredential(), endpoint=project_endpoint,)",
                "technical_level": "implementation",
                "cluster_tag": "cluster_1_7c63aa27",
            },
            {
                "fact": "Azure AI Foundry's OpenAI chat client can chat with any model deployed in the associated resource, including Microsoft Phi models.",
                "quote": "You can use the OpenAI chat client provided by an Azure AI Foundry project to chat with any model deployed in the associated Azure AI Foundry resource - even non-OpenAI models, such as Microsoft Phi models.",
                "technical_level": "integration",
                "cluster_tag": "cluster_1_097180ae",
            },
        ],
    }

    print("=== TESTING FLAGGED ITEMS FUNCTIONALITY ===")
    result = process_single_mcq(test_data)

    print("Combined Explanation:")
    print(result["combined_explanation"])
    print("\n" + "=" * 50)

    # Test without flagged_items
    test_data_no_flags = test_data.copy()
    del test_data_no_flags["flagged_items"]

    result_no_flags = process_single_mcq(test_data_no_flags)
    print("Combined Explanation (no flagged_items):")
    print(result_no_flags["combined_explanation"])
