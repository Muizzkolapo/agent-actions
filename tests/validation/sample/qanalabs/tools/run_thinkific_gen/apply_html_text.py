...
try:
    ...
    def randomize_quiz_options(content, enable_randomization=True):
        ...
        question_id = (
            content.get("id") or content.get("question_guid") or content.get("guid", "default")
        )
...
def render_code_snippets(code_snippets: List[Dict]) -> str:
    ...
    html_parts = [
        '<h4 style="color: #374151; margin-bottom: 1rem; font-weight: 600;">💻 Code Examples:</h4>'
    ]
    ...
    for snippet in code_snippets:
        ...
        if isinstance(snippet, str):
            ...
            print(
                f"WARNING: Expected dict or string but got {type(snippet)} in code_snippets: {snippet}"
            )
            continue

        if description:
            html_parts.append(
                f'<p style="color: #6B7280; font-size: 0.9rem; margin-bottom: 0.5rem; font-style: italic;">{description}</p>'
            )
...
def validate_and_clean_answers(obj: Dict[str, Any]) -> Dict[str, Any]:
    ...
    is_security_question = any(
        keyword in question for keyword in ["security", "secure", "api key", "sensitive", "crm"]
    )
    ...
    if is_security_question:
        ...
        if suspicious_answers:
            print(
                f"WARNING: Security question has suspicious correct answers: {suspicious_answers}"
            )
...
"""
    ...
    if len(correct_answers) == 1:
        ...
        html_parts.append(f'<div class="correct-answer-text">{answer["option"]}</div>')
        ...
        if answer.get("explanation_why_it_is_correct"):
            ...
            """)
            html_parts.append(
                f'<div class="explanation-text">{render_content(answer["explanation_why_it_is_correct"])}</div>'
            )
        ...
        if correct_answers and correct_answers[0].get("explanation_why_it_is_correct"):
            ...
            """)
            html_parts.append(
                f'<div class="explanation-text">{render_content(correct_answers[0]["explanation_why_it_is_correct"])}</div>'
            )
            html_parts.append("</div>")
    ...
    if distractors:
        ...
        for distractor in distractors:
            ...
            html_parts.append(f"""
                <div style="color: #DC2626; font-weight: 500; margin-bottom: 0.5rem; display: flex; align-items: flex-start;">
                    <span style="margin-right: 0.5rem;">✗</span>
                    <span>{distractor["option"]}</span>
                </div>
            """)
            if distractor.get("explanation_why_it_is_incorrect"):
                html_parts.append(
                    f'<div style="color: #6B7280; margin-left: 1.75rem; line-height: 1.5; font-size: 0.95rem;">{render_content(distractor["explanation_why_it_is_incorrect"])}</div>'
                )
            html_parts.append("</div>")
...
def format_explanation_from_options_combined(
    options_combined: List[Dict], question_text: str = ""
) -> str:
    """
...
"""
    ...
    correct_items = [
        item for item in options_combined if item.get("answer_or_distractor") == "answer"
    ]
    distractor_items = [
        item for item in options_combined if item.get("answer_or_distractor") == "distractor"
    ]
    ...
    if len(correct_items) == 1:
        ...
        html_parts.append(f'<div class="correct-answer-text">{item["option"]}</div>')
        ...
        if item["explanation_why_it_is_correct_or_incorrect"]:
            ...
            """)
            html_parts.append(
                f'<div class="explanation-text">{render_content(item["explanation_why_it_is_correct_or_incorrect"])}</div>'
            )
            html_parts.append("</div>")
        ...
        if correct_items and correct_items[0]["explanation_why_it_is_correct_or_incorrect"]:
            ...
            """)
            html_parts.append(
                f'<div class="explanation-text">{render_content(correct_items[0]["explanation_why_it_is_correct_or_incorrect"])}</div>'
            )
            html_parts.append("</div>")
    ...
    if distractor_items:
        ...
        for item in distractor_items:
            ...
            if item["explanation_why_it_is_correct_or_incorrect"]:
                html_parts.append(
                    f'<div style="color: #6B7280; margin-left: 1.75rem; line-height: 1.5; font-size: 0.95rem;">{render_content(item["explanation_why_it_is_correct_or_incorrect"])}</div>'
                )
            html_parts.append("</div>")
...
def format_options(options: List[str]) -> List[str]:
    ...
    for option in options:
        ...
        if option.strip().startswith("<") and option.strip().endswith(">"):
            ...
            formatted_options.append(f"<html><body>{option}</body></html>")
        else:
...
@udf_tool(input_type=FormatQuizObjectWithHtmlInput)
def format_quiz_object_with_html(obj: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if (
        "question_explanation" in content
        and "key_concept_analogy" in content
        and "memorable_takeaway" in content
    ):
        ...
        combined_feynman = f"\n{memorable}"
        collapsible_sections += (
            f"<details><summary>🧠 Memorable Takeaway</summary>{combined_feynman}</details>\n\n"
        )
    ...
    if "key_concepts" in content and content["key_concepts"]:
        ...
        if isinstance(key_concepts_list, list):
            items_html = "".join(
                [f"<li>{render_content(concept)}</li>" for concept in key_concepts_list]
            )
        ...
        collapsible_sections += (
            f"<details><summary>🔑 Key Concepts</summary>{concept_content}</details>\n\n"
        )
    ...
    if concept_text:
        ...
        collapsible_sections += (
            f"<details><summary>📖 Concept Explanation</summary>{concept_content}</details>\n\n"
        )
