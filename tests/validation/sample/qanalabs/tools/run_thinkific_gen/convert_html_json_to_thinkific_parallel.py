...
@udf_tool(input_type=ConvertHtmlJsonToThinkificInput, granularity=Granularity.FILE)
def convert_html_json_to_thinkific(json_data):
    ...
    for batch_name, batch_data in batches.items():
        ...
        output_path = f"/Users/muizz/Documents/codeshop/qanalabs/qanalabs-actions/qanalabs/agent_workflow/run_thinkific_gen/agent_io/target/converted_thinkific/{batch_name}.xlsx"
        ...
        with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
            try:
                ...
                template_workbook = pd.read_excel(
                    thinkific_template_path, engine="openpyxl", sheet_name=None
                )
                ...
                print(
                    f"An error occurred while preserving template formatting for batch {batch_name}: {e}"
                )
        ...
        print(
            f"Batch '{batch_name}': {len(transformed_data)} questions processed. File saved to {output_path}"
        )
