...
⚠️ **ALIGNMENT REQUIRED**: Extract Q&A that directly tests the objectives above. If content doesn't support these objectives, extract fewer questions or skip.

## WHAT TO EXTRACT

Focus on knowledge that challenges experienced engineers:

1. **Edge Cases & Gotchas**
   - What breaks if you do X incorrectly?
   - What's the non-obvious constraint?
   - What happens at scale/under load?
...
3. **Integration Complexity**
   ...
   - What ordering/sequencing matters?
...
## EXTRACT THESE (GENERAL {{ seed.exam_syllabus.platform_name }} CONCEPTS)

✅ **Protocol requirements**: MUST/SHOULD/MAY rules that apply to ALL implementations
✅ **Capability negotiation**: How clients and servers discover features
✅ **Message formats**: JSON-RPC structure, required fields, validation rules
✅ **Security boundaries**: Isolation, authorization, consent requirements
✅ **Error handling patterns**: How to handle protocol-level errors (not example-specific errors)
✅ **Objective-aligned knowledge**: Content that directly supports the learning objectives above
