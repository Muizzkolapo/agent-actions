...
actions:
  ...
  - name: classify_question_type
    ...
    context_scope:
      observe: [extract_facts.facts]     # Auto-inferred as context
```

This configuration declares:
- **What** each action does (prompt + schema)
- **When** it runs (dependencies = input sources)
- **What data** it receives (context_scope—context dependencies are auto-inferred)
...
### Design Decision: Progressive Data Exposure

A deliberate choice in Agent Actions is to implement **progressive data exposure**—only fields explicitly declared in `context_scope` are available to prompts and tools.

- **Dependencies define what data is available** in the action's `field_context`.
- **context_scope defines what data is exposed** to the LLM (`observe` and `passthrough`) and what is suppressed (`drop`).
- **Wildcards (`action.*`)** expose all content fields while **automatically filtering source metadata**.

This means an action can depend on upstream steps for execution, while still explicitly limiting what the model sees. The default is conservative: only fields explicitly listed in `observe` or `passthrough` are added to the LLM context.

**Why this choice:**
- **Safety**: explicit exposure reduces accidental leakage of sensitive fields
- **Cost control**: fewer tokens, fewer surprises
- **Predictability**: reviewers can tell exactly which fields drive model behavior
- **UDF protection**: wildcards automatically exclude metadata that would fail schema validation

**Automatic safeguards:**
- Wildcard `*` filters out source metadata (`id`, `source_guid`, `url`, `page_content`, etc.)
- Strict validation ensures declared references resolve to actual data
- Configuration errors are caught before execution, not at runtime

**Explicit override when needed:**
```yaml
context_scope:
  observe:
    - previous_action.*     # Content fields (metadata auto-filtered)
    - previous_action.url   # Explicit: include this metadata field
```

See the "Progressive Data Exposure" section under Key Features for full details.
...
### 3. Explicit Data Flow with Auto-Inferred Context

How data moves between actions should be visible, but configuration shouldn't be redundant:

```yaml
- name: generate_question
  dependencies: get_authoring_prompt    # Input source (determines execution count)
  context_scope:
    observe:
      - get_authoring_prompt.*          # Input (in dependencies)
      - classify_type.quiz_type         # Context (auto-inferred)
      - extract_facts.summary           # Context (auto-inferred)
    passthrough: [source.id]            # Passed to output
    drop: [source.raw_html]             # Excluded entirely
```

**The simplified dependency model:**
- `dependencies` = input sources only (what provides records to process)
- Context sources = auto-inferred from `context_scope` references

Actions referenced in `context_scope` but not in `dependencies` are automatically treated as context dependencies. No redundant declarations. No `primary_dependency` field needed.
...
actions:
  ...
  - name: generate_summary
    dependencies: extract_entities       # Input source (single)
    context_scope:
      observe:
        - extract_entities.entities      # Input
        - classify_sentiment.sentiment   # Context (auto-inferred)
...
### Progressive Data Exposure (Explicit Allowlisting)

Agent Actions implements a **progressive data exposure model**—only fields explicitly declared in `context_scope` are available to prompts and tools. This is a deliberate security and clarity choice.

**The Principle: Explicit Over Implicit**

```yaml
# Only these fields are available to the LLM
context_scope:
  observe:
    - extract_facts.summary      # Specific field
    - classify_type.*            # Wildcard: all fields from classify_type
  passthrough:
    - source.id
```

**Wildcard Support (`action.*`)**

The `*` wildcard works like SQL's `SELECT *`—it exposes all fields from a dependency:

```yaml
context_scope:
  observe:
    - previous_action.*    # All fields from previous_action
```

This is convenient but comes with automatic safeguards.

**Automatic Metadata Filtering**

When using wildcards, source metadata fields are **automatically excluded** to prevent context leaks:

```
Excluded from wildcard mapping:
├── id, source_guid, node_id, lineage
├── target_id, parent_target_id, root_target_id
├── url, title, source, page_content
├── referenced_in, record_index
└── _batch_filter_status, _passthrough_fields
```

This prevents accidental exposure of internal tracking fields to LLMs or UDFs. If you need a metadata field, declare it explicitly:

```yaml
context_scope:
  observe:
    - previous_action.*           # Content fields (metadata filtered)
    - previous_action.url         # Explicit: URL included despite filter
```

**Why This Design?**

| Approach | Risk | Agent Actions Solution |
|----------|------|----------------------|
| Implicit exposure (all fields available) | Token bloat, accidental data leaks | Explicit allowlist required |
| No wildcard support | Verbose configuration | `*` wildcard with safeguards |
| Raw wildcard (all fields) | Metadata leaks to UDFs | Automatic metadata filtering |

**Strict Validation**

The framework enforces that declared references resolve to actual data:

```
❌ Error: context_scope references fields not available in runtime context: unknown_action
```

This catches configuration errors before execution—you can't reference data that doesn't exist.

**Example: Safe Wildcard Usage**
...
- name: classify_question_type
  ...
  context_scope:
    observe:
      - flatten_raw_questions.*    # Gets: question_text, answer, difficulty
                                   # Excludes: id, source_guid, url, page_content
  prompt: |
    Classify this question:
    {{ flatten_raw_questions.question_text }}
```

The wildcard pulls in content fields while automatically filtering source metadata—safe by default, explicit when needed.
...
- name: improve_draft
  dependencies: critique_draft          # Input from critique
  guard:
    condition: 'critique_draft.score < 80'
    on_false: skip
  context_scope:
    observe:
      - generate_draft.content          # Context (auto-inferred)
      - critique_draft.critique         # Input
      - critique_draft.issues           # Input
  prompt: |
...
actions:
  ...
  - name: classify_type           # Step 2 (input from step 1)
    dependencies: extract_facts
  - name: generate_question       # Step 3 (input from step 2)
    dependencies: classify_type
  - name: validate_question       # Step 4 (input from step 3)
...
- ✅ Auto-Inferred Context Dependencies - Context sources are automatically inferred from `context_scope`; `dependencies` now means input sources only; `primary_dependency` deprecated
