...
def format_resolution_package(data: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    analysis = {
        ...
        "affected_area": content.get(
            "affected_area", content.get("analyze_issue", {}).get("affected_area")
        ),
    }
...
def _determine_next_actions(
    has_response: bool, has_task: bool, has_pr: bool, resolution: dict
) -> list[str]:
    ...
    if has_task:
        actions.append(
            f"Create ticket in project board (Priority: {resolution.get('priority', 'p2')})"
        )
