...
def format_ml_pipeline_report(data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    result = {
        ...
        'pipeline_summary': {
            'status': 'COMPLETED' if deployment_result['deployment_status'] == 'SUCCESS' else 'COMPLETED_NO_DEPLOY',
            'total_stages': 6,
        ...
        'metadata': {
            ...
            'pipeline_type': 'automated_ml_pipeline'
        }
    }
...
def _generate_recommendations(content):
    ...
    if high_risks:
        recommendations.append(f"Address {len(high_risks)} high-severity risks before production use")
