...
def format_rca_report(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if remediation_plan['recovery_time_estimate'] != 'Unknown':
        executive_summary += f"Estimated recovery time: {remediation_plan['recovery_time_estimate']}. "
    ...
    for idx, step in enumerate(propagation_path):
        timeline.append({
            'sequence': idx + 1,
            'event': step,
            'type': 'root_cause' if idx == 0 else ('effect' if idx == len(propagation_path) - 1 else 'intermediate')
        })
