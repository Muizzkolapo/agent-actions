...
| **Storage lookup** | Downstream actions | Saved to storage during first action; retrieved by identity (own `source_guid`, then `parent_source_guid`) | `processing.source_resolution.resolve_source_content()` |
| **FILE mode index** | FILE-granularity actions | Resolved by identity from a shared source index: own `source_guid`, then carried `parent_source_guid` (a minted row's producer, or an inherited ancestor); a miss on both falls back to the `source` namespace the record carries in its own content (as the RECORD-mode resolver does first), and only then skips it with reason `source_unresolved` (never positionally) | `scope_application._resolve_source_content()` |
