...
| **FILE mode index** | FILE-granularity actions | Resolved per-`source_guid` from a shared source index; an index miss skips the record with reason `source_unresolved` (no positional fallback) | `scope_application._resolve_source_content()` |
