...
| **FILE mode index** | FILE-granularity actions | Resolved by identity from a shared source index: own `source_guid`, then carried `parent_source_guid` (a minted row's producer, or an inherited ancestor); a miss on both skips the record with reason `source_unresolved` (no positional fallback) | `scope_application._resolve_source_content()` |
