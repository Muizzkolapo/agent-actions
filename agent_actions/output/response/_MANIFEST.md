...
| `expander.py` | Module | Facade: `ActionExpander` class orchestrates action-to-agent expansion, delegates to submodules. Forwards `expect:` unchanged; a judged expectation's `context:` refs are unioned into the action's own `context_scope.observe` so the framework's existing dependency inference picks up the referenced action automatically — inline `expectations:` only, since a named `suite:` isn't loadable at this layer. | `tooling.docs`, `schema` |
...
| `expander_merge.py` | Module | Config merge/init: directive merging, context_scope, chunk config, optional fields. `collect_judge_context_refs()` extracts an action's inline `llm_judge` `context:` refs for `expander.py` to union into `context_scope.observe`. | `config` |
