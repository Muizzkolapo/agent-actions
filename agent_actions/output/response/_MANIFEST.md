...
| `schema_conversion.py` | Module | Schema format conversion: `_convert_json_schema_to_unified`, `compile_field`. `_without_rules` strips any `expectations:` block at every depth, so a rule an author wrote on a field — or below one — never rides into a compiled provider payload. | `validation` |
...
| `response_builder.py` | Module | `ResponseBuilder` — unified output-field wrapping and usage extraction for all LLM providers. `wrap_non_json` puts plain text under the configured output field; `unwrap` is its counterpart and the one place that knows a provider returns either an already-parsed object or that envelope, so auxiliary callers (`expectations.judge`, `processing.recovery.critique`) do not each guess at key names. `UsageResult`, `ProviderResponseConfig`, `PROVIDER_RESPONSE_CONFIGS` registry. | `llm`, `output` |
