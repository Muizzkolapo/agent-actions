...
def randomize_quiz_options(content: Dict[str, Any], enable_randomization: bool = True) -> Dict[str, Any]:
...
def strip_html_tags(text: str) -> str:
    ...
    Used to extract plain text from VS Code mockups and other HTML.
    """
    if not text:
        return ''
...
def build_explanation_text(content: Dict[str, Any]) -> str:
    ...
    NOTE: Options may contain VS Code mockup HTML - we strip it to plain text
    for the explanation section to avoid rendering issues.
    """
    parts = []
    ...
    if question_text:
        parts.append(f"Question: {strip_html_tags(question_text)}")
        parts.append("")
    ...
    if correct_answers:
        if len(correct_answers) == 1:
            ...
            # Strip HTML from option (may contain VS Code mockup)
            option_text = strip_html_tags(answer.get('option', ''))
            parts.append(f"  {option_text}")
            ...
            if correct_answers[0].get('explanation_why_it_is_correct'):
                parts.append(f"  Explanation: {correct_answers[0]['explanation_why_it_is_correct']}")
        parts.append("")
    ...
    if distractors:
        ...
        for distractor in distractors:
            # Strip HTML from option (may contain VS Code mockup)
            option_text = strip_html_tags(distractor.get('option', ''))
            parts.append(f"  ✗ {option_text}")
...
def combine_quiz_fields(obj: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not content.get('answer_indices') or len(content.get('answer_indices', [])) == 0:
        ...
        if answer_str:
            ...
            if ',' in str(answer_str):
                letters = [letter.strip().upper() for letter in str(answer_str).split(',') if letter.strip()]
            else:
    ...
    concept_value = content.get('concept_explanation') or content.get('summary') or content.get('summary_content', '')
    ...
    result = {
        ...
        "url_list": content.get('url_list', []) or ([content.get('url')] if content.get('url') else [])
    }
