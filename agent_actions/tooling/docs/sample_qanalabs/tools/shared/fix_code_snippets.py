...
def is_well_formatted(snippet):
    ...
    has_markdown = any(marker in snippet for marker in ['* `', '+ `', '- `', '\n  +', '\n  *', '\n* '])
...
@udf_tool()
def fix_code_snippets(data):
    """
    Tool function: Fix code_snippets in content field

    Returns: Same data with fixed code_snippets
    """
    ...
    # Fix code snippets if they exist
    ...
    # Forward all fields using content.copy()
    result = content.copy()
