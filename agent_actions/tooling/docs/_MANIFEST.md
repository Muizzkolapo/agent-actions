...
|------|------|-------------|---------|
| `generator.py` | Module | `CatalogGenerator` that builds catalog entries, enriches actions with input/output metadata, and merges runs/logs/prompts. `_copy_readme_images()` copies referenced images to `artefact/images/` and rewrites paths in README content. Uses `_empty_runs_data` from `run_tracker` to initialize runs.json. `catalog.json` is written atomically via `tempfile.mkstemp` + `os.replace`. Integrates `change_tracker` to diff current scan against previous catalog. | `prompt_generation`, `output.response.loader`, `run_tracker`, `scanner.ReadmeData`, `change_tracker` |
| `change_tracker.py` | Module | `collect_resource_mtimes`, `load_previous_mtimes`, `compute_changes` — compares file modification times between catalog generations to produce added/modified/removed changesets per resource category. | `generator` |
