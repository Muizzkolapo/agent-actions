...
function renderActionDetails(action, workflowsUsingAction) {
    ...
    if (action.type === 'llm' && action.prompt && action.prompt.content) {
        ...
        // Use PromptRenderer for rich formatting if available
        if (window.PromptRenderer) {
            promptSection.innerHTML = `
                <h2>Prompt</h2>
                <div class="action-prompt-content" style="max-height: 500px; overflow-y: auto; border: 1px solid var(--border-color); border-radius: 8px; padding: 1rem;">
                    ${PromptRenderer.render(action.prompt.content)}
                </div>
            `;
        } else {
            promptSection.innerHTML = `
...
function renderPromptDetails(prompt) {
    // Update header meta
    document.getElementById('prompt-source-file').textContent = prompt.source_file_name || 'unknown.md';
    document.getElementById('prompt-lines').textContent = `${prompt.line_start || 0} - ${prompt.line_end || 0}`;
    document.getElementById('prompt-chars').textContent = (prompt.length || 0).toLocaleString();

    // Extract variables count
    const variables = window.PromptRenderer ? PromptRenderer.extractVariables(prompt.content || '') : [];
    document.getElementById('prompt-vars').textContent = variables.length;

    // Render tab contents
    const renderedContainer = document.getElementById('prompt-rendered');
    const rawContainer = document.getElementById('prompt-raw');
    const variablesContainer = document.getElementById('prompt-variables');

    if (window.PromptRenderer && prompt.content) {
        renderedContainer.innerHTML = PromptRenderer.render(prompt.content);
        rawContainer.innerHTML = PromptRenderer.renderRaw(prompt.content);
        variablesContainer.innerHTML = PromptRenderer.renderVariables(prompt.content);
    } else {
        // Fallback rendering
        renderedContainer.innerHTML = `<pre class="pr-raw">${escapeHtml(prompt.content || 'No content available')}</pre>`;
        rawContainer.innerHTML = `<pre class="pr-raw">${escapeHtml(prompt.content || 'No content available')}</pre>`;
        variablesContainer.innerHTML = '<div class="pr-empty">Variables extraction not available</div>';
    }

    // Initialize tabs
    initPromptTabs(prompt.content);
}

function initPromptTabs(content) {
    const tabs = document.querySelectorAll('#prompt-view .ph-tab[data-tab]');
    const contents = document.querySelectorAll('#prompt-view .ph-tab-content');
