...
from agent_actions.agents.extractors.text_loader import TextLoader
from agent_actions.agents.extractors.json_loader import JsonLoader
from agent_actions.agents.extractors.tabular_loader import TabularLoader
from agent_actions.agents.extractors.xml_loader import XmlLoader
from agent_actions.agents.generators.content_generator import ContentGenerator

# Configure logger
logger = logging.getLogger(__name__)
from agent_actions._internal.staging.staging_processor import StagingProcessor  
            ...
            for idx, record in enumerate(processed_content):
                ...
                if analysis.requires_chunking:
                    ...
                    for chunk_idx, chunk_record in enumerate(chunked_records):
                        # Add metadata similar to batch mode for consistency
                        chunk_record.update({
                    ...
                    # Keep original record with metadata
                    record.update({
                        "source_guid": str(uuid.uuid5(uuid.NAMESPACE_OID, json.dumps(record, sort_keys=True))),
                        "target_id": str(uuid.uuid4()),
                        "record_index": idx,
                    })
