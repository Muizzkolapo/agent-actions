...
class TemplateVariableError(PreFlightValidationError):
    """Raised when Jinja2 template references undefined variables.

    This error provides detailed information about which variables are
    missing and what variables are available in the context.

    Args:
        missing_variables: List of variables referenced but not in context
        available_variables: List of variables available in context
        template_line: Optional line number where the error occurred
        agent_name: Name of the agent with the template error
        mode: Execution mode ('batch' or 'online')
        context: Additional context dict
        cause: Original exception
    """
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            message,
            available_references=available_variables,
            missing_references=missing_variables,
            hint=hint,
            mode=mode,
...
class ContextStructureError(PreFlightValidationError):
    ...
    Args:
        message: Description of the structure mismatch
        expected_fields: List of fields that were expected
        actual_fields: List of fields that were provided
        agent_name: Name of the agent being validated
        mode: Execution mode ('batch' or 'online')
        context: Additional context dict
        cause: Original exception
    ...
    def __init__(
        ...
        *,
        ...
        hint = None
        if missing:
            hint = f"Add the missing field(s) to your input data: {', '.join(missing)}"
...
class DependencyValidationError(PreFlightValidationError):
    """Raised when circular or invalid dependencies are detected.

    Args:
        message: Description of the dependency issue
        cycle_path: List of agent names forming the cycle (if circular)
        agent_name: Name of the agent where the issue was detected
        mode: Execution mode ('batch' or 'online')
        context: Additional context dict
        cause: Original exception
    """
