...
| `state.py` | Module | ActionStateManager — manages action execution state persistence and queries. Terminal states: `completed`, `failed`, `skipped`. Key methods: `is_failed()`, `is_skipped()`, `get_pending_actions()`, `get_skipped_actions()`, `is_workflow_done()`, `get_summary()`. | `workflow`, `state_management` |
