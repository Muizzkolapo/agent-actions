...
| `output.py` | Module | `ActionOutputManager`: loads upstream outputs and resolves version correlation. Raises `AllVersionsFilteredError` (public; callers must catch — the executor cascade-skips) when every version source was guard-filtered, vs `ConfigurationError` when a source has output or is unexpectedly empty. `detect_explicit_version_consumption()` result is lazy-cached per instance. | `output`, `workflow` |
