...
| `executor.py` | Module | Handles running actions (LLM/tool/HITL) and interfacing with processors. Stamps an action with the record and file limits in force, the model and a config digest, so a run shortened from the command line or the environment is not later served as a finished one. The stamp is written when a batch is submitted as well as when an action completes, because the run that walks the files is the one that can describe them and a later run collects them; that collecting run keeps what the submission wrote. A run that is only repairing records neither stamps nor compares a limit. | `llm`, `workflow` |
