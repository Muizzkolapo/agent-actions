...
| `executor.py` | Module | Handles running actions (LLM/tool/HITL) and interfacing with processors. Stamps a completed action with the record limit that was in force and with what the run actually processed (`records_processed`/`truncated`), so a run truncated from the command line or the environment is not later served as a finished one, while a limit too large to have dropped anything leaves the action completed instead of clearing its dispositions and re-running it. A stamp that cannot say keeps the older, coarser behaviour; a run that is only repairing records neither stamps nor compares any of it. | `llm`, `workflow` |
...
| `runner_file_processing.py` | Module | File walking, merging, and storage-backend processing extracted from `runner.py`. Standalone functions that take a `runner` param for instance dispatch. A per-file failure is not fatal to the action, so any file the walk loses — one that failed processing, a stored entry that would not read or parse, or a whole upstream whose listing failed — marks the action's record count unknown, since a count missing those records would let a later limit read as one that could not have dropped anything. | `workflow`, `processing`, `utils` |
