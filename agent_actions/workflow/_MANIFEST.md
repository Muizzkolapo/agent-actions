...
| `pipeline_file_mode.py` | Module | FILE-granularity tool and HITL processing handlers extracted from `ProcessingPipeline`. `extract_tool_input` flattens a record envelope into a tool input dict by re-injecting `record["source"]` as the `source` namespace and merging `content[*]` action namespaces. Returns `ProcessingResult.failed()` when a tool returns empty output with non-empty input. | `processing`, `workflow` |
