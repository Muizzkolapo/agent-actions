...
| `executor.py` | Module | Handles running actions (LLM/tool/HITL) and interfacing with processors. Stamps a completed action with the record limit that was in force, so a run truncated from the command line or the environment is not later served as a finished one; a run that is only repairing records neither stamps nor compares that limit. | `llm`, `workflow` |
