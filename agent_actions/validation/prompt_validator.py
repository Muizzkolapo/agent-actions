...
class PromptValidator(BaseValidator):
    ...
    def _check_prompt_file_size(self, prompt_file: Path) -> bool:
        ...
        if file_size > self._MAX_PROMPT_SIZE:
            self.add_error(f"Prompt file '{prompt_file.name}' exceeds maximum size ({file_size} bytes > {self._MAX_PROMPT_SIZE} bytes).")
            return False
        return True
    ...
    def _check_prompt_id_duplicates(self, file_name: str, prompt_ids_in_file: List[str], all_prompt_ids_seen: Set[str]) -> Tuple[Set[str], List[str]]:
        ...
        if cross_file_duplicates:
            ...
            self.add_error(f"Prompt IDs in file '{file_name}' duplicate IDs from other files: {id_list}.")
    ...
    def _validate_single_prompt_file(self, prompt_file: Path, all_prompt_ids_seen: Set[str]) -> int:
        ...
        try:
            ...
            duplicate_ids_within_file, cross_file_duplicates = self._check_prompt_id_duplicates(prompt_file.name, prompt_ids_in_file, all_prompt_ids_seen)
