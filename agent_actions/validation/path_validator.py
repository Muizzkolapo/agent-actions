...
class PathValidator(BaseValidator):
    ...
    def _validate_path_entity_logic(self, path_obj: Path, entity_type: str, entity_name: str, required: bool, must_be_readable: bool, must_be_writable: bool, must_be_executable: bool) -> None:
        """
    ...
    def _ensure_directory_exists_logic(self, path_obj: Path, directory_name: str, create_if_missing: bool, must_be_writable_after_creation: bool) -> None:
        """
    ...
    def validate(self, data: Any, config: Optional[Dict[str, Any]]=None) -> bool:
        ...
        if operation == 'validate_user_code_path':
            ...
            if operation == 'validate_file' or operation == 'validate_directory':
                ...
                self._validate_path_entity_logic(path_obj, entity_type=entity_type, entity_name=path_name, required=data.get('required', True), must_be_readable=data.get('must_be_readable', True), must_be_writable=data.get('must_be_writable', False), must_be_executable=data.get('must_be_executable', False))
            elif operation == 'ensure_directory_exists':
                self._ensure_directory_exists_logic(path_obj, directory_name=path_name, create_if_missing=data.get('create_if_missing', True), must_be_writable_after_creation=data.get('must_be_writable_after_creation', True))
            else:
                self.add_error(f'Unknown operation: {operation}')
        if self.has_errors():
            combined_msg = '; '.join(self.get_errors())
            ServiceLogger.log_operation_error(logger, f"PathValidator operation '{operation}' failed", Exception(combined_msg), error_details=self.get_errors())
