...
class PathValidator(BaseValidator):
    ...
    def _validate_path_entity_logic(
        ...
        if options.required and not path_exists:
            ...
            ServiceLogger.log_operation_error(
                logger, operation_desc, Exception(msg)
            )
            return
        if path_exists:
            ...
            if options.must_be_readable and not os.access(path_obj, os.R_OK):
                self.add_error(
                    f'{entity_name} ({entity_type}) is not readable: {path_obj}'
                )
            if options.must_be_writable and not os.access(path_obj, os.W_OK):
                self.add_error(
                    f'{entity_name} ({entity_type}) is not writable: {path_obj}'
                )
            if options.must_be_executable and not os.access(path_obj, os.X_OK):
                self.add_error(
                    f'{entity_name} ({entity_type}) is not executable: '
                    f'{path_obj}'
                )
        if not self.has_errors():
            ServiceLogger.log_operation_success(
                logger, operation_desc, path=str(path_obj)
            )
    ...
    def _ensure_directory_exists_logic(
        ...
        """
        operation_desc = (
            f"ensure directory exists '{directory_name}' at {path_obj}"
        )
        ServiceLogger.log_operation_start(logger, operation_desc)
        if not self._ensure_path_exists(path_obj):
            if create_if_missing:
                logger.debug(
                    'Creating directory: %s at %s', directory_name, path_obj
                )
                try:
                    path_obj.mkdir(parents=True, exist_ok=True)
                    if (must_be_writable_after_creation
                            and not os.access(path_obj, os.W_OK)):
                        self.add_error(
                            f'Created directory {directory_name} but it is '
                            f'not writable: {path_obj}'
                        )
                except (OSError, ValueError) as e:
                    msg = (
                        f'Failed to create {directory_name} directory at '
                        f'{path_obj}: {e}'
                    )
                    self.add_error(msg)
                    ServiceLogger.log_operation_error(
            ...
            self.add_error(
                f'{directory_name} path exists but is not a directory: '
                f'{path_obj}'
            )
        elif (must_be_writable_after_creation
                and not os.access(path_obj, os.W_OK)):
            self.add_error(
                f'{directory_name} directory exists but is not writable: '
                f'{path_obj}'
            )
        if not self.has_errors():
            ServiceLogger.log_operation_success(
    ...
    def _validate_user_code_path_logic(
        self,
        user_code_path_str: Optional[str]
    ) -> None:
        """
    ...
    def _handle_user_code_path_operation(self, path_input: Any) -> None:
        ...
        if path_input is not None and not isinstance(path_input, str):
            self.add_error(
                "'path' for 'validate_user_code_path' must be a string "
                "or None."
            )
        else:
    ...
    def _handle_ensure_directory_operation(
        self,
        path_obj: Path,
        data: Dict[str, Any]
    ) -> None:
        ...
        self._ensure_directory_exists_logic(
            ...
            must_be_writable_after_creation=data.get(
    ...
    def validate(
        ...
        if operation == 'validate_user_code_path':
            ...
            self.add_error(
                "'path' (Path or str) is required for this operation and "
                "must be valid."
            )
