...
class SchemaValidator(BaseValidator):
    ...
    JSON_SCHEMA_RESERVED_KEYWORDS: Set[str] = {
        ...
        'pattern', 'enum', 'const', 'multipleOf', 'minimum', 'maximum',
        'exclusiveMinimum', 'exclusiveMaximum', 'format', 'contentEncoding',
        'contentMediaType', 'title', 'description', 'default', 'examples',
    ...
    def _process_schema_file(
        ...
        try:
            ...
            self.add_error(
                f'Could not read or parse {display_name} '
        ...
        common_issues = self._check_common_schema_issues_static(
    ...
    def _is_valid_json_schema_structure(schema_data: Dict[str, Any]) -> bool:
        ...
        schema_keywords = {
            ...
            'definitions', 'additionalProperties', 'allOf', 'anyOf', 'oneOf'
        }
    ...
    def _check_common_schema_issues_static(
        ...
        if 'type' not in schema_data:
            issues.append(
                f"Missing 'type' property at the root level of "
        ...
        if schema_data.get('type') == 'object':
            ...
            if not properties:
                issues.append(
                    f"Schema '{schema_name}' is 'object' type but has "
                    f"no defined 'properties'."
                )
            required = schema_data.get('required', [])
            if isinstance(required, list):
                undefined_props = [
                    prop for prop in required if prop not in properties
                ]
                if undefined_props:
                    issues.append(
    ...
    def validate(
        ...
        if schema_files_to_validate is not None and (
            ...
            self.add_error(
                "Data field 'schema_files' must be a list of strings "
                "if provided."
            )
        if self.has_errors():
            return False
        ...
        if not self._is_directory(schema_dir):
            ...
            return False
        logger.debug(
            "Starting schema validation for agent '%s' in directory: %s",
            agent_name,
            schema_dir
        )
        if schema_files_to_validate:
            files_to_process = [
                schema_dir / fname for fname in schema_files_to_validate
            ]
        else:
            files_to_process = list(schema_dir.glob('*.json'))
            if not files_to_process:
                self.add_warning(
                    f"No .json schema files found in {schema_dir} "
    ...
    def _check_object_compatibility(
        ...
        if missing:
            issues.append(
                f"'{schema2_name}' requires properties not defined in "
                f"'{schema1_name}': {missing}."
            )
    ...
    def check_schema_compatibility(
        ...
        self.clear_errors()
        logger.debug(
            "Checking schema compatibility between '%s' and '%s'.",
            schema1_name,
            schema2_name
        )
