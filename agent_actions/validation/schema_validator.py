...
class SchemaValidator(BaseValidator):
    ...
    """
    JSON_SCHEMA_RESERVED_KEYWORDS: Set[str] = {'type', 'properties', 'required', 'additionalProperties', 'items', 'minItems', 'maxItems', 'uniqueItems', 'minLength', 'maxLength', 'pattern', 'enum', 'const', 'multipleOf', 'minimum', 'maximum', 'exclusiveMinimum', 'exclusiveMaximum', 'format', 'contentEncoding', 'contentMediaType', 'title', 'description', 'default', 'examples', 'definitions', 'allOf', 'anyOf', 'oneOf', 'not', 'if', 'then', 'else', '$schema', '$id', '$ref'}
    ...
    def _process_schema_file(self, file_path: Path, schema_name: str, agent_name: Optional[str]='general') -> None:
        ...
        if not self._is_valid_json_schema_structure(schema_data):
            self.add_error(f"{display_name} (file: {file_path.name}) does not appear to be a valid JSON Schema document. It should have properties like 'type', 'properties', etc.")
        try:
    ...
    def _is_valid_json_schema_structure(schema_data: Dict[str, Any]) -> bool:
        ...
        if not isinstance(schema_data, dict):
            return False
        schema_keywords = {'type', 'properties', 'required', 'items', '$schema', 'definitions', 'additionalProperties', 'allOf', 'anyOf', 'oneOf'}
    ...
    def _check_common_schema_issues_static(cls, schema_data: Dict[str, Any], schema_name: str) -> List[str]:
        ...
        if 'definitions' in schema_data:
            ...
            unused_defs = [def_name for def_name in schema_data.get('definitions', {}) if f'#/definitions/{def_name}' not in definition_refs and f'#/$defs/{def_name}' not in definition_refs]
            if unused_defs:
        ...
        acceptable_custom = {'examples', 'errorMessage', 'readonly', 'writeonly', 'deprecated', '$defs'}
    ...
    def validate(self, data: Any, config: Optional[Dict[str, Any]]=None) -> bool:
        ...
        if schema_files_to_validate is not None and (not isinstance(schema_files_to_validate, list) or not all((isinstance(f, str) for f in schema_files_to_validate))):
        ...
        if s1_type == 'object' and s2_type == 'object':
            ...
            for prop_name in common_props:
                ...
                if prop1_detail.get('type') != prop2_detail.get('type'):
                    issues.append(f"Property '{prop_name}' type mismatch: '{schema1_name}' is '{prop1_detail.get('type')}', '{schema2_name}' is '{prop2_detail.get('type')}'.")
        for issue in issues:
