...
class StartupValidator:
    ...
    def validate_environment_variables(self, constructor_path: Optional[str] = None) -> bool:
        ...
        try:
            ...
            logger.debug(
                "Environment configuration loaded successfully. Environment: %s",
            ...
            for error in e.errors():
                ...
                self.errors.append(
                    f"Environment variable validation failed for {field_name}: {error_msg}"
                )
            return False
    ...
    def _validate_api_keys(self, required_vendors: Optional[set] = None) -> None:
        ...
        if self.environment_config.is_development():
            if required_vendors is None:
                ...
                self._check_development_required_vendors(required_vendors, vendor_key_mapping)
    ...
    def _analyze_config_for_vendors(self, constructor_path: Optional[str]) -> set:
        ...
        try:
            ...
            logger.debug("Detected vendors in configuration: %s", required_vendors)
        except (OSError, ValueError) as e:
            logger.debug("Could not analyze configuration for vendor requirements: %s", e)
            return set()
        return required_vendors
    ...
    def validate_configuration_files(
        self, constructor_path: Optional[str] = None, default_path: Optional[str] = None
    ) -> bool:
    ...
    def get_validation_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "environment": {
                ...
                "debug_enabled": (
                    self.environment_config.debug_logging if self.environment_config else None
                ),
            },
        }
    ...
    def raise_on_errors(self) -> None:
        ...
        if self.errors:
            raise StartupValidationError(
                f"Startup validation failed with {len(self.errors)} error(s)", self.errors
            )


def validate_startup(
    constructor_path: Optional[str] = None, default_path: Optional[str] = None
) -> EnvironmentConfig:
