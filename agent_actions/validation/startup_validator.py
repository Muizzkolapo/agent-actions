...
class StartupValidator:
    ...
    def _validate_api_keys(self, required_vendors: Optional[set]=None) -> None:
        ...
        if not self.environment_config:
            return
        vendor_key_mapping = {'openai': ('openai_api_key', 'OPENAI_API_KEY', 'OpenAI models'), 'anthropic': ('get_effective_claude_key', 'CLAUDE_API_KEY/ANTHROPIC_API_KEY', 'Claude models'), 'google': ('google_api_key', 'GOOGLE_API_KEY', 'Gemini models'), 'gemini': ('google_api_key', 'GOOGLE_API_KEY', 'Gemini models')}
        available_keys = []
        ...
        if self.environment_config.is_production():
            if not any((key for key in available_keys if key)):
                self.errors.append('Production environment requires at least one API key (OPENAI_API_KEY, CLAUDE_API_KEY/ANTHROPIC_API_KEY, or GOOGLE_API_KEY)')
    ...
    def validate_performance_settings(self) -> bool:
        ...
        if self.environment_config.is_production():
            ...
            if self.environment_config.default_batch_size < 50:
                self.warnings.append(f'Batch size is small for production ({self.environment_config.default_batch_size}) - consider increasing for better performance')
        return True
    ...
    def get_validation_report(self) -> Dict[str, Any]:
        ...
        return {'success': len(self.errors) == 0, 'errors': self.errors.copy(), 'warnings': self.warnings.copy(), 'environment': {'config_loaded': self.environment_config is not None, 'environment_type': self.environment_config.agent_actions_env if self.environment_config else None, 'debug_enabled': self.environment_config.debug_logging if self.environment_config else None}}

    def raise_on_errors(self) -> None:
        """Raise StartupValidationError if there are validation errors."""
        if self.errors:
            raise StartupValidationError(f'Startup validation failed with {len(self.errors)} error(s)', self.errors)

def validate_startup(constructor_path: Optional[str]=None, default_path: Optional[str]=None) -> EnvironmentConfig:
