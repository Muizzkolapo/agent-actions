...
class ValidationResultsArtifact(BaseArtifact):
    ...
    def add_attempt(self, agent_id: str, validator_type: str, attempt: int, status: str, error: Optional[str]=None, response: Optional[str]=None) -> None:
        agent_entry = self.results.setdefault(agent_id, {})
        validator_entry = agent_entry.setdefault(validator_type, [])
        validator_entry.append({'attempt': attempt, 'status': status, 'error': error, 'response': response})
