...
| `expectations_validator.py` | Module | Refuses preflight, via `find_expectation_defects`, when an action's `expect:` block (inline `expectations:` list, named `suite:` reference, or a bare block that defaults to the action's own schema) names an unregistered type, an unaccepted parameter, a missing required parameter, a severity that is not a level, or a `field:` selector the action's output cannot produce. It builds each suite the way the runner does, so rules declared on a schema's fields are validated too, and a `row_condition` argument is parsed and cross-checked like any other condition. Also checks `llm_judge`-specific parameters: a non-positive-integer `votes:`, and a `context:` value that is not a list, names an unknown action, or names a field that action does not produce (checked against the full per-action `available_fields` map, not just the current action). For inline `expectations:`, a malformed `context:` ref is often caught earlier by the pre-existing `context_scope.observe` static-type validator instead, since a judged expectation's `context:` refs are auto-injected into `observe:` from an action's inline list and from its own schema's rules — this validator remains the only check for named `suite:` mode (which never auto-injects, since a suite is not loadable at the expander layer) and for a non-list `context:` value (which never reaches `observe:` at all). For `expression` rules it also parses the condition (grammar + guard blocklist, rejecting `udf:` with a pointer to `expectation_check`), refuses a stray `field:`, a constant condition referencing no record fields, and references to fields the action does not produce (dotted refs checked at the top segment). Field-presence siblings closed in the same block: a missing, empty-string, or empty-list `field:` on any non-expression type is a defect rather than a runtime crash or vacuous pass. Independently of the entry checks, `_repair_mode_defects` refuses the execution shapes a repair policy cannot serve: a judged `context:` ref under `run_mode: batch` (batch validates from the stored result and has no `llm_context`, so the judge would fail every record on a missing context source rather than on its own rule), and — for any `repair` other than `none` — the reserved `repair: {prompt:}` form, a tool action (re-running a deterministic UDF yields the same output, mirroring the runtime gate in `InvocationStrategyFactory`), `granularity: file` (a multi-record response can never satisfy the record-shaped structural gate), and a schema that is non-mapping or declared-but-not-inlined (the gate would silently check record shape only). Enum members and cased YAML strings are both matched. Wired at `PreflightService._check_expectation_defects`. | `validation` |
