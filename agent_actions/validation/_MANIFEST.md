...
| `expectations_validator.py` | Module | Refuses preflight, via `find_expectation_defects`, when an action's `expect:` block (inline `expectations:` list or named `suite:` reference) names an unregistered type, an unaccepted parameter, a missing required parameter, or a `field:` selector the action's output cannot produce. Wired at `PreflightService._check_expectation_defects`. | `validation` |
