...
class RunCommandArgs(BaseModel):
    ...
    agent: str = Field(
        ...,
        description="Agent configuration file name without path or extension"
    )
    user_code: Optional[DirectoryPath] = Field(
        None,
        description="Path to the user's code folder containing UDFs"
    )
    ...
    concurrency_limit: int = Field(
        5,
        description="Maximum number of agents to run concurrently in "
                    "parallel execution",
        ge=1,
        le=50
    )
    upstream: bool = Field(
        False,
        description="Recursively execute upstream dependent workflows"
    )
