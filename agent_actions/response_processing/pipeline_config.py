...
class StageConfig(BaseModel):
    ...
    timeout: Optional[int] = Field(
        default=None, ge=1, description='Stage timeout in seconds'
    )
    retry_attempts: int = Field(
        default=0, ge=0, description='Number of retry attempts'
    )
    retry_delay: float = Field(
        default=1.0, ge=0.0, description='Delay between retries'
    )
    depends_on: List[str] = Field(
        default_factory=list, description='Stages this stage depends on'
    )
    ...
    validate_input: bool = Field(
        default=True, description='Validate input data'
    )
    validate_output: bool = Field(
        default=True, description='Validate output data'
    )
...
class AgentStageConfig(StageConfig):
    ...
    few_shot: int = Field(
        default=0, ge=0, description='Number of few-shot samples'
    )
    parallel_execution: bool = Field(
        default=False, description='Execute agent in parallel'
    )
...
class WorkflowConfig(BaseModel):
    ...
    error_handling: ErrorHandlingStrategy = Field(
        default=ErrorHandlingStrategy.COLLECT_ERRORS,
        description='Error handling strategy'
    )
    max_concurrency: int = Field(
        default=5, ge=1, description='Maximum concurrent executions'
    )
    global_timeout: Optional[int] = Field(
        default=None, ge=1, description='Global workflow timeout'
    )
    agents: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict, description='Agent configurations'
    )
    execution_order: List[str] = Field(
        default_factory=list, description='Agent execution order'
    )
...
class PipelineConfig(BaseModel):
    ...
    max_parallel_stages: int = Field(
        default=5, ge=1, description='Maximum parallel stages'
    )
    ...
    pre_pipeline_hooks: List[str] = Field(
        default_factory=list, description='Pre-pipeline hooks'
    )
    post_pipeline_hooks: List[str] = Field(
        default_factory=list, description='Post-pipeline hooks'
    )
...
__all__ = [
    'ExecutionMode', 'ErrorHandlingStrategy', 'StageType', 'StageConfig',
    'AgentStageConfig', 'WorkflowConfig', 'PipelineConfig', 'PipelineRegistry'
]
