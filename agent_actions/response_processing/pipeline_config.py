...
class StageConfig(BaseModel):
    ...
    run_condition: Optional[str] = Field(default=None, description='Condition for running this stage')
...
class WorkflowConfig(BaseModel):
    ...
    error_handling: ErrorHandlingStrategy = Field(default=ErrorHandlingStrategy.COLLECT_ERRORS, description='Error handling strategy')
    ...
    agents: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description='Agent configurations')
    ...
    artifact_config: Dict[str, Any] = Field(default_factory=dict, description='Artifact configuration')
    global_filters: List[str] = Field(default_factory=list, description='Global filter conditions')
    where_clauses: Dict[str, Any] = Field(default_factory=dict, description='WHERE clause configurations')
    ...
    def validate_execution_order(cls, v, info):
        ...
        if 'agents' in info.data:
            ...
            if undefined_agents:
                raise ConfigValidationError('execution_order', f'Agents in execution_order not defined: {undefined_agents}', context={'undefined_agents': undefined_agents, 'execution_order': v, 'defined_agents': list(agents.keys()), 'operation': 'validate_workflow'})
        return v
...
class PipelineConfig(BaseModel):
    ...
    pipeline_timeout: Optional[int] = Field(default=None, ge=1, description='Pipeline timeout in seconds')
    stages: List[StageConfig] = Field(default_factory=list, description='Pipeline stages')
    stage_registry: Dict[str, StageConfig] = Field(default_factory=dict, description='Registered stages')
    ...
    stage_interceptors: Dict[str, List[str]] = Field(default_factory=dict, description='Stage interceptors')
        ...
        for stage in self.stages:
            for dependency in stage.depends_on:
                if dependency not in stage_names:
                    raise ConfigValidationError('stage_dependency', f"Stage '{stage.name}' depends on undefined stage '{dependency}'", context={'stage_name': stage.name, 'undefined_dependency': dependency, 'defined_stages': list(stage_names), 'operation': 'validate_dependencies'})
        return True
...
__all__ = ['ExecutionMode', 'ErrorHandlingStrategy', 'StageType', 'StageConfig', 'AgentStageConfig', 'WorkflowConfig', 'PipelineConfig', 'PipelineRegistry']