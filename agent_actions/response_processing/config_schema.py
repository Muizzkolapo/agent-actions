...
class WhereClauseConfig(BaseModel):
    ...
    clause: str = Field(
        ..., description='SQL-like WHERE clause for filtering', max_length=10000
    )
    scope: FilterScope = Field(
        default=FilterScope.ITEM,
        description=(
            "Filtering scope: 'item' for individual items, "
            "'agent' for entire agent execution"
        )
    )
    ...
    behavior: WhereClauseBehavior = Field(
        ...
        description=(
            "Behavior when condition fails: 'skip' (passthrough) or 'filter' (remove)"
        )
    )
...
class SkipConditionConfig(BaseModel):
    ...
    condition_type: Literal[
        'previous_outputs_empty', 'previous_outputs_count',
...
class AgentConfig(BaseModel):
    ...
    model_vendor: Optional[str] = Field(
        default=None,
        description=(
            "Model vendor/provider: 'openai', 'gemini', 'anthropic', "
            "'groq', or 'tool'"
        )
    )
    api_key: Optional[str] = None
    ...
    json_mode: bool = Field(
        default=True, description='Enable JSON mode for structured output'
    )
...
__all__ = [
    'DefaultAgentConfig', 'AgentConfig', 'WhereClauseConfig',
    'EnhancedAgentConfig', 'FilterScope', 'SkipConditionConfig'
]
