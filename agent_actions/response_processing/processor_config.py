...
class ProcessorConfig(BaseModel):
    ...
    processing_mode: ProcessingMode = Field(
        default=ProcessingMode.AUTO, description="Processing mode for the processor"
    )
...
class CacheConfig(BaseModel):
    ...
    strategy: CacheStrategy = Field(
        default=CacheStrategy.MEMORY, description="Caching strategy to use"
    )
    ttl: int = Field(default=300, ge=0, description="Cache TTL in seconds (0 for no expiration)")
    max_size: Optional[int] = Field(
        default=1000, ge=1, description="Maximum cache size (strategy dependent)"
    )
    key_prefix: str = Field(default="processor", description="Prefix for cache keys")
    redis_url: Optional[str] = Field(
        default=None, description="Redis connection URL (for Redis strategy)"
    )
    cache_dir: Optional[str] = Field(
        default=None, description="Directory for file cache (for File strategy)"
    )
...
class DataProcessorConfig(ProcessorConfig):
    ...
    observe_handling: Literal["ignore", "separate", "merge"] = Field(
        default="separate", description="How to handle observe fields"
    )
    cache_config: Optional[CacheConfig] = Field(
        default=None, description="Cache configuration for data processor"
    )
...
class GeneratorConfig(ProcessorConfig):
    ...
    agent_creation_timeout: int = Field(
        default=60, ge=1, description="Timeout for agent creation in seconds"
    )
    prompt_caching_enabled: bool = Field(
        default=True, description="Enable prompt caching for performance"
    )
    few_shot_samples_limit: int = Field(
        default=10, ge=0, description="Maximum number of few-shot samples"
    )
...
class ContentProcessorConfig(ProcessorConfig):
    ...
    batch_size: int = Field(
        default=100, ge=1, le=10000, description="Batch size for content processing"
    )
    parallel_processing: bool = Field(
        default=True, description="Enable parallel processing of content"
    )
    error_handling_strategy: Literal["fail_fast", "skip_errors", "collect_errors"] = Field(
        default="collect_errors", description="Strategy for handling processing errors"
    )
    max_content_size: Optional[int] = Field(
        default=None, ge=1, description="Maximum content size in bytes (None for unlimited)"
    )
...
class BatchProcessorConfig(BaseModel):
    ...
    default_batch_size: int = Field(
        default=100, ge=1, le=10000, description="Default batch size for operations"
    )
    max_batch_size: int = Field(default=1000, ge=1, description="Maximum allowed batch size")
    batch_timeout: int = Field(
        default=600, ge=1, description="Timeout for batch operations in seconds"
    )
    parallel_batches: bool = Field(default=True, description="Enable parallel batch processing")
    max_concurrent_batches: int = Field(
        default=5, ge=1, le=50, description="Maximum number of concurrent batches"
    )
        ...
        if "default_batch_size" in info.data:
            if v < info.data["default_batch_size"]:
                raise ConfigValidationError(
                    ...
                    context={
                        "max_batch_size": v,
                        "default_batch_size": info.data["default_batch_size"],
...
class ProcessorRegistryConfig(BaseModel):
    ...
    data_processors: Dict[str, DataProcessorConfig] = Field(
        default_factory=dict, description="Registered data processors configuration"
    )
    generators: Dict[str, GeneratorConfig] = Field(
        default_factory=dict, description="Registered generators configuration"
    )
    content_processors: Dict[str, ContentProcessorConfig] = Field(
        default_factory=dict, description="Registered content processors configuration"
    )
    batch_config: BatchProcessorConfig = Field(
        default_factory=BatchProcessorConfig, description="Batch processing configuration"
    )
    cache_config: CacheConfig = Field(
        default_factory=CacheConfig, description="Global cache configuration"
    )
    ...
    def get_processor_config(
        ...
        if processor_type == "content_processor":
