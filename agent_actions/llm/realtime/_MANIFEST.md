...
| `cleaner.py` | Module | Removes a workflow's working directories during `agac clean`. With `--all` it reads the batch registry out of the store it is about to wipe and reclaims what a provider recorded locally about those batches, which nothing could find afterwards. | `cli`, `file_io`, `llm.batch` |
