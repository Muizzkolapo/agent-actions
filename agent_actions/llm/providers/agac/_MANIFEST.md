...
| `local_batch_records.py` | `.agac/batch_state/` | Deletes | — |
...
| `local_batch_records.py` | Module | Where generic batch code reclaims what a provider recorded in the project about a batch nothing can reach any more. Only this provider records one; every other keeps its copy at the vendor. | `llm.batch`, `llm.providers` |
