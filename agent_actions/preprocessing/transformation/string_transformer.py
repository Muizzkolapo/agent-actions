...
class StringProcessor:
    ...
    def call_user_function(call_args, tools_path=None, context_data_str=None):
        ...
        try:
            ...
            raise ConfigurationError(f"Could not import module for UDF '{function_name}'", context={'function_name': function_name, 'tools_path': tools_path, 'operation': 'call_user_function'}, cause=e) from e
        except AttributeError as e:
            raise ConfigurationError(f"Could not find function '{function_name}' in its module", context={'function_name': function_name, 'operation': 'call_user_function'}, cause=e) from e
        except Exception as e:
            raise AgentActionsException(f"Error executing user function '{function_name}'", context={'function_name': function_name, 'operation': 'call_user_function'}, cause=e) from e
...
class Tokenizer:
    ...
    def num_tokens_from_string(string: str, encoding_name: str) -> int:
        ...
        try:
            ...
            raise ConfigurationError(f"Invalid tiktoken encoding name '{encoding_name}'", context={'encoding_name': encoding_name, 'operation': 'num_tokens_from_string'}, cause=e) from e
        except Exception as e:
            string_preview = string[:100] if len(string) > 100 else string
            raise AgentActionsException('Tokenization error', context={'string_preview': string_preview, 'encoding_name': encoding_name, 'operation': 'num_tokens_from_string'}, cause=e) from e

    @staticmethod
    def split_text_content(text: str, chunk_size: int, overlap: int, tokenizer_model: str='cl100k_base', split_method: str='tiktoken') -> List[str]:
        """
        ...
        """
        if chunk_size <= 0:
            raise ConfigurationError('chunk_size must be a positive integer', context={'chunk_size': chunk_size, 'operation': 'split_text_content'})
        if overlap < 0:
            raise ConfigurationError('overlap cannot be negative', context={'overlap': overlap, 'operation': 'split_text_content'})
        if overlap >= chunk_size and split_method in ('tiktoken', 'chars'):
            raise ConfigurationError('overlap must be smaller than chunk_size for token/character splits', context={'overlap': overlap, 'chunk_size': chunk_size, 'split_method': split_method, 'operation': 'split_text_content'})
        try:
            ...
            raise AgentActionsException('Text splitting error', context={'text_preview': text_preview, 'chunk_size': chunk_size, 'overlap': overlap, 'split_method': split_method, 'operation': 'split_text_content'}, cause=e) from e
    ...
    def _split_with_tiktoken(text: str, chunk_size: int, overlap: int, tokenizer_model: str) -> List[str]:
        ...
        try:
            ...
            raise AgentActionsException(f"Error encoding text with tiktoken model '{tokenizer_model}'", context={'tokenizer_model': tokenizer_model, 'operation': '_split_with_tiktoken'}, cause=e) from e
        chunks = []
        start_idx = 0
    ...
    def _split_with_spacy(text: str, chunk_size: int, overlap: int, tokenizer_model: str) -> List[str]:
        ...
        try:
            ...
            raise ConfigurationError('spaCy is not installed. Install with: pip install agent-actions[nlp] or pip install spacy>=3.0.0', context={'operation': '_split_with_spacy', 'split_method': 'spacy'}, cause=e) from e
        try:
            nlp = spacy.load('en_core_web_sm')
        except OSError as e:
            raise ConfigurationError("spaCy model 'en_core_web_sm' is not installed. Download with: python -m spacy download en_core_web_sm", context={'operation': '_split_with_spacy', 'model': 'en_core_web_sm'}, cause=e) from e
        encoding = tiktoken.get_encoding(tokenizer_model)
        doc = nlp(text)
    ...
    @staticmethod
    def _split_with_custom_method(text: str, chunk_size: int, overlap: int, tokenizer_model: str, split_method: str) -> List[str]:
        try:
            ...
            raise ConfigurationError(f"Could not import custom split_method module '{split_method}'", context={'split_method': split_method, 'tools_path': tools_path, 'operation': '_split_with_custom_method'}, cause=e) from e
        except AttributeError as e:
            raise ConfigurationError(f"Could not find custom split_method function '{split_method}' in its module", context={'split_method': split_method, 'operation': '_split_with_custom_method'}, cause=e) from e
        except Exception as e:
            raise AgentActionsException(f"Error executing custom split_method '{split_method}'", context={'split_method': split_method, 'operation': '_split_with_custom_method'}, cause=e) from e