...
| `IDGenerator` | Class | Static helpers `generate_target_id`, `generate_node_id`, `derive_source_guid` (deterministic first-stage identity), `generate_source_guid` (random, for new entities), and `generate_content_hash`. | `field_management`, `lineage` |
