...
| `limits.py` | Module | Resolves the per-action record and file limits, each from three sources: the workflow config, a limit carried on the action config for this run, and an environment variable (`AGAC_RECORD_LIMIT`, `AGAC_FILE_LIMIT`). Both axes share one copy of the precedence rules: a limit asked for on the run wins by source rather than by which number is smaller, the variable is validated either way, and the retired `AGAC_MAX_RECORDS` name is rejected rather than ignored. The environment is checked once while the run is assembled, so an unusable value fails before any action starts. Resolving is silent — only the site applying a limit knows whether anything was actually held back, so it owns the announcement. Also decides which records a limit keeps: the first N by position, plus as many positions carrying an identity as the action already holds stored rows for, so a run repairing records cannot delete rows it is not rewriting nor backfill past a limit. | `config`, `storage` |
