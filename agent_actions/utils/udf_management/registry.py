...
class FileUDFResult:
    ...
    def __init__(self, outputs: list[dict[str, Any]]):
        for i, out in enumerate(outputs):
            ...
            # Range is left to the run; the input count is not knowable here. A bool
            # needs its own clause: isinstance(True, int) holds, so True would pass as
            # position 1 and hand the row that input's identity.
            ...
            if stray:
                # Bare for a scalar: wrapping it implies a list the author never wrote.
                offenders = stray if isinstance(src, list) else src
                raise ValueError(
                    f"FileUDFResult output[{i}] source_index must give input positions as "
                    f"non-negative integers; got {offenders!r}. Use None to declare that "
                    f"no single input produced this row."
                )
