...
| `expression.py` | Module | Parses and evaluates an `expression` rule's condition against the whole record. Reuses the guard machinery verbatim — `GuardParser`'s blocklist plus the `WhereClauseParser` grammar — so a condition string is portable between `guard:` and `expect:` unchanged; `udf:` conditions are rejected with a pointer to `expectation_check`. A false condition's detail carries the values of every field the condition read; a missing field or an unquoted-literal semantic error becomes a failed outcome carrying the evaluator's own message. Imports from `guards/` and `input/preprocessing/parsing/` are deliberate and stay within the pipeline-free rule, which bans only `workflow`/`processing`/`storage`. | `validation` |
