...
A named `suite:` reference is only resolvable when a project root is available.
The bare-block default reads the action's own resolved schema — its fields'
rules and its own block — and needs no root.
...
A judged expectation's `context:` refs are auto-injected into the producing
action's own `context_scope.observe` from its inline `expectations:` list and
from the rules of its own resolved schema — both are readable at
`ActionExpander._create_agent_from_action`, where the injection happens. A named
`suite:` is the exception: that layer has no suite-loading capability, so such a
suite's `context:` refs are inert even on the day project-root threading
