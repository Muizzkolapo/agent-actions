...
| `report.py` | Module | Aggregates the `expect` verdicts stored on records into per-action, per-rule counts (`tally_action`, `ActionTally`, `RuleTally`, and `output_record_count`, which counts the records an action produced output for so a caller can tell one skipped for every record from one that never ran). A skipped outcome counts as neither a pass nor a fail, so it cannot move a rate it never ran in, and `pass_rate` is `None` rather than zero when nothing was checked — an action that never ran expectations and one whose rules all failed are different answers. Rules sort by failure count, so the rule costing an action the most records leads. Pipeline-free: reads plain dicts and imports only the verdict key. | `validation` |
...
| `cli` | inbound | `cli/expect.py` reports stored verdicts through `report.tally_action` |
...
an expansion multiplies judge spend by its record count. `retry:` nests inside each repair
iteration, costing `max_iterations x retry.max_attempts` provider calls in the worst case.
