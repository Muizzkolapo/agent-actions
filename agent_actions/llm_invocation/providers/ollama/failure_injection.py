...
Environment variables:
    OLLAMA_ONLINE_FAIL_FIRST_N=2    Fail first N online calls, then succeed
    OLLAMA_BATCH_FAIL_RECORDS=2     Fail first N records in each batch
    OLLAMA_BATCH_FAIL_IDS=id1,id2   Fail specific custom_ids in batch

Usage:
    # Online mode - fail first 2 calls, 3rd succeeds
    OLLAMA_ONLINE_FAIL_FIRST_N=2 python -m agent_actions run workflow.yml

    # Batch mode - fail first 2 records in batch
    OLLAMA_BATCH_FAIL_RECORDS=2 python -m agent_actions run workflow.yml
...
def is_online_injection_enabled() -> bool:
    ...
    return int(os.getenv("OLLAMA_ONLINE_FAIL_FIRST_N", "0")) > 0


def is_batch_injection_enabled() -> bool:
    """Check if batch failure injection is configured."""
    return int(os.getenv("OLLAMA_BATCH_FAIL_RECORDS", "0")) > 0 or bool(
        os.getenv("OLLAMA_BATCH_FAIL_IDS", "").strip()
    )
...
def maybe_inject_online_failure(model: str) -> None:
    ...
    Call this BEFORE the actual Ollama API call. If injection is triggered,
    ...
    fail_n = int(os.getenv("OLLAMA_ONLINE_FAIL_FIRST_N", "0"))
    if fail_n <= 0:
        return
    ...
    if _online_call_count <= fail_n:
        ...
        logger.debug(
            "[INJECTION] Online call %d - past failure threshold (%d), allowing through",
            _online_call_count,
            fail_n,
        )
...
def should_fail_batch_record(custom_id: str, record_index: int) -> bool:
    ...
    # Check by record count
    fail_n = int(os.getenv("OLLAMA_BATCH_FAIL_RECORDS", "0"))
    ...
    fail_ids_str = os.getenv("OLLAMA_BATCH_FAIL_IDS", "").strip()
    if fail_ids_str:
        fail_ids = {id_.strip() for id_ in fail_ids_str.split(",") if id_.strip()}
        if custom_id in fail_ids and custom_id not in _failed_batch_ids:
            _failed_batch_ids.add(custom_id)
            logger.info("[INJECTION] Batch record failed (in ID list): %s", custom_id)
            return True
...
def get_injection_status() -> dict:
    ...
    return {
        ...
        "online_fail_threshold": int(os.getenv("OLLAMA_ONLINE_FAIL_FIRST_N", "0")),
        "batch_fail_records": int(os.getenv("OLLAMA_BATCH_FAIL_RECORDS", "0")),
        "batch_fail_ids": os.getenv("OLLAMA_BATCH_FAIL_IDS", ""),
