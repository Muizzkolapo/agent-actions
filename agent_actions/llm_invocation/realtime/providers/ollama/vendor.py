...
class OllamaHandler(BaseVendorHandler):
    ...
    def _prep_messages(prompt_config: str, context_data: str):
        ...
        return [{'role': 'system', 'content': prompt_config}, {'role': 'user', 'content': context_data}]
    ...
    def call_json(_api_key, _agent_config, _prompt_config, _context_data, _schema):
        ...
        """
        raise NotImplementedError('Ollama does not support JSON mode. This method should never be called due to invoke() validation.')
    ...
    def call_non_json(_api_key, agent_config, prompt_config, context_data, _schema=None):
        ...
        response = OllamaHandler._get_client(agent_config).chat(model=model, messages=messages, stream=False)
