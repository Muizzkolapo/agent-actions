...
class AgentManager:
    ...
    def get_agent_paths(agent_name: str) -> tuple[str, str, str]:
        ...
        if not project_root:
            raise AgentNotFoundError('Could not find agent_actions.yml in current or parent directories', context={'current_directory': str(Path.cwd()), 'marker_file': 'agent_actions.yml'})
        ...
        raise AgentNotFoundError('Could not find configuration for agent', context={'agent_name': agent_name, 'project_root': str(project_root), 'expected_file': f'{agent_name}.yml'})
