...
class BatchService:
    ...
    def __init__(
        ...
        self._task_preparator = task_preparator or BatchTaskPreparator(agent_indices=agent_indices, dependency_configs=dependency_configs)
        ...
        self._provider_resolver = provider_resolver or BatchProviderResolver(provider_cache=self._provider_cache, default_provider=self.provider)
    ...
    def submit_batch_job(self, agent_config, batch_name, data, output_directory=None, force=False):
        ...
        if not force_submission and output_directory:
            ...
            if entry and entry.is_in_flight:
                logger.info('Found existing in-flight batch job for %s: %s', batch_name, entry.batch_id)
    ...
    def retrieve_results(self, batch_id: str, output_dir: str, file_path: str=None):
        try:
            ...
            result_file = output_path / (f'{Path(file_path).stem}_results.jsonl' if file_path else f'{batch_id}_results.jsonl')
    ...
    def process_all_batch_results(self, output_directory: str, agent_config: Dict[str, Any]=None):
        ...
        if not registry:
            raise ProcessingError('No batch registry found', context={'output_directory': output_directory})
