...
from agent_actions.validation.validation_results import ValidationResultsArtifact
...
class ArtifactManager:
    ...
    def record_agent_success(self, result: AgentResult, response: Any, execution_time: float, adapter_response: Optional[Dict[str, Any]]=None) -> None:
        with self._lock:
    ...
    def record_agent_error(self, result: AgentResult, error: Exception, execution_time: float, context: Optional[Dict[str, Any]]=None) -> None:
        with self._lock:
            result.status = 'error'
            result.execution_time = execution_time
            result.message = str(error)
            result.failures = 1
            result.error_details = {'error_type': type(error).__name__, 'error_message': str(error), 'context': context or {}}
            if result.timing:
                result.timing[-1].complete()
            self._mark_dirty()

    def record_validation_attempt(self, agent_id: str, validator_type: str, attempt: int, status: str, error: Optional[str]=None, response: Optional[str]=None) -> None:
    ...
    def record_error(self, error_type: str, operation: str, target: str, error: Exception, context: Optional[Dict[str, Any]]=None, user_message: Optional[str]=None) -> None:
        error_context = {'error_type': error_type, 'operation': operation, 'target': target, 'error_class': type(error).__name__, 'error_message': str(error), 'context': context or {}, 'user_message': user_message, 'timestamp': datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')}
