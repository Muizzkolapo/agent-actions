...
from agent_actions.configuration.base import BaseArtifact, ArtifactMetadata
...
class ManifestArtifact(BaseArtifact):
    ...
    def add_agent(self, unique_id: str, agent_config: Dict[str, Any]) -> None:
        self.agents[unique_id] = {'unique_id': unique_id, 'name': agent_config.get('name', unique_id.split('.')[-1]), 'agent_type': agent_config.get('agent_type'), 'model_vendor': agent_config.get('model_vendor'), 'model_name': agent_config.get('model_name'), 'config': agent_config, 'depends_on': agent_config.get('depends_on', []), 'tags': agent_config.get('tags', []), 'meta': agent_config.get('meta', {}), 'interceptors': agent_config.get('interceptors', [])}

    def add_workflow(self, unique_id: str, workflow_config: Dict[str, Any]) -> None:
        self.workflows[unique_id] = {'unique_id': unique_id, 'name': workflow_config.get('name', unique_id.split('.')[-1]), 'agents': workflow_config.get('agents', []), 'dependencies': workflow_config.get('dependencies', []), 'config': workflow_config}

    def to_dict(self) -> Dict[str, Any]:
        return {'metadata': {**self.metadata.to_dict(), 'project_name': self.project_name, 'project_path': self.project_path}, 'agents': self.agents, 'workflows': self.workflows, 'dependencies': self.dependencies, 'project_config': self.project_config}
