...
class PathManager:
    ...
    def get_standard_path(
        ...
        **template_vars
        ...
        """
        cache_key = f"{path_type.value}:{agent_name}:{node_name}:{hash(frozenset(template_vars.items()))}"
    ...
    def validate_path(self, path: Path, requirements: Optional[Dict[str, bool]] = None) -> bool:
        ...
        if errors:
            if self.config.validate_permissions:
                ...
                logger.warning(f"Path validation warnings: {'; '.join(errors)}")
                return False
    ...
    def create_mirror_path(self, source_path: Path, source_base: str, target_base: str) -> Path:
        ...
        try:
            ...
            raise PathManagerError(f"Source base '{source_base}' not found in path {source_path}")
