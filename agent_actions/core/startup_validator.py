...
from ..models.environment_config import EnvironmentConfig, Environment
from ..cli.exceptions import ConfigurationError
from ..handlers.config_handler import ConfigManager
...
class StartupValidator:
    ...
    def validate_environment_variables(self, constructor_path: Optional[str] = None) -> bool:
        ...
        try:
            ...
            for error in e.errors():
                ...
                self.errors.append(f"Environment variable validation failed for {field_name}: {error_msg}")
            return False
        
        # Analyze configuration to determine required vendors
        required_vendors = self._analyze_config_for_vendors(constructor_path)
        
        # Check for API key availability based on environment
    ...
    def _validate_api_keys(self, required_vendors: Optional[set] = None) -> None:
        ...
        Args:
            required_vendors: Set of vendor types that are actually used in the configuration.
                             If None, will warn about all missing keys (legacy behavior).
        """
        if not self.environment_config:
            return
        
        # Map vendor types to their API key requirements
        vendor_key_mapping = {
            'openai': ('openai_api_key', 'OPENAI_API_KEY', 'OpenAI models'),
            'anthropic': ('get_effective_claude_key', 'CLAUDE_API_KEY/ANTHROPIC_API_KEY', 'Claude models'),
        ...
        # Get available keys
        available_keys = []
        available_keys.append(self.environment_config.openai_api_key)
        available_keys.append(self.environment_config.get_effective_claude_key())
        available_keys.append(self.environment_config.google_api_key)
        
        # Check for at least one API key in production
        ...
        if self.environment_config.is_development():
            if required_vendors is None:
                ...
                if not self.environment_config.get_effective_claude_key():
                    self.warnings.append("CLAUDE_API_KEY/ANTHROPIC_API_KEY not set - Claude models will not be available")
                ...
                # Context-aware behavior - only warn about keys for vendors being used
                for vendor in required_vendors:
                    if vendor.lower() in vendor_key_mapping:
                        key_attr, key_name, model_desc = vendor_key_mapping[vendor.lower()]
                        
                        # Get the key value
                        if key_attr == 'get_effective_claude_key':
                            key_value = self.environment_config.get_effective_claude_key()
                        else:
                            key_value = getattr(self.environment_config, key_attr)
                        
                        if not key_value:
                            self.warnings.append(f"{key_name} not set - {model_desc} will not be available")
    
    def _analyze_config_for_vendors(self, constructor_path: Optional[str]) -> set:
        """Analyze configuration file to determine which vendors are actually used.
        ...
        try:
            ...
            # Read and parse the YAML file directly (without templates)
            with open(config_file, 'r', encoding='utf-8') as f:
                content = f.read()
                
            # Simple regex-based analysis to find model_vendor entries
            import re
            vendor_pattern = r'model_vendor:\s*["\']?([^"\'\s]+)["\']?'
            vendors = re.findall(vendor_pattern, content, re.IGNORECASE)
            
            for vendor in vendors:
                required_vendors.add(vendor.lower())
                
            logger.debug(f"Detected vendors in configuration: {required_vendors}")
                                    
        except Exception as e:
            logger.debug(f"Could not analyze configuration for vendor requirements: {e}")
            # Fall back to no specific vendors (legacy behavior)
            return set()
    ...
    def validate_file_system_access(self) -> bool:
        ...
        # Check read access to current directory
        ...
        # Check write access (try to create a temporary file)
        try:
        ...
        # Check for templates directory if it exists
    ...
    def validate_configuration_files(self, constructor_path: Optional[str] = None, 
        ...
        try:
            ...
            if config_manager.user_config is None:
                self.errors.append(f"Failed to load user configuration from: {constructor_path}")
                return False
            ...
            self.errors.append(f"Unexpected error validating configuration files: {e}")
            return False
    ...
    def validate_dependencies(self) -> bool:
        ...
        required_packages = [
            'yaml', 'pydantic', 'openai', 'anthropic', 
            'tiktoken', 'flask', 'networkx', 'pandas'
        ]
    ...
    def validate_performance_settings(self) -> bool:
        ...
        # Warn about performance settings in production
        if self.environment_config.is_production():
            if not self.environment_config.enable_parallel_processing:
                self.warnings.append(
                    "Parallel processing is disabled in production - this may impact performance"
                )
            
            if self.environment_config.default_batch_size < 50:
                self.warnings.append(
                    f"Batch size is small for production ({self.environment_config.default_batch_size}) - "
                    "consider increasing for better performance"
                )
        
        return True
    
    def run_full_validation(self, constructor_path: Optional[str] = None,
                           default_path: Optional[str] = None) -> bool:
    ...
    def get_validation_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "environment": {
                ...
                "environment_type": self.environment_config.agent_actions_env if self.environment_config else None,
                "debug_enabled": self.environment_config.debug_logging if self.environment_config else None,
            }
        }
...
def validate_startup(constructor_path: Optional[str] = None,
                    default_path: Optional[str] = None) -> EnvironmentConfig:
    """Convenience function to run startup validation and return environment config.
    ...
    if not validator.environment_config:
        raise StartupValidationError("Environment configuration not loaded", [])
