...
def discover_udfs(user_code_path: Path) -> Dict[str, Dict[str, Any]]:
    ...
    # Add user_code_path to sys.path if not already present
    ...
    for py_file in python_files:
        try:
            ...
            if spec and spec.loader:
                ...
                # Add to sys.modules to enable imports within the module
                sys.modules[module_name] = module
                spec.loader.exec_module(module)

        except DuplicateFunctionError:
            # Re-raise duplicate function errors
            raise
        except Exception as e:
            # Wrap other errors in UDFLoadError
            raise UDFLoadError(
                module=module_name,
                file=str(py_file),
                error=str(e),
...
def validate_udf_references(config: Dict[str, Any]) -> None:
    ...
    # Extract all impl references from config
    extract_impl_refs(config)

    # Validate each reference
    for impl_ref in impl_references:
        # All references must be simple function names (no dots)
        try:
            get_udf(impl_ref)
        except FunctionNotFoundError:
            # Re-raise with additional context about where it was referenced
            raise
