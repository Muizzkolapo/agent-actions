...
class RenderCommand:
    ...
    def _render_template(self, agent_config_file: Path) -> str:
        ...
        try:
            logger.info('Rendering template with configuration...', extra={'agent_name': self.args.agent_name, 'config_file': str(agent_config_file), 'template_dir': str(self.template_dir)})
            rendered_template = render_pipeline_with_templates(str(agent_config_file), str(self.template_dir))
