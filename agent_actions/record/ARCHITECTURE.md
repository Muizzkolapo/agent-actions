...
```
{
    ...
    "producer_source_guids": ["abc-123"], ← inputs of this action the row consumed but does not carry (per-stage)
...
RECORD_LIFECYCLE_FIELDS (carried forward; _state_history is appended to)
├── _state_history
...
Source GUIDs are **not** generated by the record module — they are assigned upstream by `IDGenerator` (`utils/id_generation.py`) and carried through by `_carry_persistent_fields()`.

```
First-stage records:
    IDGenerator.derive_source_guid(record) → uuid5(raw payload)
    A deterministic hash of the raw record, taken BEFORE the volatile framework
    envelope (target_id/batch_id/node_id) is added — nothing is stripped by name,
    so the same input yields the same guid across runs (checkpoint resume) and
    across the ingestion vs processing stamp sites.
