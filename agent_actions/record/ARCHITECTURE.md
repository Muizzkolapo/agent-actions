...
```
First-stage records:
    IDGenerator.derive_source_guid(record) → uuid5(content)
    A deterministic content hash so the same input yields the same guid across
    runs (checkpoint resume) and across the ingestion vs processing stamp sites.
