...
| `record_helpers.py` | Module | Shared record assembly helpers: `build_tombstone`, `build_exhausted_tombstone`, `carry_framework_fields`, `apply_version_merge`, `extract_existing_content`, `get_existing_content`. `extract_existing_content` is now a thin alias over `get_existing_content` (no first-stage source synthesis -- `source` lives on the envelope). | `record`, `processing` |
...
| `task_preparer.py` | Module | Unified task preparation (normalize, prompt, guard) for batch/online. Short-circuits upstream-unprocessed records before context loading. Reads `source` from `item["source"]` (envelope top-level) and forwards into prompt context. `_get_source_content` is deprecated and only used as a defensive fallback for legacy callers. | `input`, `prompt` |
