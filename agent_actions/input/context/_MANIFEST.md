...
| `normalizer.py` | Module | Expands `context_scope` directives, keeps raw/expanded copies, and mutates agent configs in place during preprocessing. | `validation`, `filtering` |
...
| `normalize_all_agent_configs` | Function | Walks execution order, builds version-name maps, and attaches `context_scope_expanded` without modifying the original `context_scope`. | `preprocessing`, `validation` |
