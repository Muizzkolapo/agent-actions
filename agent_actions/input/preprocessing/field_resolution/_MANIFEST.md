...
| `context_provider.py` | Module | `EvaluationContextProvider`/`EvaluationContext` build rich guard/filter contexts that include upstream action outputs, workflow metadata, source data, and version info. Reads `source` from the record envelope (`current_item["source"]`) instead of accepting a `source_content` parameter. | `filtering`, `preprocessing`, `validation` |
