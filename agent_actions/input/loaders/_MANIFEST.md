...
| `source_data.py` | Module | `SourceDataLoader` that loads source data via storage backend (`read_source`), requiring a `StorageBackend` at init. | `config.interfaces`, `errors`, `storage.backend` |
