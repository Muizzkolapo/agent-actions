...
def _parse_data_source(raw: Any) -> DataSourceConfig:
    ...
    if isinstance(raw, str):
        ...
        # Known type names that require dict config — catch typos early
...
def _resolve_local(
    ...
    if not config.folder:
        raise ConfigurationError(
            ...
            context={"data_source": config.model_dump(), "operation": "resolve_local_data_source"},
        )
...
def _resolve_api(
    ...
    if not config.url:
        raise ConfigurationError(
            ...
            context={"data_source": config.model_dump(), "operation": "resolve_api_data_source"},
        )
