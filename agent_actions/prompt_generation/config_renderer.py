...
class JinjaTemplateRenderer(TemplateRenderer):
    ...
    def render(self, config_path: str, template_dir: str, output_path: Optional[str]=None) -> str:
        ...
        try:
            ...
            data_config_file = {'operation': 'validate_file', 'path': Path(config_path), 'path_name': 'Configuration file', 'required': True, 'must_be_readable': True}
            if not path_validator.validate(data_config_file):
                all_validations_passed = False
                error_messages.extend(path_validator.get_errors())
            data_template_dir = {'operation': 'validate_directory', 'path': Path(template_dir), 'path_name': 'Template directory', 'required': True, 'must_be_readable': True}
            ...
            if output_path:
                ...
                data_output_dir = {'operation': 'ensure_directory_exists', 'path': output_dir_as_path, 'path_name': 'Output directory', 'create_if_missing': True, 'must_be_writable_after_creation': True}
...
class YAMLConfigParser(ConfigParser):
    ...
    def parse(self, config_data: str) -> AgentConfigMap:
        ...
        try:
            ...
            if not config_data:
                raise ConfigurationError('Empty configuration data', context={'operation': 'parse_yaml'})
            config = yaml.safe_load(config_data)
            if not isinstance(config, dict):
                raise ConfigurationError('Expected configuration to be a dictionary', context={'operation': 'parse_yaml', 'actual_type': type(config).__name__})
            ServiceLogger.log_operation_success(logger, 'parse YAML configuration')
            return cast(AgentConfigMap, config)
        except YAMLError as e:
            ErrorHandler.handle_config_error(e, 'parse', 'YAML configuration', context={'config_data': config_data})
        except Exception as e:
            ErrorHandler.handle_config_error(e, 'parse', 'configuration', context={'config_data': config_data})
...
class FileOutputWriter(OutputWriter):
    ...
    def write(self, output_path: str, content: str) -> None:
        ...
        try:
            ...
            if output_dir:
                PathValidator.create_directory_if_needed(output_dir, 'Output directory')
            with open(output_path, 'w', encoding='utf-8') as f:
                f.write(content)
            ServiceLogger.log_operation_success(logger, 'write output', output_path=output_path)
        except Exception as e:
            ErrorHandler.handle_file_error(e, 'write', output_path, context={'content_length': len(content)})

class ConfigRenderingService:
    """Service for rendering and loading configuration data."""

    def __init__(self, template_renderer: TemplateRenderer=None, config_parser: ConfigParser=None, output_writer: OutputWriter=None):
        """
    ...
    def _safe_load_yaml(self, raw: str, src: Path) -> AgentConfigMap:
        ...
        if not raw.strip():
            raise ConfigurationError('Configuration file is empty', context={'file_path': str(src), 'operation': 'load_yaml'})
        try:
    ...
    def _validate_agent_config_block(self, config: AgentConfigMap, agent_name: str) -> None:
        ...
        if 'actions' in config and 'name' in config:
            ...
            for action in actions:
                agent_entry = {'agent_type': action.get('name', 'unknown'), 'name': action.get('name'), 'model_vendor': action.get('vendor', 'openai'), 'model_name': action.get('model', 'gpt-4'), 'is_operational': True, 'dependencies': [], 'granularity': action.get('granularity', 'record'), 'run_mode': 'online', 'few_shot': action.get('few_shot', 0), 'json_mode': action.get('json_mode', True)}
    ...
    def render_and_load_config(self, agent_name: str, config_path: Union[str, Path], template_dir: Union[str, Path], output_dir: Union[str, Path]) -> AgentConfigMap:
        ...
        """
        ServiceLogger.log_operation_start(logger, 'render and load config', agent_name=agent_name, config_path=str(config_path), template_dir=str(template_dir), output_dir=str(output_dir))
        config_path_str = str(config_path)
        template_dir_str = str(template_dir)
        output_dir_str = str(output_dir)
        cfg_path = Path(config_path)
        if not cfg_path.exists():
            raise ConfigurationError('Configuration file not found', context={'file_path': str(cfg_path), 'operation': 'render_and_load_config'})
        if cfg_path.is_dir():
            raise ConfigurationError('Expected a YAML/JSON file, got a directory', context={'file_path': str(cfg_path), 'operation': 'render_and_load_config'})
        rendered_template = self.template_renderer.render(config_path_str, template_dir_str, output_dir_str)
        ...
        try:
            ...
            raise ConfigurationError('Schema validation failed', context={'agent_name': agent_name, 'template_dir': str(template_dir), 'operation': 'validate_schema'}, cause=e)
