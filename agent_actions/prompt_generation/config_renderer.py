...
from agent_actions.response_processing.config_types import (
    AgentEntryDict, AgentConfigMap)
...
class TemplateRenderer(ABC):  # pylint: disable=too-few-public-methods
    ...
    @abstractmethod
    def render(self, config_path: str, template_dir: str,
...
class JinjaTemplateRenderer(TemplateRenderer):  # pylint: disable=too-few-public-methods
    ...
    def render(self, config_path: str, template_dir: str,  # pylint: disable=too-many-locals
               output_path: Optional[str]=None) -> str:
        """
        ...
        try:
            ServiceLogger.log_operation_start(
                logger, 'render template', user_facing=True,
                config_path=config_path, template_dir=template_dir,
            ...
            if output_path:
                ...
                if not path_validator.validate(data_output_dir):
                    ...
                    output_file_to_write = str(
                        output_dir_as_path / f'{config_name_stem}.yml')
            else:
                logger.info(
                    'No output path provided; template will be '
                    'rendered to memory/stdout.')
            if not all_validations_passed:
                error_prefix = (
                    'Input validation failed for template rendering: \n')
                formatted_errors = '\n'.join(
            ...
            logger.info(
                ...
                'Proceeding to render template.')
            rendered_template = render_pipeline_with_templates(
                config_path, template_dir)
            if output_file_to_write:
                with open(output_file_to_write, 'w',
                          encoding='utf-8') as f:
                    f.write(rendered_template)
                logger.info(
                    'Rendered template saved to: %s', output_file_to_write)
...
class YAMLConfigParser(ConfigParser):  # pylint: disable=too-few-public-methods
    ...
    def parse(self, config_data: str) -> AgentConfigMap:
        ...
        try:
            ServiceLogger.log_operation_start(
                logger, 'parse YAML configuration')
            if not config_data:
            ...
            ErrorHandler.handle_config_error(
                e, 'parse', 'YAML configuration',
                context={'config_data': config_data})
            raise
        except Exception as e:
            ErrorHandler.handle_config_error(
                e, 'parse', 'configuration',
                context={'config_data': config_data})
            raise
...
class FileOutputWriter(OutputWriter):  # pylint: disable=too-few-public-methods
    ...
    def write(self, output_path: str, content: str) -> None:
        ...
        try:
            ServiceLogger.log_operation_start(
                logger, 'write output', output_path=output_path)
...
class ConfigRenderingService:  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, template_renderer: TemplateRenderer=None,
        ...
        self.template_renderer = (
            template_renderer or JinjaTemplateRenderer())
    ...
    def _validate_agent_config_block(  # pylint: disable=too-many-locals,too-many-branches
            self, config: AgentConfigMap,
            agent_name: str) -> None:
        """
        Validate the config - handle both old and new formats.
        """
        current_directory = Path.cwd()
        project_root_path = AgentManager.find_project_root(
            start_path=current_directory)
        if 'actions' in config and 'name' in config:
            ...
            for action in actions:
                ...
                if action.get('kind') == 'tool':
                    ...
                    agent_entry['model_name'] = action.get(
                        'impl', action.get('name'))
                if (action.get('schema') or
                        action.get('output_schema')):
                    schema_value = (
                        action.get('schema') or
                        action.get('output_schema'))
                ...
                try:
                    entry_model = AgentConfig.model_validate(
                        agent_entry)
                    validated_entries.append(
                        entry_model.model_dump(exclude_unset=True))
                except ValidationError as e:
                    raise ConfigValidationError(
                        config_key='action_configuration',
                        reason='Invalid action configuration',
                        context={'action_name': action.get(
                            'name', 'unknown'),
                                 'agent_name': agent_name},
                        cause=e) from e
            config['_validated_actions'] = validated_entries
        else:
            agent_entries_list = cast(
                List[AgentEntryDict], config.get(agent_name))
        ...
        if not config_validator_instance.validate(
                validation_payload):
    ...
    def render_and_load_config(
            self, agent_name: str, config_path: Union[str, Path],
            template_dir: Union[str, Path],
            output_dir: Union[str, Path]) -> AgentConfigMap:
        """
        ...
        """
        ServiceLogger.log_operation_start(
            logger, 'render and load config', agent_name=agent_name,
        ...
        if not cfg_path.exists():
            raise ConfigurationError(
                ...
                context={'file_path': str(cfg_path),
                         'operation': 'render_and_load_config'})
        if cfg_path.is_dir():
            raise ConfigurationError(
        ...
        try:
            ...
            schema_validate_instance.validate(agent_name,
                                              Path(template_dir))
        except Exception as e:  # pylint: disable=broad-exception-caught
            raise ConfigurationError(
...
class ConfigRenderer:  # pylint: disable=too-few-public-methods
    ...
    def _safe_load_yaml(self, raw: str, src: Path) -> AgentConfigMap:
        ...
        try:
            ...
            raise ConfigValidationError(
                ...
                context={'file_name': src.name,
                         'line': mark.line + 1,
                         'column': mark.column + 1,
                         'problem': msg,
                         'operation': 'parse_yaml'}) from exc
    ...
    @staticmethod
    @as_validation_error(ConfigValidationError)
    def render_and_load_config(agent_name: str, config_path: Path,
                               template_dir: Path,
                               output_dir: Path) -> AgentConfigMap:
        """
        ...
        return service.render_and_load_config(
            agent_name, config_path, template_dir, output_dir)
