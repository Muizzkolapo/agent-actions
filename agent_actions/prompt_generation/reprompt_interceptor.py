"""Interceptor that generates improved prompts on validation failure."""
from __future__ import annotations

from typing import Any

from agent_actions.errors import ConfigurationError
from agent_actions.prompt_generation.reprompt_strategy import (
    LLMRepromptStrategy,
    RepromptContext,
    RepromptStrategy,
    TemplateRepromptStrategy,
)
from agent_actions.response_processing.base import (
    InterceptorResult,
    ResponseInterceptor,
)

class RepromptInterceptor(ResponseInterceptor):
    """Interceptor that generates improved prompts on validation failure."""

    def __init__(self) -> None:
        self.strategy: RepromptStrategy | None = None
        self.max_attempts: int = 3
        self.prompt_debug: bool = False

    def configure(self, config: dict[str, Any]) -> None:
        """Configure the reprompt strategy based on provided config."""
        self.prompt_debug = config.get('prompt_debug', False)
        if self.prompt_debug:
            print('🔄 REPROMPT INTERCEPTOR CONFIGURE:')
            print(f'   Config received: {config}')
        strategy_type = config.get('strategy', 'llm')
        self.max_attempts = config.get('max_attempts', 3)
        if self.prompt_debug:
            print(f'   Strategy type: {strategy_type}')
            print(f'   Max attempts: {self.max_attempts}')
        if strategy_type == 'llm':
            llm_config = config.get('llm_config', {})
            llm_config['prompt_debug'] = self.prompt_debug
            if self.prompt_debug:
                print(f'   LLM config: {llm_config}')
            self.strategy = LLMRepromptStrategy(llm_config)
        elif strategy_type == 'simple':
            simple_config = {
                'include_previous_response': config.get(
                    'include_previous_response',
                    True
                ),
                'prompt_debug': self.prompt_debug,
            }
            if 'feedback_template' in config:
                simple_config['feedback_template'] = config['feedback_template']
            if self.prompt_debug:
                print(f'   Simple config: {simple_config}')
            self.strategy = LLMRepromptStrategy(simple_config)
        elif strategy_type == 'template':
            self.strategy = TemplateRepromptStrategy(
                config.get('templates', {}),
                prompt_debug=self.prompt_debug
            )
        else:
            raise ConfigurationError(
                'Unknown reprompt strategy',
                context={
                    'strategy_type': strategy_type,
                    'supported_strategies': ['llm', 'simple', 'template'],
                    'suggestion': (
                        f"Use one of the supported strategies: 'llm', "
                        f"'simple', or 'template'. Got '{strategy_type}'"
                    ),
                }
            )
        if self.prompt_debug:
            strategy_name = type(self.strategy).__name__
            print(f'   Created strategy: {strategy_name}')

    def intercept(self, response: Any, context: dict[str, Any]) -> InterceptorResult:
        """Intercept and handle validation failures with improved prompts."""
        if self.prompt_debug:
            print('🧠 REPROMPT INTERCEPTOR INTERCEPT:')
            print(f'   Context keys: {list(context.keys())}')
            has_error = 'validation_error' in context
            print(f'   Has validation_error: {has_error}')
        if 'validation_error' not in context:
            if self.prompt_debug:
                print('   No validation error - continuing')
            return InterceptorResult(continue_processing=True)
        attempt = context.get('attempt', 0)
        if self.prompt_debug:
            check_result = attempt >= self.max_attempts
            print(f'   Current attempt: {attempt}, Max attempts: {self.max_attempts}')
            print(f'   Check: {attempt} >= {self.max_attempts} = {check_result}')
        if attempt >= self.max_attempts:
            if self.prompt_debug:
                print('   MAX ATTEMPTS REACHED - stopping')
            return InterceptorResult(
                continue_processing=False,
                metadata={'max_attempts_reached': True}
            )
        validation_criteria = context.get('validator_args', {}).copy()
        excluded_keys = {
            'validator_args',
            'validation_error',
            'prompt',
            'original_prompt',
            'attempt',
            'agent_config',
            'history',
            'failed_response',
        }
        for key, value in context.items():
            if key not in excluded_keys:
                validation_criteria[key] = value
        reprompt_context = RepromptContext(
            original_prompt=context.get(
                'original_prompt',
                context.get('prompt')
            ),
            validation_error=context['validation_error'],
            validation_criteria=validation_criteria,
            attempt_number=attempt + 1,
            failed_response=context.get('failed_response'),
            agent_config=context.get('agent_config', {}),
        )
        if not self.strategy:
            raise ConfigurationError(
                'Reprompt strategy not configured',
                context={
                    'attempt': attempt,
                    'max_attempts': self.max_attempts,
                }
            )
        improved_prompt = self.strategy.generate_improved_prompt(
            reprompt_context
        )
        history = context.get('history', []) + [
            {
                'attempt': attempt,
                'prompt': context.get('prompt'),
                'error': context['validation_error'],
            }
        ]
        return InterceptorResult(
            continue_processing=False,
            retry_context={
                'prompt': improved_prompt,
                'original_prompt': reprompt_context.original_prompt,
                'attempt': attempt + 1,
                'history': history,
            }
        )
