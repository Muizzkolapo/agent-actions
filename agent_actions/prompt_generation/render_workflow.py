    ...
    for template_file in template_files:
        try:
            ...
            print(
                ...
                f"not found in {templates_folder}."
            )
            ...
            raise TemplateRenderingError(
                f"Unexpected error loading template '{template_file}': "
                f"{safe_format_error(e)}",
                context={
                    'template_file': template_file,
                    'templates_folder': templates_folder
                },
...
def _save_failed_render(rendered_yaml_content, workflow_name):
    ...
    """
    cache_dir = (
        Path.cwd() / '.agent-actions' / 'cache' / 'rendered_workflows'
    )
    cache_dir.mkdir(parents=True, exist_ok=True)
    failed_render_path = cache_dir / f'{workflow_name}_failed.yml'
    try:
        with open(
            failed_render_path, 'w', encoding='utf-8'
        ) as f:
...
def render_pipeline_with_templates(yaml_path, templates_folder):
    ...
    try:
        ...
        raise TemplateRenderingError(
            f"Error rendering YAML template from '{yaml_path}': "
            f"{safe_format_error(e)}",
            context={
                'yaml_path': yaml_path,
                'templates_folder': templates_folder
    ...
    try:
        ...
        saved_file_msg = _save_failed_render(
            rendered_yaml_content, Path(yaml_path).stem
        )
        raise ConfigurationError(
            f'Error parsing YAML after template '
            f'rendering{saved_file_msg}',
            context={
                'yaml_path': yaml_path,
