...
class ProcessorErrorHandlerMixin:
    ...
    def handle_partial_failure(
        ...
        if failure_rate > threshold:
            error_summary = {
                ...
                'failure_rate': failure_rate,
                'threshold': threshold,
                'sample_errors': [str(e) for e in errors[:5]]  # First 5 errors
            }
            
            raise ProcessorError(
                f"Batch operation '{operation}' exceeded failure threshold: "
                f"{failure_rate:.2%} > {threshold:.2%}. "
                f"Failed: {len(errors)}/{total_items} items. "
                f"Details: {json.dumps(error_summary)}"
            )
    ...
    def _get_recovery_instructions(
        self,
        operation: str,
        error: Exception
    ) -> Dict[str, Any]:
