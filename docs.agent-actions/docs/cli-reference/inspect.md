...
Dependency Analysis: my_workflow

┏━━━━━━━━━━━━━━━━━━━┳━━━━━━━━━━━━━━━━┳━━━━━━━━━━━━━━━━━━┳━━━━━━━━━━━━━━━━━━━━━┓
┃ Action            ┃ Input Sources  ┃ Context Sources  ┃ Type                ┃
┡━━━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━━━━┩
│ extract_facts     │ source data    │ none             │ Source              │
│ validate_facts    │ extract_facts  │ none             │ Transform           │
│ generate_question │ validate_facts │ extract_facts    │ Transform + Context │
└───────────────────┴────────────────┴──────────────────┴─────────────────────┘
```
...
Flow: extract_facts → validate_facts → generate_question

Actions
├── extract_facts (Source)
│   ├── ← source data
│   └── → candidate_facts_list
├── validate_facts (Transform)
│   ├── ← extract_facts
│   └── → validated_facts, quality_score
└── generate_question (Transform + Context)
    ├── ← validate_facts
...
Input Fields (from context_scope)
├── observe:
│   └── • extract_facts.candidate_facts_list
└── passthrough:
    └── • validate_facts.quality_score
...
```json
{
  ...
  "kind": "llm",
  "model": "gpt-4o-mini",
  "input_sources": ["validate_facts"],
  "context_sources": ["extract_facts"],
  "context_scope": {
    "observe": ["extract_facts.candidate_facts_list"],
    "passthrough": ["validate_facts.quality_score"]
  },
  "output_fields": ["question", "options", "answer"]
}
```
