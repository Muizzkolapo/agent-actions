...
| `AGAC_RECORD_LIMIT` | unset | Cap each action at this many records per input file, overriding a larger `record_limit` and applying to actions that set none. [`--record-limit`](../cli/run#running-a-project-smaller-than-it-is) takes precedence when both are set, and can raise this value as well as lower it. For running a project cheaply from outside it — a smoke check, a quick sanity pass. On a workflow you have already run it is not read-only where it bites: an action whose stored run processed more records than this value re-executes under it, and its output is rewritten to the smaller set. One it is too large to have cut is left alone. Unset it, or use a scratch project, when the stored output matters. An action that actually drops records logs a warning naming this variable, since a truncated run otherwise looks complete; an action that had nothing to drop stays quiet. A value below 1, or one that is not an integer, fails the run rather than being ignored. Never excludes a record that [`agac retry`](../cli/retry) is re-running, since retry selects by id. |
