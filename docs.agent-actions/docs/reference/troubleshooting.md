...
| `function` | UDF that failed | `add_answer_text` |
...
**Cause:** Schema requires field but UDF didn't return it.

**Fix:** Ensure UDF returns all required fields:
```python
...
| `X is a required property` | Return field from UDF or use `reprompt` |
