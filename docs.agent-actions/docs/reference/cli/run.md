...
| `--fresh` | Clear stored results, dispositions, status, and event logs (`events.json`, `errors.json`) before execution, and discard any batch this workflow submitted and never collected. Gives a clean slate for debugging. |
