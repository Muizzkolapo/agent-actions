...
| `--max-records N` | Cap every action at N records, whatever the workflow config sets. Applies to actions that set no limit of their own, and takes precedence over `AGAC_MAX_RECORDS`. Each capped action says so, since a truncated run otherwise looks complete |
...
[`agac retry`](./retry) takes the same flag.
