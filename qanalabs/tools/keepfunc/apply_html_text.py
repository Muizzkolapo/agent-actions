...
def parse_and_style_combined_explanation(combined_text: str) -> str:
    ...
    This ensures options get the overlay vibe even when coming from combined_explanation.
    """
    ...
    # Apply styling by wrapping option content in .option divs
    # Look for patterns like "**Option 1:** text" or "**Option:** text"
    
    # Pattern for correct answers section
    html_content = re.sub(
        r'<p><strong>Option(?:\s+\d+)?:</strong>\s*([^<]+)</p>',
        r'<div class="option"><strong>Option:</strong> \1</div>',
        html_content
    )
    
    # Pattern for distractor sections - wrap the option text after headings
    # This handles "### Distractor 1:" followed by option text
    html_content = re.sub(
        r'(<h3[^>]*>Distractor \d+:</h3>)\s*<p>([^<]+)</p>',
        r'\1<div class="option"><strong>\2</strong></div>',
        html_content
    )
    
    # Additional pattern for any standalone option text that should be styled
    # Look for paragraph that looks like an option (starts with specific patterns)
    html_content = re.sub(
        r'<p>((?:Integrate|Train|Allow|Use|Store|Apply)[^<]*)</p>',
        r'<div class="option"><strong>\1</strong></div>',
        html_content
    )
...
def format_explanation_from_options_combined(options_combined: List[Dict]) -> str:
    ...
    Handles both SA and MA formats with rich styling (overlay vibe).
    """
...
''']
    ...
    # Correct answer(s) section with rich styling
    if len(correct_items) == 1:
        html_parts.append('<h2 class="correct">Correct Answer:</h2>')
        item = correct_items[0]
        html_parts.append(f'<div class="option"><strong>{escape(item["option"])}</strong></div>')
        ...
        for i, item in enumerate(correct_items, 1):
            html_parts.append(f'<div class="option"><strong>Option {i}:</strong> {escape(item["option"])}</div>')
        
        # Add shared explanation (assuming all correct answers have same explanation)
    ...
    # Incorrect answers section with rich styling
    if distractor_items:
